...
Trong phỏng vấn, bạn thường gặp câu hỏi này: trên một máy, TCP đã lắng nghe `8080`, vậy UDP có thể lắng nghe tiếp `8080` không?

Kết luận trước: **Có thể. Namespace bind port của TCP và UDP được phân biệt theo giao thức transport layer, nên cùng một số port ở các giao thức khác nhau sẽ không xung đột.** Một process lắng nghe `TCP/8080`, process khác lắng nghe `UDP/8080`, kernel sẽ phân phối riêng theo protocol stack.

## Port number do ai quản lý?

Port là số hiệu mà transport layer dùng để phân biệt các application process. Địa chỉ IP định vị host, port number định vị application cụ thể trên host đó.

Header của TCP và UDP đều có trường source port và destination port, độ dài đều là 16 bit (16 bits), nên phạm vi port number đều là `0~65535`. Tuy nhiên, port `0` thường có ý nghĩa đặc biệt trong API thực tế, chẳng hạn để hệ thống tự cấp temporary port, nên không phù hợp để giải thích như một port lắng nghe của service thông thường.

**Cùng phạm vi số không có nghĩa là cùng đối tượng bind**. Trong service registration, listening, packet capture, firewall và security group rule, thường phải xem transport layer protocol cùng với port, chẳng hạn `TCP/53`, `UDP/53`, `TCP/443`, `UDP/443`.

`TCP/443` và `UDP/443` chỉ giống nhau về số, còn đường xử lý trong protocol stack khác nhau. Sau khi nhận IP packet, kernel trước tiên xem protocol identifier của IP layer: trong IPv4 là trường Protocol, còn trong IPv6 tương ứng là Next Header. Protocol number của TCP là `6`, của UDP là `17`. Trước khi phân phối port, kernel đã dựa trên protocol number để chuyển packet đến TCP hoặc UDP protocol stack tương ứng.
...
TCP và UDP đều nằm ở transport layer nhưng khác nhau rất nhiều. Bảng dưới đây so sánh theo 8 khía cạnh để giúp hình thành nhận thức tổng thể:
...
| **Độ tin cậy**          | Reliable (sequence number, ACK, retransmission, flow control, congestion control)                       | Unreliable, giao hàng với nỗ lực tối đa         |
| **Duy trì state**       | Stateful, duy trì thông tin connection                                                                  | Stateless, gửi xong thì không quản lý nữa       |
...
Chính vì TCP và UDP là hai transport layer protocol hoàn toàn độc lập, kernel mới tách chúng ra xử lý trước khi phân phối port.

## Vì sao không xung đột khi bind socket?

Server program thường tạo socket trước, sau đó dùng `bind()` để bind local IP và port. Một TCP socket bind `8080`, một UDP socket khác cũng bind `8080`, thông thường có thể cùng tồn tại. Khi xác định xung đột, kernel không chỉ xem số port mà còn xem protocol, local address và các thông tin khác.
...
Có thể dùng `nc` để thử nhanh. Tuy nhiên, implementation `nc` giữa các system khác nhau không hoàn toàn giống nhau, cách viết tham số `-l`, `-u` và port cũng có thể khác. Dưới đây là cách viết thường gặp của OpenBSD netcat; nếu command báo lỗi, trước tiên có thể dùng `nc -h` để xem help của máy hiện tại.
...
Hai command có thể cùng tồn tại. Trên Linux, có thể kiểm tra thêm:
...
Nếu muốn tránh khác biệt về tham số của `nc`, cũng có thể dùng code để xác minh: trong Java, `ServerSocket(8000)` và `DatagramSocket(8000)` có thể được tạo đồng thời; trong Go, `net.Listen("tcp", ":8000")` và `net.ListenPacket("udp", ":8000")` cũng có thể cùng tồn tại. Nếu lắng nghe lặp lại một lần nữa bằng cùng protocol, thông thường sẽ thấy lỗi address đã được sử dụng.

## Khi nào xảy ra xung đột?

![Khi nào TCP xảy ra xung đột port](https://oss.javaguide.cn/github/javaguide/cs-basics/network/when-does-tcp-conflict-occur.png)

TCP và UDP không xung đột không có nghĩa là có thể tùy ý bind lặp lại port.
...
**TCP và UDP lắng nghe cùng một số port riêng biệt dựa trên sự khác nhau về protocol, không cần `SO_REUSEPORT`. `SO_REUSEADDR` / `SO_REUSEPORT` chủ yếu ảnh hưởng việc reuse address và port, restart nhanh và lắng nghe bằng nhiều process trong cùng một protocol; việc có được phép hay không và phân phối như thế nào còn phụ thuộc operating system và loại socket cụ thể.**
...
### HTTPS và 443 của HTTP/3 cũng tương tự
...
Điều này không xung đột với `TCP/443` cũ. Một server hoàn toàn có thể đồng thời cung cấp:
...
Trong production environment cũng cần chú ý: nếu chỉ allow `TCP/443`, HTTP/1.1 và HTTP/2 có thể đều hoạt động bình thường, nhưng HTTP/3 sẽ không hoạt động. Cần kiểm tra riêng `TCP/443` và `UDP/443` trên cloud security group, load balancer, Nginx / gateway và host firewall, sau đó dùng `curl --http3` hoặc browser developer tools để xác nhận protocol có thực sự chuyển sang HTTP/3 hay không.

## Trả lời thế nào trong phỏng vấn?

TCP và UDP có thể sử dụng cùng một số port vì chúng là các transport layer protocol khác nhau; kernel trước tiên phân phối theo IP protocol number đến TCP stack hoặc UDP stack, sau đó tìm socket theo address và port trong từng protocol stack tương ứng, vì vậy `TCP/8080` và `UDP/8080` có thể cùng tồn tại.

Xung đột thực sự dễ xảy ra là bind trong cùng protocol, chẳng hạn hai TCP service thường không thể đồng thời lắng nghe cùng một local IP và port; khi đó mới liên quan đến các socket reuse option như `SO_REUSEADDR`, `SO_REUSEPORT`. Chỉ cần nhớ hai ví dụ là đủ: DNS đồng thời sử dụng `UDP/53` và `TCP/53`; HTTP/3 thường được deploy với `UDP/443`, có thể cùng tồn tại với `TCP/443` của HTTPS truyền thống.
