...
Khi nhập URL vào thanh địa chỉ trình duyệt và hiển thị trang, phía sau sẽ kết nối nhiều khâu như DNS, TCP, TLS, HTTP, ARP, đóng gói dữ liệu và trình duyệt render.

Câu hỏi này thường được dùng để kiểm tra hiểu biết tổng thể về mạng, vì nó có thể kết nối các kiến thức ở application layer, transport layer, network layer và link layer. Chỉ học thuộc từng protocol riêng lẻ rất dễ bị rời rạc; đi qua toàn bộ quy trình truy cập trang web một lần sẽ rõ ràng hơn nhiều.
...
1. **Trình duyệt phân tích URL và kiểm tra cache**: Trình duyệt phân tích các thành phần của URL và kiểm tra HTTP cache xem đã có bản sao hợp lệ của resource đó chưa (strong cache, negotiated cache).
2. **DNS resolution**: Trình duyệt thông qua DNS protocol để lấy IP address tương ứng với domain name.
3. **Thiết lập transport connection**: HTTP/1.1 và HTTP/2 thường trước tiên thiết lập connection thông qua TCP three-way handshake; HTTP/3 thì thiết lập QUIC connection dựa trên UDP.
...
URL (Uniform Resource Locator, bộ định vị resource thống nhất) là địa chỉ duy nhất của resource trên Internet. Mỗi resource có thể truy cập trên mạng đều tương ứng với một URL; về lý thuyết, file và URL tương ứng một-một. Trên thực tế cũng có ngoại lệ, chẳng hạn trong trường hợp redirect hoặc CDN, nhiều URL có thể trỏ đến cùng một resource.
...
1. **Protocol** (Scheme): Prefix của URL biểu thị protocol được sử dụng; phổ biến nhất là `http` và `https`, ngoài ra còn có `ftp:` dùng để truyền file.
2. **Domain name** (Host): Tên chung của mục tiêu truy cập, cũng có thể dùng trực tiếp IP address. Về bản chất, domain name là phiên bản dễ đọc của IP address.
...
4. **Resource path** (Path): Bắt đầu từ `/` đầu tiên, biểu thị vị trí resource trên server. Trong thiết kế ban đầu, path tương ứng với file vật lý trên server; hiện nay thường là virtual path được backend route mapping.
...
DNS (Domain Name System, hệ thống domain name) giải quyết **bài toán mapping giữa domain name và IP address**. Domain name chỉ là tên thuận tiện cho con người ghi nhớ; network communication thực sự cần IP address.

### Quy trình DNS resolution

Sau khi nhận domain name, DNS resolution của trình duyệt thường diễn ra theo các bước sau:
...
3. **Local DNS server**: Local DNS server được operating system cấu hình (thường do ISP cung cấp hoặc dùng public DNS như `8.8.8.8`, `114.114.114.114`). Nếu local DNS server có cache chưa hết hạn thì trả kết quả trực tiếp.
4. **Recursive/iterative query**: Khi local DNS server không hit cache, nó thay client thực hiện iterative query: trước tiên hỏi root DNS server, sau đó hỏi top-level domain DNS server (chẳng hạn `.com`), cuối cùng hỏi authoritative DNS server để lần lượt lấy IP address mục tiêu.
...
Hình dưới đây minh họa một quy trình DNS iterative query điển hình:
...
Sau khi lấy được IP address của server mục tiêu, trình duyệt cần thiết lập transport connection với server. Sau đây trước tiên giới thiệu TCP connection thường dùng bởi HTTP/1.1 và HTTP/2; nếu negotiate sử dụng HTTP/3 thì sẽ chuyển sang thiết lập QUIC connection, QUIC được xây dựng trên UDP.
...
3. **Handshake lần thứ ba (ACK)**: Client nhận được rồi gửi ACK, acknowledgement number `ack=y+1`; hai bên chuyển sang trạng thái `ESTABLISHED`, connection được thiết lập hoàn tất.

Thiết kế three-way handshake không nhằm mục đích «đi thêm một lần», mà để hai bên đều xác nhận rằng: đối phương có thể nhận data của mình và mình cũng có thể nhận data của đối phương. Two-way handshake không làm được điều này — sau handshake lần thứ hai, server vẫn chưa biết client đã nhận SYN+ACK của mình hay chưa.

> Để biết phân tích chi tiết về three-way handshake, half-connection queue/full-connection queue, SYN Flood protection, v.v., bạn có thể tham khảo [TCP three-way handshake và four-way handshake (Transport Layer)](https://javaguide.cn/cs-basics/network/tcp-connection-and-disconnection.html).

### Nếu là HTTPS: TLS handshake

Nếu sử dụng HTTPS dựa trên TCP, sau khi TCP connection được thiết lập còn phải thực hiện TLS handshake. HTTP/3 tích hợp TLS 1.3 trong quá trình thiết lập QUIC connection. Mục tiêu cốt lõi của TLS có ba phần: **encryption** (chống nghe lén), **authentication** (chống giả mạo), **integrity check** (chống sửa đổi).
...
Cần lưu ý rằng quy trình trên mô tả phương thức RSA-based key exchange trong TLS 1.2. HTTPS hiện đại thường dùng ECDHE key exchange; TLS 1.3 cũng có thể dùng PSK hoặc PSK+(EC)DHE trong các trường hợp như session resumption. Shared secret của ECDHE được hai bên tạo ra thông qua temporary key negotiation và không truyền trực tiếp trên network; chỉ có private key của server certificate thường không đủ để decrypt các historical session đã dùng temporary key negotiation trước đó. TLS 1.3 tiếp tục đơn giản hóa handshake, thường rút gọn full handshake xuống 1-RTT và loại bỏ static RSA key exchange.

Sau khi TLS handshake hoàn tất, các HTTP request và response tiếp theo đều dùng symmetric key đã negotiate để truyền dưới dạng encrypted. Tính an toàn của HTTPS đến từ TLS layer, không phải từ việc thay đổi bản thân HTTP protocol.
...
2. **Route distribution**: Dựa trên URL path để route request đến backend processing logic tương ứng (Controller, Servlet, v.v.).
3. **Business processing**: Thực thi business logic cụ thể, có thể liên quan đến database query, đọc cache, gọi service khác, v.v.
4. **Xây dựng response**: Đóng gói processing result thành HTTP response message.
...
- **Response body**: Nội dung thực tế được request, chẳng hạn HTML document, JSON data, binary data của ảnh, v.v.
...
Sau khi HTTP request được gửi từ trình duyệt, data không trực tiếp «bay» đến server. Nó cần trải qua quá trình đóng gói từng lớp của protocol stack, rồi được chuyển tiếp từng hop trên network vật lý đến đích.

### Quy trình data encapsulation

HTTP message ở application layer trải qua việc đóng gói từng lớp của transport layer, network layer và link layer, cuối cùng trở thành bit stream có thể truyền trên physical medium:
...
Mỗi layer chỉ quan tâm đến header mà mình cần thêm và sử dụng service do layer bên dưới cung cấp để truyền data:

- **Transport layer (TCP)**: Thêm source port và destination port, dùng sequence number và acknowledgement number để bảo đảm reliable transmission.
...
Mỗi router duy trì một routing table, tra bảng dựa trên destination IP address để quyết định next hop. Data packet trong network giống như một bưu kiện; mỗi trạm chỉ xem «gửi đến đâu ở trạm tiếp theo» mà không cần quan tâm path toàn chặng.

### ARP protocol: từ IP address đến MAC address

Khi data frame được truyền ở link layer, cần biết MAC address của thiết bị ở next hop chứ không thể chỉ dùng IP address. ARP (Address Resolution Protocol, address resolution protocol) giải quyết bài toán «đã biết IP address thì lấy MAC address tương ứng như thế nào».
...
4. Bên gửi request nhận response rồi lưu IP-MAC mapping vào ARP cache table, các communication sau dùng trực tiếp mapping này.
...
Trong phần lớn network gia đình và doanh nghiệp, host trong intranet dùng private IP address (chẳng hạn `192.168.x.x`), không thể route trực tiếp trên public network. NAT (Network Address Translation) protocol chịu trách nhiệm chuyển đổi IP address giữa intranet và public network.
...
Sau khi server trả về HTML response, công việc thực sự của trình duyệt mới bắt đầu. Trình duyệt cần phân tích HTML, xây dựng DOM tree, tải sub-resource, tính style, layout và cuối cùng render lên màn hình.
...
6. **Composite**: Composite các layer khác nhau thành khung hình cuối cùng để hiển thị trên màn hình.

### Tải sub-resource

HTML document thường tham chiếu rất nhiều resource bên ngoài:

- **CSS file** (`<link rel="stylesheet">`): Stylesheet blocking thông thường sẽ block rendering; trình duyệt thường chờ CSS load và parse xong mới layout và paint, vì CSS có thể thay đổi layout của element. Có thể thay đổi hành vi này thông qua attribute `media`, dynamic loading hoặc `preload`.
- **JavaScript file** (`<script>`): Mặc định sẽ block HTML parsing, vì JavaScript có thể sửa DOM. Có thể thay đổi hành vi loading thông qua attribute `async` hoặc `defer`.
...
Việc thực thi JavaScript sẽ block HTML parsing. Khi gặp tag `<script>`, trình duyệt tạm dừng xây dựng DOM, trước tiên download và execute script. Nếu script có DOM operation, có thể trigger DOM reconstruction và page reflow (Reflow) hoặc repaint (Repaint).
...
HTTP/2 đưa multiplexing vào trên nền long connection. Nhiều request và response có thể được truyền đồng thời trên cùng một TCP connection; data được tách thành các frame nhỏ hơn và phân biệt bằng stream (Stream). Điều này giải quyết head-of-line blocking ở application layer của HTTP/1.1 — request chậm ở phía trước sẽ không chặn request phía sau. Cần lưu ý rằng HTTP/2 vẫn dựa trên TCP; khi xảy ra packet loss ở TCP layer, mọi stream trên cùng connection đều bị ảnh hưởng (head-of-line blocking ở TCP layer). HTTP/3 dựa trên QUIC protocol (UDP) mới tiếp tục giảm nhẹ vấn đề này.

### Đóng connection

Khi connection thực sự cần đóng (bên chủ động đóng khởi xướng):
...
Sự tồn tại của trạng thái `TIME_WAIT` nhằm bảo đảm ACK cuối cùng có thể đến đầu bên kia, đồng thời để các message cũ còn sót lại trên network tiêu biến, tránh gây nhiễu connection mới tiếp theo.
...
2. **DNS resolution** → Lần lượt query browser cache, operating system cache, local DNS server; khi cần thì iterative query qua root → TLD → authoritative server để lấy target IP address.
...
6. **Data encapsulation và forwarding** → Request message được đóng gói từng lớp theo TCP → IP → link layer, truyền từng hop qua router, switch và các intermediate device khác đến server; response quay về trình duyệt theo chiều ngược lại.
...
Truy cập một trang web nhìn có vẻ đơn giản, nhưng thực tế kết nối gần như toàn bộ protocol cốt lõi của computer network. Hiểu theo quy trình này, các protocol như DNS, TCP, TLS, HTTP, IP, ARP không còn là những kiến thức rời rạc mà trở thành các khâu khác nhau trên một chain hoàn chỉnh.

## Tài liệu tham khảo

1. 《Computer Network (ấn bản 7)》
2. 《HTTP minh họa》
