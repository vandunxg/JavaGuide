---
title: "Vì sao TCP hướng theo byte stream, UDP hướng theo datagram? (Transport Layer)"
description: "Làm rõ khác biệt bản chất giữa byte stream của TCP và datagram của UDP, phân tích nguyên nhân và cách giải quyết sticky packet/packet splitting, bao quát các chủ đề phỏng vấn thường gặp như Nagle, Delayed ACK."
...
Lấy một ví dụ, application layer liên tiếp gửi hai message:
...
Vì vậy, cách gọi “TCP sticky packet/packet splitting” giống một hiện tượng nhìn từ góc nhìn của application layer hơn. Nói chính xác, TCP không có khái niệm “packet”, nó truyền một byte stream liên tục. Điều thực sự cần giải quyết là: **application-layer protocol định nghĩa ranh giới message như thế nào.**

#### Vì sao sticky packet và packet splitting xuất hiện?

Có một số nguyên nhân thường gặp.

**1. TCP là protocol dạng byte stream, không có ranh giới message của application layer.**
...
Đó cũng là lý do cơ chế encode/decode protocol rất quan trọng trong Netty, Dubbo, custom RPC, IM gateway và game service. Chỉ cần tầng bên dưới dùng TCP thì application layer bắt buộc phải định nghĩa rõ ranh giới message.
...
Cách này dễ triển khai nhưng kém linh hoạt. Message ngắn phải được padding, gây lãng phí không gian; message dài thì phải tách thành nhiều phần. Nó phù hợp với trường hợp format message rất cố định, không phù hợp lắm với protocol nghiệp vụ tổng quát.

**2. Delimiter**

Thêm delimiter đặc biệt giữa các message, ví dụ ký tự newline `\n`, `\r\n`, hoặc end marker tự định nghĩa.
...
Đây là cách phổ biến hơn trong thực tế. Đặt cố định một length field trong protocol header để biểu thị message body phía sau có bao nhiêu byte.
...
Length header cũng có điểm cần chú ý. Phải quy ước byte order cho length field, thường dùng network byte order; đồng thời phải giới hạn độ dài body tối đa để tránh phía đối diện gửi một giá trị length quá lớn làm cạn memory. Khi parse protocol trong production, không thể chỉ xét đường đi bình thường mà còn phải xử lý half packet, length bất thường, connection đóng giữa chừng, request độc hại và các trường hợp khác.
...
Mục tiêu của Nagle algorithm là giảm số lượng packet nhỏ. Khi băng thông network thời kỳ đầu còn hạn chế, nếu application mỗi lần chỉ ghi 1 byte nhưng header TCP/IP lại có hàng chục byte, network sẽ đầy các “packet nhỏ”, hiệu suất rất thấp. RFC 896 thảo luận về small-packet problem này và đề xuất rằng khi connection còn data chưa được ACK, data nhỏ mới có thể tạm hoãn gửi, đợi ACK đến rồi mới tiếp tục gửi.

Delayed ACK là một tối ưu ở phía nhận. Sau khi nhận data, phía nhận không nhất thiết gửi ACK ngay mà có thể đợi một khoảng thời gian ngắn, xem có thể gửi ACK cùng với data cần trả về hay không, nhằm giảm số packet chỉ chứa ACK. RFC 9293 cũng gọi chiến lược “ít hơn một ACK cho mỗi data segment” này là delayed ACK.

Xét riêng, hai cơ chế này đều có lý, nhưng khi kết hợp có thể khuếch đại latency. Tình huống điển hình là:
...
Data nhỏ A đã được gửi đi, data nhỏ B có thể bị Nagle algorithm tạm giữ trong send buffer để chờ ACK của A. Sau khi server nhận A, nếu tạm thời không có response nghiệp vụ cần trả về thì Delayed ACK lại có thể trì hoãn việc gửi ACK. Vì vậy, phía gửi chờ ACK, phía nhận chờ thêm data hoặc chờ delayed ACK timer, khiến latency bị khuếch đại.
...
- Các lần ghi nhỏ có thể gộp được thì nên gộp ở application layer thành một message hoàn chỉnh trước, rồi gọi `write()` một lần.
...
- Với trường hợp ưu tiên throughput và muốn gom đủ data rồi mới gửi, có thể đánh giá `TCP_CORK` trên Linux, nhưng nó không phù hợp khi viết code đa nền tảng.
- Trước khi điều chỉnh tham số, hãy bắt packet để xác nhận, đừng thấy “chậm” là lập tức sửa socket option.

Trong Java, nhiều network framework cung cấp config `TCP_NODELAY`, ví dụ `ChannelOption.TCP_NODELAY` của Netty. Nó thực sự có thể giảm thời gian chờ của message nhỏ, nhưng cũng có thể làm tăng số packet nhỏ. Với service QPS cao, cần xem trade-off này cùng với message size, RTT, throughput, CPU và số packet qua NIC. Linux `tcp(7)` cũng nêu rằng `TCP_NODELAY` sẽ tắt Nagle algorithm, còn `TCP_CORK` dùng để tránh gửi frame chưa hoàn chỉnh và đợi application xác nhận “có thể gửi” rồi mới gửi.

#### Trả lời thế nào khi phỏng vấn?

Có thể trả lời như sau:

TCP hướng theo byte stream. Dữ liệu do application layer ghi vào buffer của kernel; TCP chỉ đảm bảo các byte này đến phía đối diện một cách tin cậy, đúng thứ tự, không đảm bảo một lần `send()` tương ứng một lần `recv()`, cũng không giữ ranh giới message của application layer. Vì vậy, phía nhận có thể đọc nhiều message trong một lần, cũng có thể chỉ đọc được nửa message. Đây là hiện tượng thường gọi là sticky packet, packet splitting.
...
Bản chất của việc giải quyết TCP sticky packet/packet splitting là application-layer protocol tự định nghĩa ranh giới message. Các cách thường gặp gồm độ dài cố định, delimiter và length header. Trong thực tế, length header được dùng phổ biến hơn vì phù hợp hơn với binary protocol và message có độ dài thay đổi, nhưng phải xử lý byte order, giới hạn độ dài tối đa, buffer cho phần packet chưa hoàn chỉnh và việc connection đóng giữa chừng.
