...
Nhưng nói chính xác thì HTTPS chưa bao giờ đồng nghĩa với RSA encryption. Ngay cả trong thời đại TLS 1.0, TLS 1.1, RSA cũng chỉ là một trong các phương án có thể chọn; trong protocol vẫn có những phương thức key exchange như DHE. Đến TLS 1.3, static RSA key exchange đã bị loại bỏ, RSA chủ yếu xuất hiện ở các vị trí như certificate signature và authentication.

Vì vậy, điều bài viết này thực sự muốn so sánh không phải là “RSA và ECDHE, cái nào cao cấp hơn”.

**Trong RSA handshake, session key material được client tạo rồi mã hóa để gửi cho server; trong ECDHE handshake, session key material không được truyền trực tiếp mà do client và server tự tính ra.**
...
Khi đã làm rõ các câu hỏi này, bạn sẽ hiểu được `PreMasterSecret`, `Server Key Exchange`, forward secrecy và lý do TLS 1.3 loại bỏ static RSA.
...
## Hai vấn đề cốt lõi trong TLS handshake
...
Sau khi handshake hoàn tất, dữ liệu nghiệp vụ thực sự thường được bảo vệ bằng các symmetric encryption algorithm như AES-GCM, chứ không phải bằng RSA để mã hóa toàn bộ request và response.

Ở đây có hai vấn đề.

**Vấn đề thứ nhất: browser và server cần negotiate một session key.**

Khi truyền HTTP request, Cookie và response body sau đó, hai bên sẽ dùng session key này cho symmetric encryption. Symmetric encryption phù hợp hơn với việc xử lý lượng lớn dữ liệu; asymmetric encryption có chi phí tính toán cao nên thường không được dùng để trực tiếp mã hóa toàn bộ nội dung web.

**Vấn đề thứ hai: browser cần xác nhận đối phương thực sự là website mục tiêu.**

Nếu chỉ “server gửi một public key cho browser”, thì attacker trung gian cũng có thể gửi public key của mình. Browser tưởng đó là public key của website mục tiêu, rồi mã hóa thông tin bí mật gửi cho attacker. Certificate, CA và digital signature giải quyết vấn đề này: chứng minh public key thực sự được liên kết với domain đó, chứ không phải do ai đó trên đường truyền chèn vào.
...
Lúc này, client và server đều có ba thành phần:
...
Hai bên tiếp tục derive `Master Secret` từ ba thành phần này, rồi session key dùng về sau cũng tiếp tục được derive từ đó. Khi thực sự truyền HTTP request và response, các symmetric key được derive này mới là thứ được sử dụng.

Tóm gọn trong một câu:

**Session key material của RSA handshake do client tạo rồi “đóng gói” gửi cho server.**

Ở đây, “đóng gói” dựa vào RSA public key của server. Chỉ server có RSA private key tương ứng mới có thể giải mã gói này.

Thoạt nhìn, cách này khá hợp lý. Client tạo secret, server dùng private key để giải mã, hai bên nhận được cùng một bộ material, sau đó kết hợp với hai random value để derive session key cho các bước tiếp theo.
...
Trong RSA key exchange, `PreMasterSecret` mà client gửi năm đó được mã hóa bằng RSA public key của server. Nếu attacker đã bắt đầy đủ các random value ở dạng plaintext trong handshake, tức `Client Random` và `Server Random`, đồng thời lưu `PreMasterSecret` đã mã hóa, thì khi kết hợp với private key của server bị lộ sau này, attacker có thể giải mã `PreMasterSecret` của thời điểm đó, rồi tiếp tục derive session key đã được dùng cho connection đó.

Dữ liệu cũ có thể bị giải mã.

Cần lưu ý điều kiện ở đây: không phải “chỉ cần private key bị lộ là mọi traffic trong quá khứ chắc chắn giải mã được”. Attacker ít nhất phải lấy được đầy đủ handshake data và application data. Nếu chỉ có một chiều dữ liệu hoặc handshake log không đầy đủ thì ngay cả khi có private key, attacker chưa chắc khôi phục được session đó.

Nhưng xét về security design, rủi ro này đã đủ nghiêm trọng. Khi long-term private key trở thành master key để mở traffic trong quá khứ, ảnh hưởng của nó không còn chỉ bao phủ các connection trong tương lai mà còn lan sang những trao đổi đã xảy ra trước đó.

Điều bị chỉ trích ở đây không phải bản thân RSA algorithm là “không dùng được”. RSA vẫn có thể được dùng cho signature authentication và vẫn có thể xuất hiện trong certificate system. Vấn đề nằm ở việc “dùng private key dài hạn của server để giải mã key material trong các handshake trước đó”.
...
RSA key exchange còn có một rắc rối ở khía cạnh triển khai: `PreMasterSecret` không được mã hóa trực tiếp mà trước hết được đóng gói theo format như RSAES-PKCS1-v1_5 rồi mới mã hóa.

Chi tiết này từng dẫn đến các padding oracle attack, chẳng hạn như Bleichenbacher.
...
TLS 1.2 từng đưa ra yêu cầu phòng vệ cho tình huống này: ngay cả khi giải mã thất bại, server cũng không được để lộ lý do thất bại cụ thể mà phải tiếp tục dùng random value để hoàn tất flow, tránh cho attacker phán đoán ciphertext có gần đúng format hay không thông qua behavior khác biệt.
...
Nhưng điều đó không có nghĩa là “attacker trung gian chỉ cần tùy tiện xóa ECDHE khỏi ClientHello là có thể âm thầm downgrade connection xuống RSA”. `Finished` ở cuối handshake sẽ verify handshake transcript; việc sửa đơn giản `ClientHello` thường sẽ khiến verification thất bại và connection không thể được thiết lập.

Trong lịch sử thực sự đã xảy ra các attack liên quan đến downgrade, chẳng hạn FREAK và Logjam. Chúng lợi dụng việc một số client và server khi đó vẫn hỗ trợ các weak cipher suite dành cho export, sau đó kết hợp với vấn đề implementation và configuration để ép connection đi theo RSA_EXPORT hoặc DHE_EXPORT yếu hơn, chứ không phải “tùy ý xóa ECDHE là có thể âm thầm thành công”. TLS 1.3 thêm downgrade protection value vào `ServerHello.random` cũng nhắc chúng ta rằng protocol vẫn luôn phải bổ sung cơ chế bảo vệ trước các attack kiểu này.

Điều thực sự cần quan tâm là configuration của server: nếu không còn cần tương thích với client quá cũ thì nên tắt static RSA key exchange suite và chỉ giữ các suite hỗ trợ forward secrecy. Nếu không, trong environment vẫn có thể tồn tại client hoặc configuration lỗi khiến RSA handshake được sử dụng.
...
`DHE` trong ECDHE đến từ Diffie-Hellman Ephemeral, nghĩa là Diffie-Hellman tạm thời. `EC` ở phía trước là Elliptic Curve, cho biết nó dựa trên elliptic curve.

Đừng bị cái tên làm cho sợ. Tạm thời chưa xét elliptic curve, hãy xem trước DH muốn giải quyết vấn đề gì.

Mục tiêu của DH khá thú vị: hai bên giao tiếp không truyền shared secret trực tiếp nhưng vẫn có thể tự tính ra cùng một shared secret.

Có thể hiểu sơ lược như sau:

Client tạo một temporary private key, chỉ giữ cục bộ, rồi tính ra một temporary public key và gửi cho server. Server cũng tạo một temporary private key, chỉ giữ cục bộ, rồi tính ra một temporary public key và gửi cho client.

Hai bên chỉ trao đổi public key. Attacker trên network có thể nhìn thấy các public key này nhưng không nhìn thấy temporary private key của mỗi bên.
...
ECDHE chỉ đưa quy trình này vào elliptic curve system để thực hiện. Lý thuyết toán học của elliptic curve trừu tượng hơn, nhưng với cùng mức security strength, nó thường có thể dùng key ngắn hơn để đạt mức security tương đương; chi phí tính toán và truyền dữ liệu cũng thấp hơn DHE trên finite field truyền thống. Để hiểu TLS handshake, chỉ cần nhớ một câu:

**Session key material của ECDHE không do một bên tạo rồi gửi cho bên kia mà do hai bên negotiate thông qua các temporary key.**
...
- `RSA` biểu thị cơ chế authentication bằng signature.
...
Tiếp theo server gửi certificate. Với `ECDHE_RSA`, RSA public key trong certificate chủ yếu dùng để verify server signature, chứ không phải để client dùng nó mã hóa `PreMasterSecret`.

Sau đó, ECDHE và RSA handshake bắt đầu đi theo hai nhánh khác nhau.

Trong ECDHE handshake, server sẽ gửi `Server Key Exchange`. Message này chứa các elliptic curve parameter do server chọn và temporary ECDHE public key của server.

**Vấn đề là: làm sao client biết temporary ECDHE public key này không bị attacker ở giữa thay thế?**

**Câu trả lời là signature.**

Server dùng private key tương ứng với certificate để ký handshake parameter. Sau khi nhận được, client dùng public key trong certificate để verify signature. Nếu signature verification thành công, client có thể xác nhận: temporary ECDHE public key này thực sự đến từ server sở hữu private key của certificate, không phải bị ai đó thay thế trên đường truyền.
...
Đến bước này, hai bên đã có các thành phần cần thiết để tính shared secret.
...
Tuy nhiên, forward secrecy không phải lá bùa hộ mệnh.
...
Một loại rủi ro khác đến từ parameter validation. Chẳng hạn, nếu server không verify đúng xem elliptic curve point do client gửi có nằm trên curve hợp lệ hay không, nó có thể tạo cơ hội cho invalid-curve attack. Developer thông thường không nhất thiết phải trực tiếp viết phần code này, nhưng điều đó nhắc chúng ta rằng cryptographic protocol không kết thúc ở việc “chọn đúng algorithm”; implementation và configuration của TLS library cũng quan trọng không kém.
...
Lấy TLS 1.2 session ticket làm ví dụ, server dùng ticket encryption key để bảo vệ session state; lần sau client mang ticket quay lại, server giải mã ticket rồi khôi phục session. Nếu ticket encryption key này không được rotate trong thời gian dài, một khi bị lộ, attacker có thể giải mã các ticket đã thu thập trong quá khứ và tiếp tục khôi phục key material của các session liên quan được resume.

Khi đó, forward secrecy window không còn là “một connection” mà bị kéo dài thành “lifecycle của ticket encryption key”.

Vì vậy, configuration trong production không thể chỉ xem “đã bật ECDHE hay chưa”. Cũng cần tính đến cách tạo và rotate session ticket key, việc có dùng chung key giữa nhiều machine hay không và mức độ ảnh hưởng nếu key bị lộ.

### Performance không miễn phí

ECDHE mang lại forward secrecy nhưng cũng có chi phí.

Luồng chính của RSA key exchange là server dùng long-term RSA private key để mở `PreMasterSecret` do client gửi đến. Còn ECDHE_RSA cần hoàn thành ECDH negotiation tạm thời và ký temporary parameter của server.
...
Nếu thực sự muốn đánh giá chi phí, phương pháp đáng tin cậy nhất không phải là trích dẫn một con số cố định của người khác mà là benchmark trên machine mục tiêu. Ít nhất cần phân biệt ba việc:
...
Security và performance không phải hai lựa chọn loại trừ nhau, nhưng cũng không thể giả vờ rằng không có chi phí.
...
TLS 1.3 đưa key exchange parameter vào `key_share` trong `ClientHello` ngay từ đầu. Server có thể trả về `key_share` của mình ngay trong phản hồi đầu tiên, nên full handshake thường được rút xuống còn 1 RTT.

2 RTT thành 1 RTT tiết kiệm được bao nhiêu millisecond còn tùy thuộc network environment. Trong cùng một data center có thể chỉ là vài millisecond; với kết nối liên vùng, mobile network hoặc tình huống packet loss cao, việc giảm một RTT sẽ dễ nhận thấy hơn.

Tuy nhiên, TLS 1.3 cũng không phải lúc nào cũng ổn định ở 1 RTT. Nếu `key_share` client gửi không khớp với curve server hỗ trợ, server sẽ trả về `HelloRetryRequest`, yêu cầu client đổi sang một bộ parameter khác rồi thử lại. Khi đó, handshake có thể lại gần 2 RTT.

Vì vậy, trong production, client và server nên hỗ trợ các key negotiation group phổ biến tương thích với nhau, chẳng hạn `X25519` và `secp256r1`. Nếu không, lợi thế 1 RTT của TLS 1.3 có thể bị giảm.
...
| Vai trò của server private key      | Giải mã `PreMasterSecret` do client gửi đến                                                  | Ký ECDHE temporary parameter để chứng minh parameter đến từ server thực sự                                                |
...
| Ảnh hưởng sau khi private key bị lộ | Traffic trong quá khứ có thể bị giải mã nếu handshake data được thu thập đầy đủ              | Thông thường không thể giải mã traffic trong quá khứ chỉ bằng certificate private key                                     |
| Vấn đề điển hình                    | Long-term private key có giá trị quá lớn, có gánh nặng lịch sử từ PKCS#1 v1.5 padding oracle | Handshake có thêm chi phí tính toán, parameter validation và temporary key management phụ thuộc chất lượng implementation |
...
### Cách hiểu sai thường gặp: ECDHE_RSA không có nghĩa cả hai algorithm đều dùng để encryption

Cái tên `ECDHE_RSA` rất dễ khiến người ta nghĩ rằng cả hai algorithm đều thực hiện việc mã hóa. Lấy cipher suite này làm ví dụ:
...
Khác biệt cốt lõi giữa RSA và ECDHE nằm ở việc session key material được “truyền qua” hay được các bên negotiate.

Trong static RSA handshake của TLS 1.2, client tạo `PreMasterSecret`, mã hóa bằng RSA public key trong server certificate rồi gửi cho server; server tiếp tục dùng RSA private key để giải mã. Vấn đề là nếu attacker lưu handshake traffic của thời điểm đó, sau này private key của server lại bị lộ thì attacker có thể quay lại giải mã session key của phiên cũ, nên phương thức này không có forward secrecy.

ECDHE không truyền shared secret trực tiếp. Client và server mỗi bên tạo một temporary key pair, trao đổi temporary public key rồi tự tính cùng một shared secret cục bộ. Certificate private key của server chủ yếu dùng cho signature authentication, chứng minh temporary parameter không bị attacker ở giữa thay thế, chứ không dùng để giải mã session key.

Khi phỏng vấn, bạn có thể trả lời như sau: static RSA handshake của TLS 1.2 do client tạo `PreMasterSecret`, sau đó dùng RSA public key của server để mã hóa và gửi đi; nếu attacker lưu trọn handshake traffic rồi sau đó lấy được private key của server, session cũ có thể bị giải mã, vì vậy nó không có forward secrecy. ECDHE không truyền shared secret trực tiếp; sau khi hai bên trao đổi temporary public key, mỗi bên tự tính cùng một secret cục bộ; certificate private key chủ yếu dùng cho signature authentication chứ không trực tiếp giải mã session key. TLS 1.3 đã loại bỏ static RSA key exchange và dùng (EC)DHE, PSK hoặc cách kết hợp cả hai để thiết lập key.
