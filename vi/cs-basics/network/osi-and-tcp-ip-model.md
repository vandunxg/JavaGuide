---
title: Giải thích chi tiết mô hình OSI 7 tầng và TCP/IP 4 tầng
description: Giải thích chi tiết mô hình phân tầng và việc phân chia trách nhiệm của OSI và TCP/IP, đồng thời so sánh khác biệt và lựa chọn engineering qua lịch sử và thực tiễn.
...
Phân tầng mạng là bản đồ đầu tiên khi học computer network. Không có bản đồ này, các khái niệm HTTP, TCP, IP, Ethernet, DNS rất dễ bị xếp lẫn vào nhau, không phân biệt được khái niệm nào phụ thuộc vào khái niệm nào và khái niệm nào chịu trách nhiệm cho việc gì.

Hai mô hình phân tầng thường gặp là mô hình OSI 7 tầng và mô hình TCP/IP 4 tầng. Mô hình trước phù hợp hơn để xây dựng framework khái niệm, còn mô hình sau gần với việc triển khai thực tế trên Internet hơn.
...
**Mô hình OSI 7 tầng** là một mô hình phân tầng mạng do Tổ chức Tiêu chuẩn hóa Quốc tế đề xuất. Cấu trúc tổng thể và chức năng do mỗi tầng cung cấp được thể hiện trong hình dưới đây:
...
Mỗi tầng chỉ tập trung làm một việc, đồng thời mỗi tầng đều cần sử dụng chức năng do tầng bên dưới cung cấp. Chẳng hạn, transport layer cần sử dụng chức năng routing và addressing do network layer cung cấp, nhờ đó transport layer mới biết cần truyền dữ liệu đến đâu.

**Kiến trúc 7 tầng của OSI có khái niệm rõ ràng và lý thuyết cũng rất hoàn chỉnh, nhưng khá phức tạp, thiếu tính thực tiễn; hơn nữa, một số chức năng còn lặp lại ở nhiều tầng.**

Hình trên có thể khá trừu tượng, vì vậy hãy xem thêm một hình ảnh sinh động hơn. Tôi nhìn thấy hình ảnh này trên một website nước ngoài và thấy nó rất hay!

![Mô hình OSI 7 tầng 2](https://oss.javaguide.cn/github/javaguide/osi七层模型2.png)

**Đã có mô hình OSI 7 tầng mạnh như vậy, vì sao nó lại không vượt qua được mô hình TCP/IP 4 tầng?**

Đúng là vào thời điểm đó, mô hình OSI 7 tầng nhận được sự ủng hộ của một số công ty lớn, thậm chí cả chính phủ một số quốc gia. Vậy vì sao nó lại thất bại trong bối cảnh đó? Theo tôi, chủ yếu có những nguyên nhân sau:
...
Mặc dù mô hình OSI 7 tầng đã thất bại, nó vẫn cung cấp nhiều nền tảng lý thuyết hữu ích. Để hiểu rõ hơn về phân tầng mạng, việc học mô hình OSI 7 tầng vẫn rất cần thiết.

Cuối cùng, hãy xem thêm một hình tổng hợp rất hay về mô hình OSI 7 tầng!
...
**Mô hình TCP/IP 4 tầng** là một mô hình hiện được sử dụng rộng rãi. Có thể xem mô hình TCP/IP là phiên bản rút gọn của mô hình OSI 7 tầng, gồm 4 tầng sau:
...
Cần lưu ý rằng không thể ghép nối hoàn toàn chính xác mô hình TCP/IP 4 tầng với mô hình OSI 7 tầng. Tuy nhiên, có thể đơn giản hóa việc đối chiếu hai mô hình như hình dưới đây:
...
**Application layer nằm trên transport layer, chủ yếu cung cấp dịch vụ trao đổi thông tin giữa các application trên hai thiết bị đầu cuối. Tầng này định nghĩa format trao đổi thông tin, sau đó message sẽ được chuyển cho transport layer để truyền đi.** Đơn vị dữ liệu được trao đổi ở application layer được gọi là message.
...
Application layer protocol định nghĩa các quy tắc network communication. Những network application khác nhau cần các application layer protocol khác nhau. Trên Internet có rất nhiều application layer protocol, chẳng hạn HTTP hỗ trợ Web application, SMTP hỗ trợ email, v.v.
...
- **HTTP (Hypertext Transfer Protocol, protocol truyền hypertext)**: Dựa trên TCP, là protocol dùng để truyền hypertext và multimedia content, chủ yếu được thiết kế cho communication giữa Web browser và Web server. Khi dùng browser để duyệt web, trang web được tải thông qua HTTP request.
...
- **POP3/IMAP (protocol nhận email)**: Dựa trên TCP, cả hai đều chịu trách nhiệm nhận email. IMAP là protocol mới hơn POP3 và mạnh hơn về chức năng cũng như performance. IMAP hỗ trợ các chức năng nâng cao như tìm kiếm, đánh dấu, phân loại, lưu trữ email; đồng thời có thể đồng bộ trạng thái email giữa nhiều thiết bị. Hầu hết email client và server hiện đại đều hỗ trợ IMAP.
- **FTP (File Transfer Protocol, protocol truyền file)**: Dựa trên TCP, là protocol dùng để truyền file giữa các computer, có thể che giấu sự khác biệt giữa operating system và cách lưu trữ file. Lưu ý ⚠️: FTP là một protocol không an toàn vì không mã hóa dữ liệu trong quá trình truyền. Khi truyền dữ liệu nhạy cảm, nên dùng protocol an toàn hơn như SFTP.
- **Telnet (protocol remote login)**: Dựa trên TCP, được dùng để login vào server khác thông qua một terminal. Một trong những nhược điểm lớn nhất của Telnet là toàn bộ dữ liệu, bao gồm username và password, đều được gửi dưới dạng plaintext, tiềm ẩn rủi ro bảo mật. Đây là lý do chính khiến Telnet ngày nay hiếm khi được sử dụng, thay vào đó là network transfer protocol rất an toàn có tên SSH.
- **SSH (Secure Shell Protocol, protocol network transfer an toàn)**: Dựa trên TCP, thực hiện các tác vụ như truy cập an toàn và truyền file thông qua cơ chế encryption và authentication.
...
Để xem phần giới thiệu chi tiết về các protocol này, hãy đọc bài [Tổng hợp các protocol thường gặp ở application layer (Application layer)](./application-layer-protocol.md).

### Transport layer (Transport layer)

**Nhiệm vụ chính của transport layer là cung cấp dịch vụ truyền dữ liệu tổng quát cho communication giữa các process của hai thiết bị đầu cuối.** Application process sử dụng dịch vụ này để truyền application layer message. “Tổng quát” nghĩa là dịch vụ này không dành riêng cho một network application cụ thể, mà nhiều application có thể dùng chung một transport layer service.
...
- **UDP (User Datagram Protocol, protocol datagram người dùng)**: Cung cấp dịch vụ truyền dữ liệu **connectionless**, **best effort** (không đảm bảo độ tin cậy của việc truyền dữ liệu), đơn giản và hiệu quả.

### Network layer (Network layer)

**Network layer chịu trách nhiệm cung cấp communication service cho các host khác nhau trên packet-switched network.** Khi gửi dữ liệu, network layer đóng gói segment hoặc datagram do transport layer tạo ra thành packet để truyền đi. Trong kiến trúc TCP/IP, vì network layer sử dụng IP protocol nên packet còn được gọi là IP datagram, gọi tắt là datagram.

⚠️ Lưu ý: **Không được nhầm “user datagram UDP” của transport layer với “IP datagram” của network layer.**

**Một nhiệm vụ khác của network layer là chọn route phù hợp, để packet do transport layer của source host truyền xuống có thể đi qua router trong network layer và đến destination host.**

Ở đây cần nhấn mạnh rằng chữ “network” trong “network layer” không còn chỉ một network cụ thể như cách chúng ta thường nói, mà là tên của tầng thứ ba trong mô hình kiến trúc computer network.

Internet được kết nối từ rất nhiều network dị thể (heterogeneous) thông qua router. Network layer protocol được Internet sử dụng là Internet Protocol không connection-oriented và nhiều routing protocol. Vì vậy network layer của Internet còn được gọi là **internet layer** hoặc **IP layer**.
...
- ICMP protocol (control message protocol, dùng để gửi control message)
...
Ở cuối bài viết này, tôi muốn trao đổi về câu hỏi: “Vì sao network cần phân tầng?”.

Khi nói đến phân tầng, trước tiên hãy lấy việc sử dụng framework để phát triển một backend program làm ví dụ. Chúng ta thường chia system thành 3 tầng theo nguyên tắc mỗi tầng làm những việc khác nhau (system phức tạp có thể có nhiều tầng hơn):

1. Repository (database operation)
2. Service (business operation)
3. Controller (trao đổi dữ liệu giữa frontend và backend)

**System phức tạp cần phân tầng vì mỗi tầng cần tập trung vào một nhóm công việc. Lý do network phân tầng cũng giống như vậy: mỗi tầng chỉ tập trung làm một nhóm công việc.**

Vậy hãy quay lại câu hỏi: “Vì sao network cần phân tầng?”. Theo tôi, chủ yếu có 3 nguyên nhân:
