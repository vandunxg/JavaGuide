...
description: "Tổng hợp các khái niệm cốt lõi và trường hợp sử dụng điển hình của các giao thức tầng ứng dụng thường gặp, tập trung so sánh mô hình giao tiếp và ranh giới khả năng của HTTP với WebSocket."
...
head:
  - - meta
    - name: keywords
      content: Application layer protocol,HTTP,WebSocket,DNS,SMTP,FTP,characteristics,scenarios
---
...
**Giao thức truyền siêu văn bản (HTTP, HyperText Transfer Protocol)** là một giao thức tầng ứng dụng dùng để truyền siêu văn bản và nội dung đa phương tiện. Trường hợp sử dụng phổ biến nhất là giao tiếp giữa Web browser và Web server.
...
Khi truy cập một trang Web trong browser, browser gửi HTTP request đến server, server xử lý rồi trả về HTTP response. HTML, CSS, JavaScript, hình ảnh, video và nhiều tài nguyên khác trên trang thường được tải qua HTTP.
...
Cần lưu ý rằng HTTP là giao thức tầng ứng dụng, bản thân nó không trực tiếp phụ trách truyền tải đáng tin cậy. Phần phụ thuộc tầng dưới của các phiên bản HTTP khác nhau cũng không hoàn toàn giống nhau:
...
- **HTTP/3**: dựa trên QUIC, còn QUIC dựa trên UDP, chủ yếu nhằm giảm chi phí thiết lập connection và giảm ảnh hưởng của head-of-line blocking do TCP gây ra.

Trong HTTP/1.1, Keep-Alive được bật mặc định, tức là connection dài hạn. Nhờ đó, cùng một connection TCP có thể được nhiều HTTP request sử dụng lại, tránh phải thiết lập lại connection TCP cho mỗi request, từ đó giảm chi phí của TCP three-way handshake.
...
HTTP/2 đưa multiplexing vào một connection TCP, cho phép truyền song song nhiều request và response, giảm head-of-line blocking ở tầng HTTP. Tuy nhiên, vì tầng bên dưới vẫn là TCP, khi một packet TCP bị mất, dữ liệu trên toàn bộ connection vẫn bị ảnh hưởng.
...
Ngoài ra, HTTP là một **stateless protocol**. Server không tự nhiên ghi nhớ “request trước đó do ai gửi và đang ở trạng thái nào”. Vì vậy, trong phát triển Web thực tế, thường cần dựa vào các cơ chế như Cookie, Session, Token (bao gồm JWT) để duy trì trạng thái đăng nhập và trạng thái session của người dùng.

## WebSocket: Giao thức giao tiếp full-duplex

**WebSocket** là một giao thức giao tiếp full-duplex dựa trên connection TCP. Client và server có thể đồng thời gửi và nhận dữ liệu trên cùng một connection.
...
Giao thức WebSocket ra đời năm 2008 và trở thành tiêu chuẩn quốc tế năm 2011. Các browser phổ biến hiện nay hầu như đều hỗ trợ nó. WebSocket không chỉ dùng trong browser; nhiều ngôn ngữ lập trình, framework và server cũng cung cấp hỗ trợ tương ứng.
...
- Đối kháng game thời gian thực
- Chỉnh sửa cộng tác nhiều người dùng
- CSKH trực tuyến / chat mạng xã hội
...
Ngoài ra, connection WebSocket thường được dùng kết hợp với **cơ chế heartbeat**. Ví dụ, định kỳ gửi Ping/Pong frame hoặc gửi heartbeat packet ở tầng nghiệp vụ để kiểm tra connection còn khả dụng hay không, tránh connection bị treo giả.
...
Nói cách khác, khi email được gửi từ mail server của bạn đến mail server của đối phương, quá trình này thường vẫn dùng SMTP. Còn khi người dùng dùng client để xem email trong hộp thư, thường sử dụng POP3 hoặc IMAP.
...
Ví dụ email của tôi là `<dabai@cszhinan.com>` và tôi muốn gửi email đến `<xiaoma@qq.com>`. Toàn bộ quá trình có thể hiểu đơn giản như sau:

1. Tôi soạn email bằng email client hoặc webmail.
2. Email client dùng giao thức SMTP để submit email cho mail server tương ứng với `cszhinan.com`.
3. Mail server bên gửi tra cứu địa chỉ mail server tương ứng theo domain người nhận `qq.com`.
...
Để ngăn spam, credential stuffing và rò rỉ quyền riêng tư, nhiều nhà cung cấp dịch vụ email sẽ chặn việc thăm dò sự tồn tại của email hoặc luôn trả về kết quả không rõ ràng. Vì vậy, thăm dò SMTP chỉ có thể dùng để tham khảo, không thể xác định 100% một email chắc chắn tồn tại hay không tồn tại.
...
**POP3 và IMAP đều là các giao thức dùng để nhận email**, đồng thời đều là giao thức tầng ứng dụng dựa trên TCP.
...
POP3 có thiết kế khá đơn giản. Cách dùng phổ biến là tải email từ server về local. Nó phù hợp với việc nhận thư trên một thiết bị, nhưng trải nghiệm đồng bộ nhiều thiết bị khá kém.
...
Thiết kế tách riêng lệnh và dữ liệu để truyền giúp command điều khiển và dữ liệu file không ảnh hưởng lẫn nhau.
...
- **Active mode**: client thông báo cho server qua control connection về cổng mà mình đang listen. Sau đó server chủ động kết nối đến cổng này của client để thiết lập data connection. Vì server phải chủ động kết nối đến client, nếu client ở sau NAT hoặc firewall thì connection rất dễ thất bại.
- **Passive mode**: client yêu cầu server mở một data port, sau đó client chủ động kết nối đến data port của server. Vì hướng kết nối vẫn là từ client đến server, cách này dễ đi qua NAT và firewall hơn, nên passive mode được dùng phổ biến hơn trong môi trường production thực tế.
...
Username, password, nội dung lệnh và kết quả trả về đều không được mã hóa. Nếu attacker có thể nghe traffic mạng, họ có thể nhìn thấy trực tiếp thông tin nhạy cảm.
...
Vì vậy, Telnet hiện nay hiếm khi được dùng cho quản trị từ xa thực sự. Trong môi trường production, SSH thường được dùng để thay thế Telnet.

## SSH: Giao thức truyền tải mạng an toàn

**SSH (Secure Shell)** là một giao thức mạng an toàn dựa trên TCP, cổng mặc định là 22. Nó cung cấp bảo vệ an toàn cho đăng nhập từ xa, thực thi lệnh và truyền file thông qua cơ chế mã hóa và authentication.

![Tổng quan về SSH: Giao thức truyền tải mạng an toàn](https://oss.javaguide.cn/github/javaguide/cs-basics/network/ssh-overview.png)
...
![SSH: Giao thức truyền tải mạng an toàn](https://oss.javaguide.cn/github/javaguide/cs-basics/network/ssh-client-server.png)
...
Trong môi trường production thực tế, nên ưu tiên public key authentication và tắt đăng nhập bằng password yếu.

## RTP: Giao thức truyền tải thời gian thực

**RTP (Real-time Transport Protocol, giao thức truyền tải thời gian thực)** là một giao thức dùng để truyền dữ liệu thời gian thực như audio và video. Nó thường chạy trên UDP. Trong mô hình phân tầng TCP/IP, tầng phía trên UDP là tầng ứng dụng, nên theo quy tắc phân tầng, RTP được xếp vào tầng ứng dụng. Tuy nhiên, trách nhiệm của nó (sequence number, timestamp, đồng bộ, phản hồi chất lượng) gần với chức năng của tầng truyền tải hơn. RFC 3550 cũng nói rằng nó “thường được tích hợp vào xử lý ứng dụng thay vì được triển khai như một tầng độc lập”.
...
RTP chủ yếu được dùng trong các trường hợp thời gian thực như thoại, video conference và livestream. Bản thân nó không đảm bảo truyền tải đáng tin cậy, cũng không đảm bảo dữ liệu đến đúng hạn, mà dùng sequence number, timestamp và các thông tin khác để giúp bên nhận sắp xếp, đồng bộ và điều khiển phát. Mặc dù cũng có cách đóng gói RTP over TCP (như RFC 4571), cách này chủ yếu dùng trong các trường hợp đặc biệt như đi qua firewall hoặc tương thích với protocol stack cụ thể. Trong các trường hợp audio/video thời gian thực thực tế, RTP vẫn chủ yếu dùng UDP.
...
Cần lưu ý rằng bản thân RTP không phụ trách resource reservation và không đảm bảo chất lượng truyền tải thời gian thực. Nó cung cấp năng lực nền tảng cho truyền media thời gian thực; việc kiểm soát chất lượng cụ thể cần dựa vào sự phối hợp của các cơ chế tầng trên.
...
Sau này, EDNS0 mở rộng giới hạn kích thước packet của DNS over UDP, giúp DNS có thể chứa response lớn hơn, chẳng hạn dữ liệu liên quan đến DNSSEC. Tuy nhiên, nếu response vượt quá kích thước UDP đã thương lượng hoặc xảy ra zone transfer (các DNS server đồng bộ dữ liệu của toàn bộ zone, việc phân giải domain thông thường hầu như không kích hoạt trường hợp này), TCP vẫn được sử dụng.
...
Bài viết này chỉ tổng hợp nhanh các giao thức tầng ứng dụng thường gặp, không đi sâu vào packet của giao thức và chi tiết triển khai cụ thể.
...
- SMTP phụ trách gửi email và chuyển tiếp giữa các server, POP3/IMAP phụ trách việc người dùng nhận email.
- Các cổng thường dùng của SMTP gồm 25, 587, 465, lần lượt tương ứng với các trường hợp như chuyển tiếp giữa server, client submit và submit với TLS ngầm định.
- FTP có active mode và passive mode; passive mode phổ biến hơn trong môi trường production thực tế.
...
- DNS thường dựa trên UDP, nhưng cũng dùng TCP khi response quá lớn, xảy ra truncated hoặc thực hiện zone transfer.
