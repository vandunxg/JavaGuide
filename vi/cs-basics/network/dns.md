...
Sau khi nhập domain name vào thanh địa chỉ của trình duyệt, trước khi thực sự gửi HTTP request, thông thường cần trải qua bước phân giải DNS.

DNS giải quyết **bài toán ánh xạ giữa domain name và địa chỉ IP**. Thoạt nhìn, nó chỉ là “dịch domain name thành IP”, nhưng phía sau liên quan đến cả một hệ thống cơ chế gồm cache cục bộ, truy vấn đệ quy, truy vấn lặp, authoritative server, root server, chuyển đổi giữa UDP/TCP và nhiều thành phần khác.
...
Trong thực tế, có một trường hợp trình duyệt có thể biết ánh xạ giữa domain name và địa chỉ IP mà không cần dùng đến DNS. Trình duyệt duy trì một danh sách `hosts` cục bộ. Thông thường, trước tiên trình duyệt sẽ kiểm tra domain name cần truy cập có trong danh sách `hosts` hay không. Nếu có, nó trực tiếp lấy bản ghi địa chỉ IP tương ứng là xong. Nếu danh sách `hosts` cục bộ không có bản ghi tương ứng giữa domain name và IP, DNS sẽ xuất hiện.

Hiện nay, DNS sử dụng cấu trúc database phân tán, phân cấp. **DNS là protocol tầng ứng dụng, thường dựa trên protocol UDP và sử dụng port 53**. Khi dữ liệu response vượt quá giới hạn độ dài của UDP packet (512 byte, EDNS0 có thể mở rộng lên lớn hơn), hoặc khi thực hiện truyền zone (Zone Transfer), nó sẽ chuyển sang protocol TCP để bảo đảm tính toàn vẹn dữ liệu.
...
DNS có thể được mô tả theo hai chiều. Về phân cấp authoritative, có root server, top-level domain server và authoritative server của các zone cụ thể; về phía truy vấn, có các vai trò như stub resolver, recursive resolver và forwarder. Cùng một phần mềm hoặc cùng một server cũng có thể đảm nhận nhiều vai trò, vì vậy các nhóm này không phải là những “loại server” loại trừ lẫn nhau và đầy đủ tuyệt đối.
...
- Authoritative DNS server. Authoritative server lưu dữ liệu của một hoặc nhiều DNS zone và đưa ra câu trả lời authoritative cho các truy vấn trong những zone đó.
...
Về mặt logic, hệ thống root server có 13 root server identifier được đặt tên, từ `a.root-servers.net` đến `m.root-servers.net`, do 12 tổ chức vận hành độc lập phụ trách. Con số này liên quan đến giới hạn kích thước packet khi DNS sử dụng truyền tải UDP trong giai đoạn đầu, nhưng không thể hiểu rằng toàn cầu chỉ có 13 physical server.

Đằng sau mỗi root server identifier có thể triển khai nhiều physical instance thông qua **IP Anycast**. BGP sẽ dựa trên network route hiện tại để hướng query đến instance phù hợp trên đường đi, không nhất thiết là instance gần nhất về mặt địa lý. Số lượng và địa điểm của các instance sẽ liên tục thay đổi; cần lấy dữ liệu thời gian thực từ **[Root-Servers.org](https://root-servers.org/)** làm chuẩn.
...
Lấy hình dưới đây làm ví dụ để giới thiệu quy trình query và resolution của DNS. Quy trình query và resolution của DNS được chia thành hai mode:

- **Lặp**
- **Đệ quy**

Hình dưới đây minh họa cách thường được sử dụng trong thực tế: query từ host request đến local DNS server là đệ quy, các query còn lại là lặp.
...
1. Trước tiên, host `cis.poly.edu` gửi một DNS request đến local DNS server `dns.poly.edu`. Query packet này chứa domain name cần được chuyển đổi là `gaia.cs.umass.edu`.
2. Local DNS server `dns.poly.edu` kiểm tra local cache, phát hiện không có record, đồng thời không biết địa chỉ IP của `gaia.cs.umass.edu` nằm ở đâu, nên buộc phải gửi request đến root server.
...
5. TLD DNS server của `edu` vẫn không biết địa chỉ IP của domain được query, nhưng nhận thấy domain này có prefix `umass.edu`, nên trả lời cho local DNS rằng authoritative server của `umass.edu` có thể đã ghi địa chỉ IP của domain mục tiêu.
6. Lần này, local DNS gửi request đến authoritative DNS server `dns.cs.umass.edu`.
7. Cuối cùng, vì `gaia.cs.umass.edu` đã được đăng ký trên authoritative DNS server, ở đây có record địa chỉ IP của nó. Authoritative DNS trả thành công địa chỉ IP về local DNS.
8. Cuối cùng, local DNS lấy được địa chỉ IP của domain mục tiêu và trả nó về host request.
...
- Question section. Chứa host name đang được query và loại question đang được hỏi.
...
Khi response query, DNS server cần query database của chính nó. Các entry trong database được gọi là **Resource Record (RR)**. RR cung cấp ánh xạ từ host name đến địa chỉ IP. RR là một tuple gồm 4 field `Name`, `Value`, `Type` và `TTL`.
...
- Nếu `Type=CNAME` (Canonical Name Record, record tên chuẩn), `Value` là canonical host name của host có alias là `Name`. Giá trị `Value` mới là canonical host name. Record `CNAME` ánh xạ một host name sang một host name khác. Record `CNAME` được dùng để tạo alias cho record `A` hiện có. Phần dưới có ví dụ.
