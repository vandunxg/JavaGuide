---
title: "Giải thích chi tiết về lock và cơ chế đồng bộ trong Operating System: mutex, semaphore, condition variable, spinlock và futex"
description: Tổng hợp các câu hỏi phỏng vấn thường gặp về lock và cơ chế đồng bộ trong Operating System, làm rõ critical section, mutex, spinlock, semaphore, condition variable, futex, atomic instruction, memory order, priority inversion, cùng sự khác biệt giữa user-space lock và Linux kernel lock.
...
head:
  - - meta
    - name: keywords
      content: lock trong Operating System,cơ chế đồng bộ,critical section,mutex,spinlock,semaphore,condition variable,futex,atomic instruction,memory barrier,priority inversion,Linux kernel lock,câu hỏi phỏng vấn Operating System,Concurrent Programming
---

Hai thread đồng thời cộng 1 vào cùng một counter, nhìn thì chỉ là một việc rất nhỏ nhưng kết quả cuối cùng lại có thể bị thiếu một lần cộng.
...
Để tránh các vấn đề concurrent như vậy, Operating System cung cấp lock và một loạt cơ chế đồng bộ. Những vấn đề chúng giải quyết không chỉ là một đoạn code có thể được thực thi đồng thời hay không, mà còn gồm thread có nên block hay không, số lượng resource được kiểm soát thế nào, và phải chờ ra sao khi điều kiện chưa thỏa mãn. Trong kernel còn phải tiếp tục cân nhắc interrupt, preemption, nhiều CPU, tính realtime và scheduling latency.

Bài viết này chỉ nói về cơ chế đồng bộ dưới góc nhìn Operating System. `synchronized`, `ReentrantLock`, AQS, CAS và lock optimization trong Java đã được trình bày trong [Giải thích chi tiết về Java lock](../../java/concurrent/java-lock.md), nên ở đây sẽ không lặp lại nội dung đó. Trọng tâm của bài là xem mutex, semaphore, condition variable, spinlock và futex mỗi khái niệm giải quyết vấn đề gì. Sau khi hiểu các synchronization primitive này, đọc tiếp [Giải thích chi tiết về deadlock](./dead-lock.md) sẽ dễ hiểu hơn vì sao “quan hệ chờ lại tạo thành vòng”.

Trước tiên, hãy xem sơ bộ các cơ chế đồng bộ này lần lượt giải quyết vấn đề gì:

| Cơ chế             | Giải quyết chủ yếu                               | Cách chờ                                                 | Tình huống thường gặp                                                 |
| ------------------ | ------------------------------------------------ | -------------------------------------------------------- | --------------------------------------------------------------------- |
| mutex              | Mutual exclusion của critical section            | Về mặt ngữ nghĩa là chờ lock khả dụng, có thể spin/block | Bảo vệ shared structure                                               |
| spinlock           | Mutual exclusion của critical section rất ngắn   | Busy-wait                                                | Path trong kernel không thể sleep                                     |
| semaphore          | Đếm resource, kiểm soát số lượng concurrent      | Chờ khi counter bằng 0                                   | Slot buffer, số connection, số task concurrent                        |
| condition variable | Chờ một shared state chuyển thành đúng           | Atomic release mutex rồi chờ                             | Queue không rỗng, task hoàn tất, buffer không đầy                     |
...
**Critical section** là đoạn code truy cập shared mutable state và không thể để nhiều execution flow tùy ý đan xen thực thi. Nó có thể là một đoạn cập nhật counter trong user program, cũng có thể là code sửa scheduling queue, file descriptor table, page table hoặc device state trong kernel.

![Sơ đồ minh họa protocol bảo vệ critical section: nhiều thread truy cập shared state qua một entry point lock thống nhất; bypass lock hoặc thay lock object sẽ phá vỡ quan hệ mutual exclusion](https://oss.javaguide.cn/github/javaguide/cs-basics/operating-system/os-lock-critical-section.png)
...
**Thứ nhất là correctness.** Không được để nhiều execution flow tùy ý đan xen sửa shared state tại cùng một thời điểm; trong môi trường nhiều CPU, còn cần synchronization semantic cần thiết để state được thread ghi trước khi release lock có thể được thread tiếp theo lấy cùng lock nhìn thấy như dự kiến.

**Thứ hai là progress.** Bản thân synchronization mechanism không được nhốt tất cả waiter, khiến hệ thống không còn ai có thể tiếp tục tiến lên.

**Thứ ba là fairness.** Khi nhiều thread cùng chờ một lock, cần cố gắng tránh để một thread không lấy được lock trong thời gian dài. Hệ thống thực tế không nhất thiết phải strict FIFO, nhưng phải nghiêm túc xử lý vấn đề starvation.

**Thứ tư là performance.** Khi không có contention, path lock và unlock phải đủ nhẹ; khi contention rất lớn, các waiting thread không được lãng phí quá nhiều CPU vào những vòng lặp vô ích.

OSTEP cũng quan tâm các vấn đề này khi nói về lock: có thật sự đạt được mutual exclusion hay không, waiting thread có bị starvation không, khi không có contention phải trả bao nhiêu cost, và biểu hiện trên single CPU và multi-CPU khác nhau thế nào. Chỉ hỏi “lock nào nhanh nhất” không có nhiều ý nghĩa; cùng một lock nhưng đặt trên machine khác nhau, critical section có độ dài khác nhau và contention intensity khác nhau thì câu trả lời thường thay đổi.
...
Dùng POSIX thread viết ra đại khái như sau:
...
Trong đoạn code này, `pthread_mutex_lock()` không chỉ thực hiện một lần đánh dấu. Xét theo POSIX semantic, nếu lock đang được thread khác giữ, calling thread sẽ chờ lock khả dụng; chỉ sau khi return thành công, thread đó mới sở hữu lock này.

Implementation cụ thể có thể linh hoạt hơn. Nhiều pthread mutex hoặc lock trong language runtime có thể tạm thời spin trong user space trước; nếu contention chưa được giải quyết thì mới đi vào blocking path như futex. Với người sử dụng, trọng tâm là semantic: trước khi lấy lock thì không được vào critical section, sau khi lấy lock mới có quyền truy cập state được bảo vệ.
...
Còn một chi tiết: mutex thường có owner semantic. Nói đơn giản, ai lấy lock thì người đó release. Tài liệu Linux kernel khi giới thiệu lock types đã nhấn mạnh owner semantics; phần lớn lock yêu cầu context lấy lock phải chịu trách nhiệm release. Semaphore hơi khác: nó giống một counter hơn, và sự khác biệt này sẽ thấy rõ ở phần sau.

## Spinlock: đừng sleep, chờ tại chỗ một lát

Khi không lấy được mutex, thread có thể sleep và chờ kernel wakeup sau. **Spinlock** thì ngược lại: trước tiên đừng sleep, hãy tiếp tục loop trên CPU để kiểm tra lock đã được release hay chưa.
...
Nếu một lock chỉ bảo vệ vài dòng code và thread giữ lock sẽ rời critical section ngay, để waiting thread sleep ngược lại còn không đáng. Sleep và wakeup đều phải đi qua scheduler, trong thời gian đó còn có thể xảy ra context switch; trên machine nhiều CPU, thread giữ lock có thể đang chạy trên CPU khác và sẽ release lock sau vài instruction. Lúc này, waiting thread quay vài vòng tại chỗ có thể ít cost hơn.

Nhưng spin có hai giới hạn cứng.

**Thứ nhất, critical section phải ngắn.** Nếu thread giữ lock phải truy cập disk, chờ network hoặc allocate memory có thể sleep, bên chờ sẽ đốt CPU time vào việc quay không.

**Thứ hai, phải cẩn thận trong single CPU hoặc preemptible scenario.** Nếu thread giữ lock bị preempt trong khi waiting thread spin trên cùng CPU, dù waiting thread có quay chăm chỉ đến đâu cũng không chờ được hành động release.
...
Tài liệu Linux kernel phân loại lock khá rộng thành sleeping locks, CPU local locks và spinning locks. `mutex`, `semaphore`, `rw_semaphore` thuộc nhóm lock có thể sleep; `raw_spinlock_t` đều là strict spinlock trong kernel thường và PREEMPT_RT kernel. Semantic của `spinlock_t` thay đổi theo PREEMPT_RT: dưới kernel không phải PREEMPT_RT, nó map tới `raw_spinlock_t`; dưới PREEMPT_RT, nó được implement dựa trên `rt_mutex`, không còn disable preemption ngầm, và hậu tố `_irq` / `_irqsave` cũng không còn trực tiếp thay đổi trạng thái disable hard interrupt.

User-space business code thường không nên tự viết spinlock. Library và runtime có thể thực hiện adaptive spin trên path rất ngắn, nhưng trong application code, tự viết vòng `while` để chờ lock phần lớn chỉ khiến CPU nóng lên.

## Semaphore: không chỉ là lock có 0 và 1

**Semaphore** có thể được xem như một counter không giảm xuống số âm. `sem_wait()` thử giảm counter đi 1; nếu giá trị hiện tại lớn hơn 0 thì giảm xong sẽ tiếp tục; nếu giá trị hiện tại bằng 0 thì calling thread block. `sem_post()` tăng counter lên 1 và có thể wakeup waiter.

Khi đặt initial value bằng 1, semaphore có thể được dùng như mutex:
...
Tuy nhiên, cách dùng phổ biến thực sự của semaphore là “đếm resource”. Ví dụ buffer có N empty slot, connection pool cho phép tối đa N connection, hoặc một loại task chỉ được chạy đồng thời tối đa N task. Lúc này, initial value của semaphore chính là số lượng resource.

Binary semaphore có thể mô phỏng mutual exclusion, nhưng không đồng nghĩa với mutex. Mutex nhấn mạnh owner và ownership của critical section, còn semaphore nhấn mạnh counter và số lượng permit. Một bounded buffer thường tách hai loại vấn đề này: semaphore quản lý số lượng slot, mutex quản lý internal structure của buffer.

![Sơ đồ minh họa semaphore quản lý số lượng resource của bounded buffer: empty_slots ghi lại số vị trí trống, filled_slots ghi lại số lượng phần tử có thể consume, buffer_mutex bảo vệ structure của buffer](https://oss.javaguide.cn/github/javaguide/cs-basics/operating-system/os-lock-semaphore-buffer.png)

Đoạn code dưới đây lược bỏ implementation cụ thể của `item_t` và buffer, chỉ giữ lại synchronization skeleton:
...
`empty_slots` ghi lại còn bao nhiêu slot trống, `filled_slots` ghi lại đã có bao nhiêu phần tử có thể consume. Producer trước tiên consume một slot trống, sau khi đặt data vào thì tăng số phần tử có thể consume lên 1; consumer làm ngược lại. `buffer_mutex` chỉ phụ trách bảo vệ việc `put_item()` và `take_item()` sửa structure của buffer.

Việc retry `EINTR` trong `wait_sem()` cũng không phải để trang trí. Linux man-pages nêu rõ `sem_wait()` có thể bị signal handler interrupt và return `-1`, đồng thời đặt `errno` thành `EINTR`. Nếu example code hoàn toàn không xử lý branch này, người đọc copy lại rất dễ để sót một bug ngẫu nhiên.

Tài liệu Linux Kernel locking nêu rõ semaphore có thể dùng để serialize và wait; khi viết code mới, nên tách semantic như mutual exclusion và event completion vào các mechanism như mutex và completion. Một trong các nguyên nhân là semaphore không có owner rõ ràng, PREEMPT_RT không thể cung cấp priority inheritance cho nó, và việc block trên semaphore có thể dẫn đến priority inversion.

## Condition variable: chờ không phải lock, mà là một condition được thỏa mãn
...
`pthread_cond_wait()` thực hiện một việc rất quan trọng: nó release mutex một cách atomic và cho thread hiện tại chờ condition variable; trước khi wakeup và return, nó lại acquire mutex. Hành động “release lock rồi sleep” này phải được nối liền, nếu không có thể xảy ra lost signal: thread vừa chuẩn bị sleep thì producer đã gửi notification, sau đó consumer mới sleep và không còn ai đánh thức nó.
...
**Thứ nhất, condition chờ phải được viết trong `while`, không viết thành `if`.** POSIX cho phép condition wait xuất hiện spurious wakeup, nghĩa là condition chưa chắc đã được thỏa mãn khi thread thức dậy. Ngay cả khi không có kiểu wakeup này, sau khi nhiều consumer đồng thời được đánh thức, cũng có thể chỉ một thread lấy được data, còn các thread khác lại phát hiện queue rỗng.

**Thứ hai, bản thân condition variable không lưu state; state thực sự phải nằm trong shared variable được mutex bảo vệ**. `pthread_cond_signal()` không phải là đặt một vé có hiệu lực vĩnh viễn vào queue. Nếu signal xảy ra khi không có ai waiting, notification lần đó có thể trôi qua. Thứ thực sự quyết định consumer có thể tiếp tục thực thi hay không là queue length phía sau `queue_empty()`.

**Thứ ba, trong thời gian có waiter, cùng một condition variable nên được dùng cùng một mutex.** POSIX gọi đây là dynamic binding: chỉ cần vẫn còn thread block trên một condition variable nào đó, nếu thread khác dùng mutex khác để wait trên cùng condition variable thì behavior sẽ là undefined. Quy tắc này bình thường không thường được nhắc đến, nhưng nó giải thích vì sao code condition variable thường quản lý “state variable, mutex, condvar” trong cùng một data structure.
...
## Futex: thử trước ở user space, thất bại mới tìm kernel
...
`FUTEX_WAIT` sử dụng compare-and-block: kernel trước tiên xác nhận futex word vẫn bằng expected value mà caller truyền vào, chỉ khi match mới suspend thread. Phép so sánh và hành động block là atomic, nên nó có thể nối user-space atomic operation với kernel sleep queue.
...
Đó cũng là lý do môn Operating System thường dạy atomic instruction trước rồi mới dạy lock implementation. Thứ lock expose cho programmer là `lock()` / `unlock()`, còn bên dưới là các update không thể tách rời được CPU và kernel cùng duy trì.
...
Vấn đề kinh điển là priority inversion. Thread priority thấp L đang giữ một lock, thread priority cao H chờ lock đó; lúc này thread priority trung bình M liên tục chạy và preempt L. Kết quả là H rõ ràng có priority cao nhất nhưng vẫn không chờ được L release lock.

Một hướng giải quyết là priority inheritance. Thread priority thấp đang giữ lock tạm thời inherit priority cao nhất trong các waiter, nhanh chóng chạy xong critical section rồi release lock.

Trong protocol attribute của POSIX mutex có `PTHREAD_PRIO_INHERIT` và `PTHREAD_PRIO_PROTECT`. `rt_mutex` của Linux cũng được thiết kế xoay quanh priority inheritance, dùng để hỗ trợ PI-futex và pthread mutex có priority inheritance attribute.
...
User-space program quan tâm đến cách các thread phối hợp với nhau. Pthreads cung cấp mutex, condition variable và semaphore; C++, Java, Go, Rust lại encapsulate các synchronization tool gần với ngôn ngữ hơn trong runtime và standard library tương ứng.

Lock trong kernel có thêm một tầng ràng buộc về context. Kernel code có thể chạy trong ordinary process context, cũng có thể chạy trong interrupt, soft interrupt hoặc vùng không thể preempt; có path có thể sleep, có path tuyệt đối không thể sleep; khi được giữ, một số lock sẽ disable preemption hoặc interrupt; realtime kernel còn phải xử lý priority inversion và scheduling latency.
...
- Có khả năng bị interrupt handler reenter không?
...
![Sơ đồ minh họa khác biệt về context giữa user-space lock và kernel lock: user space chủ yếu quan tâm thread collaboration, kernel space còn phải xác định có thể sleep, có thể preempt, có đang ở interrupt path và có shared giữa các CPU hay không](https://oss.javaguide.cn/github/javaguide/cs-basics/operating-system/os-lock-kernel-context.png)
...
Tài liệu Linux Kernel locking viết rất chi tiết các quy tắc này, đặc biệt là sự thay đổi trong lock semantic dưới PREEMPT_RT. Application developer thông thường không cần học thuộc toàn bộ chi tiết, nhưng cần biết một điều: không thể hiểu kernel lock chỉ qua vài cái tên như “mutual exclusion/read-write/spin”; nó còn gắn chặt với việc context hiện tại có thể sleep, có thể preempt và có thể xử lý interrupt hay không.
...
Nếu cần giới hạn số thread đồng thời sử dụng một loại resource, semaphore tự nhiên hơn. Ví dụ tối đa 10 task download concurrent hoặc connection pool tối đa 50 connection. Điểm mấu chốt của scenario này là “số lượng”, không phải ai vào critical section.
...
Nếu trong kernel cần bảo vệ một path rất ngắn và context hiện tại không thể sleep, mới cân nhắc spinlock. Trong user-space business code, spin lâu thường là một bad smell.

Nếu đang implement language runtime, thread library hoặc high-performance synchronizer, những cơ chế như futex mới nằm trong phạm vi cần quan tâm. Business code thông thường nên dùng standard library hoặc concurrent library đã được kiểm chứng, thay vì trực tiếp lập trình với futex system call.

Các phán đoán này cũng giải thích vì sao bài viết Java thường đặt `synchronized`, `ReentrantLock`, AQS và CAS cạnh nhau. Java developer đối diện với abstraction ở cấp ngôn ngữ; Operating System đối diện với thread scheduling, block/wakeup, CPU atomic instruction và kernel context.

## Lỗi thường gặp

**Coi lock như một performance switch.**

Lock trước hết bảo đảm correctness, sau đó mới nói đến performance. Nếu shared state bị ghi hỏng, bớt một lock chỉ khiến bug phụ thuộc vào scheduling timing.
...
Semaphore làm được nhiều việc, cũng chính vì vậy code dễ mất semantic khi đọc. Chỉ cần mutual exclusion thì dùng mutex; chỉ cần chờ một one-shot event thì trong kernel các tool trực tiếp hơn như completion thường phù hợp hơn; chỉ dùng semaphore khi cần đếm resource.

**Thực hiện slow operation trong thời gian giữ lock.**

Truy cập disk, gửi network request hoặc chờ external system khi đang giữ lock đều kéo dài critical section. Thread càng nhiều, lock contention càng dễ khuếch đại thành throughput giảm, queue backlog, thậm chí deadlock.

**Bỏ qua lock order.**

Hai thread lần lượt lấy lock theo `A -> B` và `B -> A` rất dễ tạo thành wait cycle. Operating System, database và Java thread đều gặp cùng một vấn đề. Có thể xem phần giới thiệu đầy đủ về deadlock trong [Giải thích chi tiết về deadlock](./dead-lock.md).

## Tổng kết

Không thể hiểu lock trong Operating System chỉ qua một API riêng lẻ. Đây là một nhóm synchronization mechanism được thiết kế xoay quanh shared state, wait condition, số lượng resource và scheduling context.
