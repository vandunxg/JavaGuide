...
Nhiều độc giả phàn nàn rằng các kiến thức về Operating System khá phức tạp, bản thân cũng không có nhiều kiên nhẫn để đọc, nhưng khi phỏng vấn lại thường xuyên gặp phải. Vì vậy, tôi mang đến các câu hỏi thường gặp về Operating System đã được tôi tổng hợp!

Bài viết 《Tổng hợp câu hỏi phỏng vấn Operating System thường gặp (phần trên)》 sẽ bắt đầu từ kiến thức cơ bản về Operating System, sau đó tập trung hệ thống hóa các trọng tâm thường gặp như **user mode/kernel mode, system call, process và thread, IPC, process scheduling, deadlock**. Bài viết phù hợp để nhanh chóng xây dựng danh sách câu hỏi phỏng vấn, đồng thời cũng thích hợp làm điểm bắt đầu để bổ sung kiến thức còn thiếu khi ôn tập.

Học Operating System không chỉ để học thuộc các câu trả lời phỏng vấn. Những tư tưởng như cache, scheduling, synchronization, memory mapping, zero-copy và I/O multiplexing đều có thể thấy trong Redis, Kafka, Nginx, Netty, JVM và database. Khi hiểu rõ cơ chế bên trong, việc hiểu framework tầng trên và các vấn đề performance trên production sẽ dễ dàng hơn nhiều.

Bài viết thiên về “tra cứu nhanh khi phỏng vấn + liên kết các khái niệm cốt lõi”; nếu muốn học sâu, bạn vẫn nên đọc cùng giáo trình và các bài viết chuyên đề. Một phần nội dung trong bài tham khảo 《Operating System hiện đại》 phiên bản thứ ba, xin cảm ơn tác giả.

## Kiến thức cơ bản về Operating System

![Sơ đồ tư duy kiến thức cơ bản về Operating System](https://oss.javaguide.cn/2020-8/image-20200807161118901.png)

### Operating System là gì?

Có thể khái quát Operating System qua bốn điểm sau:

1. Operating System (Operating System, viết tắt là OS) là chương trình quản lý tài nguyên phần cứng và phần mềm của máy tính, là nền tảng của máy tính.
2. Về bản chất, Operating System là một chương trình phần mềm chạy trên máy tính, chủ yếu dùng để quản lý tài nguyên phần cứng và phần mềm của máy tính. Ví dụ: tất cả application chạy trên máy tính của bạn đều gọi memory hệ thống, disk và các phần cứng khác thông qua Operating System.
3. Operating System che giấu sự phức tạp của tầng phần cứng. Operating System giống như người phụ trách việc sử dụng phần cứng, điều phối tất cả công việc liên quan.
4. Kernel của Operating System là phần cốt lõi của Operating System, chịu trách nhiệm quản lý memory hệ thống, thiết bị phần cứng, file system và application. Kernel là cầu nối giữa application và phần cứng, quyết định performance và stability của hệ thống.

Nhiều người dễ nhầm Kernel của Operating System với Central Processing Unit (CPU, Central Processing Unit). Bạn có thể phân biệt đơn giản qua hai điểm sau:

1. Kernel của Operating System thuộc tầng Operating System, còn CPU thuộc phần cứng.
2. CPU chủ yếu cung cấp khả năng tính toán và xử lý các instruction. Kernel chủ yếu chịu trách nhiệm quản lý hệ thống, chẳng hạn như memory management; nó che giấu thao tác trực tiếp với phần cứng.
...
### Operating System có những chức năng chính nào?

Xét từ góc độ resource management, Operating System có 6 chức năng lớn:
...
5. **Quản lý network**: Operating System chịu trách nhiệm quản lý việc sử dụng computer network. Network là phương thức kết nối các máy tính khác nhau trong computer system; Operating System cần quản lý configuration, connection, communication và security của computer network để cung cấp network service hiệu quả và đáng tin cậy.
...
Memory management và file system là hai phần dễ bị hỏi sâu nhất trong phỏng vấn Operating System, sẽ được trình bày riêng trong bài này: [Tổng hợp câu hỏi phỏng vấn Operating System thường gặp (phần dưới)](./operating-system-basic-questions-02.md).

### Những Operating System thường gặp là gì?

#### Windows

Operating System desktop cá nhân phổ biến nhất hiện nay, không cần giới thiệu nhiều vì mọi người đều biết. Giao diện đơn giản, dễ sử dụng, hệ sinh thái software rất tốt.
...
![Giao diện Operating System desktop Windows](./images/windows.png)

#### Unix

Unix là một trong những Operating System multi-user, multi-task có ảnh hưởng nhất thời kỳ đầu. Các system dạng Unix như Linux và BSD sau này đều chịu ảnh hưởng từ Unix. Thị phần của Unix thương mại truyền thống đã giảm rõ rệt, nhưng Unix standard, system certification và các tư tưởng thiết kế của nó vẫn đang được sử dụng.

![Logo Operating System Unix](./images/unix.png)

#### Linux

**Linux là một Operating System dạng Unix miễn phí và open source.** Linux có nhiều distribution khác nhau, nhưng tất cả đều sử dụng **Linux kernel**.

> Nói chính xác, bản thân từ Linux chỉ biểu thị Linux kernel. Trong system GNU/Linux, Linux thực tế chính là Linux kernel, còn các phần còn lại của system chủ yếu là các program do GNU project viết và cung cấp. Riêng Linux kernel không thể trở thành một Operating System có thể hoạt động bình thường.
>
> **Nhiều người có xu hướng dùng từ "GNU/Linux" để biểu đạt "Linux" mà mọi người thường nói đến.**

![Desktop và command line interface của Operating System Linux](https://oss.javaguide.cn/github/javaguide/cs-basics/operating-system/linux/linux.png)

#### Mac OS

Operating System do Apple phát triển, trải nghiệm lập trình tương đương Linux, nhưng giao diện, hệ sinh thái software và user experience đều tốt hơn Operating System Linux.

![Giao diện Operating System desktop macOS](./images/macos.png)
...
Việc chuyển đổi user mode/kernel mode cần đi qua entry point do architecture quy định, lưu state cần thiết và kiểm tra permission cũng như parameter, vì vậy nặng hơn function call thông thường. Tuy nhiên, điều đó không đồng nghĩa với thread context switch: chỉ khi scheduler chuyển sang execution entity khác mới xảy ra thread switch.
...
Cơ chế privilege level này giới hạn application thông thường trong môi trường được kiểm soát, còn phần cứng và system resource được giao thống nhất cho kernel quản lý.
...
Các event chính khiến CPU đi từ user mode vào kernel mode có ba loại:
...
3. **Synchronous exception (Exception)**: do instruction hiện tại kích hoạt, chẳng hạn chia cho zero, illegal instruction hoặc Page Fault. Exception không nhất thiết biểu thị program bị lỗi; lazy allocation, COW và paging từ file cũng có thể kích hoạt Page Fault có thể khôi phục.

Interrupt, exception và system call mô tả lý do CPU đi vào kernel; signal là một software mechanism để kernel thông báo cho process hoặc thread. Truy cập memory bất hợp pháp có thể trước tiên kích hoạt Page Fault, sau khi kernel xác định không thể sửa chữa thì mới gửi `SIGSEGV` đến thread hiện tại.

Chi tiết entry point của các architecture khác nhau không hoàn toàn giống nhau, nhưng đều chuyển đến kernel handler tương ứng dựa trên event type. Có thể xem phần so sánh khái niệm và processing path đầy đủ tại: [Giải thích chi tiết interrupt, exception và system call: từ kernel entry đến page fault](./interrupt-exception-syscall.md).

### System call

#### System call là gì?

System call là service interface có kiểm soát mà kernel cung cấp cho user program. Application không thể trực tiếp thao tác disk, page table, network card và các resource được bảo vệ khác, mà cần thông qua system call để kernel thực hiện thay.
...
- Process management: tạo, hủy, block, wake up process, IPC, v.v.
...
#### Bạn có hiểu process của system call không?

Lấy `read(fd, buf, count)` trên Linux x86-64 làm ví dụ, process của system call có thể khái quát như sau:
...
4. Khi data đã sẵn sàng, kernel hoàn thành việc đọc và trả về result; khi data chưa sẵn sàng, thread hiện tại có thể chuyển sang waiting state, scheduler chuyển sang chạy task có thể chạy khác.
...
![Process của system call](https://oss.javaguide.cn/github/javaguide/cs-basics/operating-system/system-call-procedure.png)
...
Process và thread là một nhóm khái niệm không thể bỏ qua khi phỏng vấn Operating System. Trước tiên, dưới đây là câu trả lời ngắn gọn cho các cách hỏi thường gặp; nếu muốn học có hệ thống, bạn có thể tiếp tục đọc các bài viết chi tiết sau:

- [Giải thích chi tiết process và thread: khác biệt, state, communication, context switch và virtual thread](./process-and-thread.md), path: `./process-and-thread.md`
- [Giải thích chi tiết IPC: pipe, message queue, shared memory, Socket và Binder](./ipc.md), path: `./ipc.md`
- [Giải thích chi tiết CPU scheduling và system load](./cpu-scheduling-and-load.md), path: `./cpu-scheduling-and-load.md`

### Process và thread khác nhau như thế nào?

Process và thread là hai khái niệm cốt lõi về concurrent execution trong Operating System, có thể hiểu mối quan hệ giữa chúng như mối quan hệ giữa **nhà máy và công nhân**.
...
**Process (Process) giống như một nhà máy**. Khi phân bổ resource, Operating System lấy process làm đơn vị cơ bản. Ví dụ, khi tôi khởi động một WeChat, Operating System sẽ tạo cho nó một nhà máy độc lập, phân bổ memory space, file handle và các resource khác dành riêng cho nó. Nhà máy này được cô lập nghiêm ngặt với các nhà máy khác (chẳng hạn browser process tôi mở).

**Thread (Thread) giống như công nhân trong nhà máy**. Một nhà máy có thể có nhiều công nhân, họ dùng chung resource của nhà máy, nhưng mỗi công nhân có toolbox và task list riêng, giúp họ độc lập thực thi các task khác nhau. Ví dụ, trong nhà máy WeChat có thể có một công nhân (thread) chịu trách nhiệm nhận message và một công nhân chịu trách nhiệm render giao diện.
...
| ---------------------- | ------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- |
| Vị trí cơ bản          | Đơn vị cơ bản để phân bổ và cô lập resource                                                      | Đơn vị cơ bản để CPU scheduling và execution                                                    |
...
Số lượng thread cần được thiết lập dựa trên task characteristic, CPU quota, tỷ lệ block và kết quả stress test; không thể chỉ suy ra trực tiếp theo số physical core của host hoặc số concurrent request.

### Có những cách nào để synchronization giữa các thread?

Thread synchronization là việc thực thi concurrent của hai hoặc nhiều thread cùng chia sẻ resource quan trọng. Cần synchronization thread để tránh conflict khi sử dụng resource quan trọng.

Dưới đây là một số cách thread synchronization thường gặp:

1. **Mutex (Mutex)**: sử dụng cơ chế mutex object; chỉ thread sở hữu mutex object mới có quyền truy cập public resource. Vì chỉ có một mutex object nên có thể bảo đảm nhiều thread không đồng thời truy cập public resource. Ví dụ, keyword `synchronized` và các `Lock` khác nhau trong Java đều là cơ chế này.
...
**PCB (Process Control Block)**, tức process control block, là data structure được Operating System dùng để quản lý và theo dõi process; mỗi process tương ứng với một PCB độc lập. Bạn có thể xem PCB là bộ não của process.

Khi Operating System tạo process mới, nó phân bổ một process ID duy nhất cho process đó, đồng thời tạo process control block tương ứng. Khi process thực thi, thông tin trong PCB liên tục thay đổi; Operating System dựa vào những thông tin này để quản lý và scheduling process.
...
Khi xảy ra context switch, Operating System lưu register và execution context khác của process hiện tại vào PCB, sau đó khôi phục execution context từ PCB của process tiếp theo để process đó tiếp tục thực thi từ vị trí tạm dừng lần trước.
...
Trong một số giáo trình hoặc implementation của system, PCB và TCB tách biệt: PCB thiên về process-level resource, TCB thiên về thread-level execution context. Implementation của Linux khá đặc biệt: nó xem cả process và thread đều là task, dùng `task_struct` để mô tả scheduling entity, sau đó phân biệt process và thread thông qua việc resource structure có được share hay không. Khi hiểu không cần quá bận tâm đến tên gọi; điều quan trọng là phân biệt: **address space, file table và các thành phần khác thuộc resource boundary; stack, register, program counter và các thành phần khác thuộc execution context**.

### Process có những state nào?

Thông thường chúng ta chia process thành 5 state chính, khá giống thread:
...
- **Terminated state (terminated)**: process đang biến mất khỏi system. Có thể process kết thúc bình thường hoặc bị gián đoạn và thoát do nguyên nhân khác.
...
Khi xem state transition, cần chú ý nguyên nhân trigger: ready state nhận CPU rồi đi vào running state; running state hết time slice có thể quay lại ready state; khi đang chạy mà khởi tạo blocking I/O, chờ lock hoặc chờ event thì sẽ đi vào blocked state; sau khi event mà blocked state chờ hoàn tất, thường process trước tiên quay lại ready state và chờ được scheduling lần tiếp theo.

Một số giáo trình còn thêm **suspended state**. Suspend nhấn mạnh process tạm thời không ở trong memory hoặc bị user/system tạm dừng; block nhấn mạnh process đang chờ một event. Hai khái niệm này không giống nhau: process có thể bị block nhưng vẫn ở trong memory, cũng có thể bị swap ra external storage rồi ở trạng thái blocked và suspended.

### Có những cách nào để IPC?

Process mặc định có virtual address space độc lập, không thể trực tiếp truy cập user-mode memory của nhau, vì vậy cần **IPC (Inter-Process Communication, IPC)**.

Trong phỏng vấn, trước tiên chỉ cần trả lời theo use case:

- Process cha-con truyền ít byte stream: anonymous pipe.
- Process không có quan hệ họ hàng communication trên cùng máy: named pipe, Unix Domain Socket.
...
- Communication giữa các máy: TCP/UDP Socket hoặc RPC framework tầng trên.

Có thể xem cách phân loại, boundary và lựa chọn chi tiết hơn tại: [Giải thích chi tiết IPC: pipe, message queue, shared memory, Socket và Binder](./ipc.md), path: `./ipc.md`.

### fork, exec, wait lần lượt làm gì?

Trong lập trình Unix/Linux, khi tạo process và thay thế program thường không thể bỏ qua ba thao tác `fork()`, `exec()`, `wait()`. Trước tiên hãy ghi nhớ câu trả lời ngắn khi phỏng vấn; các chi tiết khác về file descriptor inheritance, copy-on-write và `fork` trong multi-thread có thể xem tại: [Giải thích chi tiết process và thread](./process-and-thread.md), path: `./process-and-thread.md`.
...
Khi Shell khởi động external command, call chain thường là: Shell trước tiên `fork()` child process, child process sau đó `exec()` thành target program, parent process dùng `wait()` hoặc `waitpid()` để chờ và thu hồi exit state. Nếu parent process không thu hồi child process đã thoát, có thể để lại zombie process.

### Context switch là gì?

Context switch là việc CPU chuyển từ execution entity này sang execution entity khác. Operating System cần lưu execution context như register, program counter và stack pointer của execution entity hiện tại, sau đó khôi phục execution context của execution entity tiếp theo.
...
### Có những thuật toán scheduling process nào?
...
Với cách này, một khi CPU được phân bổ cho process, process sẽ tiếp tục chạy cho đến khi task hoàn tất hoặc chủ động nhường CPU (chẳng hạn chờ I/O).

1. **First Come, First Served (FCFS, First Come, First Served)**: chạy theo thứ tự đến, implementation đơn giản; khi task dài xếp trước, task ngắn phía sau cũng phải chờ, dẫn đến convoy effect.
2. **Shortest Job First (SJF, Shortest Job First)**: ưu tiên chạy task có execution time dự kiến ngắn, có thể giảm average waiting time; trong thực tế khó dự đoán chính xác độ dài task, đồng thời task dài có thể không được chạy trong thời gian dài.

**Loại thứ hai: preemptive scheduling (Preemptive)**

Operating System có thể tạm dừng task hiện tại và giao CPU cho task có thể chạy khác phù hợp hơn. Operating System đa dụng hiện đại thường hỗ trợ preemption.

- **Round-Robin (RR, Round-Robin)**: mỗi task lần lượt chạy một time slice. Time slice quá ngắn sẽ khuếch đại context switch overhead, quá dài thì dần gần với FCFS.
- **Priority scheduling (Priority)**: ưu tiên chạy task có priority cao, có thể thể hiện mức độ khẩn cấp của task, nhưng cần xử lý vấn đề starvation của task có priority thấp.

**Multi-Level Feedback Queue (MLFQ, Multi-Level Feedback Queue)** thiết lập nhiều queue theo priority và điều chỉnh vị trí dựa trên behavior khi chạy của task. Task mới thường trước tiên đi vào queue priority cao; CPU-bound task thường dùng hết time slice sẽ dần bị hạ priority, còn interactive task thường chủ động chờ I/O có thể giữ priority cao hơn. Quy tắc nâng/hạ priority cụ thể và chống starvation phụ thuộc vào implementation.

FCFS, SJF, RR, priority và MLFQ chủ yếu là model đơn giản hóa trong giáo trình. Linux thực tế scheduling task hoặc scheduling entity; ordinary task trong thời gian dài được CFS phân bổ CPU theo weight và `vruntime`; từ Linux 6.6, fair scheduling class bắt đầu đưa EEVDF vào, dùng lag và virtual deadline để cải thiện việc lựa chọn task. Implementation cụ thể trên máy production còn phụ thuộc kernel version và distribution patch.

Giới thiệu chi tiết: [Giải thích chi tiết CPU scheduling và system load](./cpu-scheduling-and-load.md).

### Vậy rốt cuộc ai scheduling process này?

Đảm nhiệm scheduling là **scheduler (Scheduler)** trong kernel của Operating System. Khi task hiện tại bị block, chủ động nhường CPU, time slice hoặc execution quota dùng hết, priority thay đổi, hoặc task phù hợp hơn được wake up, kernel đều có thể trigger scheduling.

Giáo trình còn dùng **dispatcher** để mô tả process đưa quyết định scheduling lên CPU:

- Scheduler chọn task tiếp theo từ run queue.
- Dispatch process hoàn thành context switch cụ thể:
...
Deadlock (Deadlock) mô tả tình huống như sau: một nhóm process/thread chờ nhau giải phóng resource hoặc hoàn thành action, quan hệ waiting tạo thành vòng khép kín, khiến tất cả participant đều không thể tự tiếp tục thực thi.
...
**Lưu ý**: bốn điều kiện này là điều kiện cần để deadlock xảy ra và phải đồng thời tồn tại. Chỉ thỏa mãn một hoặc hai điều kiện chưa chắc xảy ra deadlock; ngược lại, chỉ cần có thể ổn định phá vỡ bất kỳ một điều kiện nào thì có thể ngăn deadlock từ cấu trúc.
...
Khi phỏng vấn, trả lời đến mức này là đủ: giải quyết deadlock thường có bốn hướng tư duy là **prevention, avoidance, detection và resolution/recovery**.
...
- **Avoidance**: trước khi phân bổ resource, kiểm tra xem system còn ở safe state hay không; đại diện điển hình là Banker’s algorithm. Cách này thiên về hiểu theo giáo trình, system business thông thường rất ít khi implementation trực tiếp.
...
- 《Operating System của Tưởng Tiểu Đan》 phiên bản thứ tư
- 《Hiểu sâu về Computer Systems》
- 《Học lại Operating System》
- Tại sao Operating System cần phân biệt user mode và kernel mode: <https://blog.csdn.net/chen134225/article/details/81783980>
