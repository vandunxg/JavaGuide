---
title: "Chuyên đề Operating System: process và thread, memory management, file system, I/O multiplexing, Linux và Shell"
description: "Lộ trình học và ôn tập phỏng vấn về Operating System, bao quát process, thread, inter-process communication, lock và synchronization mechanism, deadlock, virtual memory, zero-copy, I/O multiplexing, file system, Linux cơ bản, Shell programming và các câu hỏi phỏng vấn Operating System thường gặp."
...
head:
  - - meta
    - name: keywords
      content: Operating System,câu hỏi phỏng vấn về Operating System,process,thread,inter-process communication,IPC,lock và synchronization,mutex,semaphore,condition variable,futex,deadlock,memory management,virtual memory,zero-copy,I/O multiplexing,select,poll,epoll,file system,Linux,Shell,câu hỏi phỏng vấn backend
---
...
- Backend developer đang học có hệ thống kiến thức cơ bản về Operating System.
- Người chuẩn bị cho câu hỏi phỏng vấn Operating System trong tuyển dụng campus, tuyển dụng người đã có kinh nghiệm và các công ty công nghệ vừa/lớn.
- Độc giả chỉ học thuộc rời rạc về process và thread, deadlock, memory management, Linux command.
- Kỹ sư muốn xây nền tảng cho Java Concurrency, JVM, database và network programming.

## Trọng tâm học

- Operating System chịu trách nhiệm quản lý CPU, memory, file, I/O và process, là nền tảng để hiểu cơ chế vận hành của phần mềm ở tầng trên.
- Process, thread và inter-process communication là các khái niệm nền tảng của concurrent programming, performance của server và troubleshooting.
...
- Linux và Shell là những kỹ năng thường dùng trong backend development, deploy, troubleshooting và viết automation script.

## Thứ tự đọc đề xuất

1. [Tổng hợp câu hỏi phỏng vấn Operating System thường gặp (phần 1)](./operating-system-basic-questions-01.md): trước tiên nắm các câu hỏi trọng tâm về kiến thức cơ bản của Operating System, process và thread, deadlock, memory management.
...
4. [Giải thích chi tiết interrupt, exception và system call: từ kernel entry đến page fault](./interrupt-exception-syscall.md): lấy `read()` làm đầu mối để kết nối hardware interrupt, synchronous exception, system call, signal, page fault và thread switch.
5. [Giải thích chi tiết CPU scheduling và system load](./cpu-scheduling-and-load.md): tìm hiểu scheduling algorithm, CFS/EEVDF, load average, CPU usage và cách troubleshooting trong production.
...
7. [Giải thích chi tiết lock và synchronization mechanism trong Operating System: mutex, semaphore, condition variable, spinlock và futex](./os-lock-and-sync.md): tìm hiểu ranh giới trách nhiệm của critical section, mutex, semaphore, condition variable, spinlock và futex.
8. [Giải thích chi tiết deadlock: bốn điều kiện cần, troubleshooting Java deadlock và xử lý database deadlock](./dead-lock.md): làm rõ deadlock wait cycle, bốn điều kiện cần, troubleshooting Java thread deadlock và retry transaction trong database.
...
11. [Giải thích chi tiết file system của Operating System: inode, VFS, Page Cache và journaling mechanism](./file-system.md): tìm hiểu file, directory, inode, VFS, Page Cache và cơ chế khôi phục từ journal.
12. [Giải thích chi tiết I/O multiplexing: nguyên lý và khác biệt của select, poll, epoll](./io-multiplexing.md): tìm hiểu cơ chế kernel phía sau việc một thread xử lý lượng lớn connection.
13. [Giải thích chi tiết zero-copy: mmap, sendfile và splice](./zero-copy.md): làm rõ copy path và trường hợp sử dụng của traditional I/O, mmap, sendfile, splice.
14. [Tổng hợp kiến thức cơ bản Linux](./linux-intro.md): nắm cấu trúc directory, file permission, command thường dùng và kỹ năng troubleshooting cơ bản.
...
- [Tổng hợp câu hỏi phỏng vấn Operating System thường gặp (phần 1)](./operating-system-basic-questions-01.md): bao quát các câu hỏi thường gặp về kiến thức cơ bản của Operating System, process và thread, deadlock, memory management.
...
- [Giải thích chi tiết process và thread: khác biệt, trạng thái, communication, context switch và virtual thread](./process-and-thread.md): làm rõ ranh giới resource của process và thread, state transition, cơ chế tạo process trong Linux và Java virtual thread.
...
- [Giải thích chi tiết lock và synchronization mechanism trong Operating System: mutex, semaphore, condition variable, spinlock và futex](./os-lock-and-sync.md): làm rõ critical section, mutex, semaphore, condition variable, spinlock, futex, memory ordering và context của kernel lock.
...
- [Giải thích chi tiết file system của Operating System: inode, VFS, Page Cache và journaling mechanism](./file-system.md): làm rõ file, directory, inode, dentry, file descriptor, VFS, Page Cache, fsync và cơ chế journaling.
...
- Process và thread khác nhau thế nào? Thread chia sẻ những resource nào với nhau?
- Inter-process communication có những phương thức nào? Mỗi phương thức phù hợp với trường hợp sử dụng nào?
- Context switch là gì? Việc context switch thường xuyên gây ảnh hưởng gì?
- Mutex, semaphore, condition variable, spinlock và futex lần lượt giải quyết vấn đề gì?
- Điều kiện cần để deadlock xảy ra là gì? Trong Java và database, troubleshooting deadlock như thế nào?
...
- [Hệ thống kiến thức về Computer Basics](../)
...
- [Giải thích chi tiết memory area của JVM](../../java/jvm/memory-area.md)
