...
description: Giới thiệu ngắn gọn một số khái niệm và lệnh thường gặp về Linux mà lập trình viên Java cần biết.
...
Giới thiệu ngắn gọn một số khái niệm và lệnh thường gặp về Linux mà lập trình viên Java cần biết.
...
- **Hệ thống tương tự Unix**: Linux là một operating system tự do, mã nguồn mở và tương tự Unix.
...
- **Cha đẻ của Linux (Linus Benedict Torvalds)**: Một nhân vật huyền thoại trong lĩnh vực lập trình, một chuyên gia thực thụ và là hình mẫu để chúng ta ngưỡng mộ. Ông là tác giả đầu tiên của **Linux kernel**, sau đó khởi xướng open-source project này và giữ vai trò kiến trúc sư chính của Linux kernel. Ông cũng khởi xướng Git, một open-source project, và là một trong những developer chính.
...
Năm 1989, Linus Torvalds gia nhập lữ đoàn địa phương của Quân đội Phần Lan, thực hiện nghĩa vụ quân sự 11 tháng với quân hàm thiếu úy, chủ yếu phục vụ tại bộ phận máy tính và làm nhiệm vụ tính toán đạn đạo. Trong thời gian phục vụ, ông mua giáo trình do Andrew Stuart Tanenbaum viết cùng source code của minix và bắt đầu nghiên cứu operating system. Năm 1990, sau khi xuất ngũ và trở lại trường đại học, ông bắt đầu tiếp xúc với Unix.

> **Minix** là một phiên bản mini của operating system tương tự Unix, do giáo sư Tanenbaum tạo ra để phục vụ giảng dạy và sử dụng thiết kế microkernel. Nó đã truyền cảm hứng cho sự ra đời của Linux kernel.

Năm 1991, Linus Torvalds open-source Linux kernel. Linux lấy một chú chim cánh cụt đáng yêu làm biểu tượng, tượng trưng cho tinh thần dám nghĩ dám làm và yêu cuộc sống.
...
Linus Torvalds chỉ open-source Linux kernel. Như đã đề cập ở trên, kernel của operating system có vai trò riêng. Một số tổ chức hoặc nhà cung cấp đóng gói Linux kernel cùng nhiều software và tài liệu, đồng thời cung cấp giao diện cài đặt hệ thống và các công cụ cấu hình, thiết lập, quản lý hệ thống, từ đó tạo thành Linux distribution.

> Kernel chủ yếu chịu trách nhiệm quản lý memory của hệ thống, quản lý hardware device, quản lý file system và quản lý application.
...
Đối với người mới học Linux, hiện không nên tiếp tục chọn CentOS một cách không điều kiện. CentOS Linux 8 đã ngừng được duy trì vào cuối năm 2021, CentOS Linux 7 cũng đã kết thúc vòng đời vào tháng 6 năm 2024; CentOS Stream hiện nay là upstream continuous delivery branch của RHEL, định vị khác với “phiên bản tái xây dựng tương thích với RHEL ổn định” trước đây.
...
Trong Linux operating system, mọi resource được operating system quản lý, chẳng hạn network interface card, disk drive, printer, thiết bị input/output, regular file hoặc directory, đều được xem là file. Đây là một khái niệm quan trọng trong Linux system: “mọi thứ đều là file”.

Khái niệm này bắt nguồn từ triết lý UNIX, tức quản lý và truy cập mọi resource bằng cách abstraction chúng thành file. Linux file system cũng kế thừa ý tưởng thiết kế của UNIX file system. Thiết kế này giúp Linux system quản lý và thao tác các loại resource khác nhau thông qua một file interface thống nhất, từ đó thực hiện cách thao tác file thống nhất. Ví dụ, có thể xử lý network interface, disk drive, device file theo cách tương tự việc đọc và ghi file, khiến việc thao tác và quản lý các resource này thống nhất và đơn giản hơn.

Ý tưởng thiết kế lấy file làm trung tâm mang lại tính linh hoạt và khả năng mở rộng cho Linux system, giúp Linux trở thành một operating system mạnh mẽ. Đồng thời, đây cũng là một đặc điểm lớn của Linux system, được đông đảo user và developer yêu thích, đánh giá cao.
...
Nói đơn giản: inode dùng để duy trì thông tin file được chia thành bao nhiêu block, địa chỉ của từng block, owner, creation time, permission, size và các thông tin khác.

Tóm tắt lại inode và block:

- **inode**: Ghi lại attribute information của file, có thể dùng lệnh `stat` để xem thông tin inode.
...
Linux/Unix file system sử dụng inode để định danh file system object. Khi rename file trong cùng một file system, thông thường chỉ directory entry bị thay đổi, inode number không đổi; sau khi xóa hard-link cuối cùng, nếu cũng không còn process nào tiếp tục mở file, inode sẽ được giải phóng và số của nó có thể được sử dụng lại sau này. Khi truy cập file qua path, vẫn phải parse directory entry trước, không thể bỏ qua việc tìm path và dùng inode number như một file identifier toàn cục ổn định.

Tuy nhiên, việc sử dụng inode number cũng khiến file system trở nên abstract và phức tạp hơn ở tầng user và application, cần thông qua system command hoặc file system interface để truy cập và quản lý thông tin inode của file.
...
- Hard link có một số hạn chế: không thể tạo hard link cho directory hoặc file không tồn tại, đồng thời hard link không thể vượt qua file system.
- Lệnh `ln` dùng để tạo hard link.

**2. Soft Link (Symbolic Link hoặc Symlink)**

- Soft link có inode number khác source file mà trỏ tới một file path.
- Sau khi source file bị xóa, soft link vẫn tồn tại nhưng trỏ tới một file path không hợp lệ.
- Soft link tương tự shortcut trong Windows system.
- Khác với hard link, có thể tạo soft link cho directory hoặc file không tồn tại, đồng thời soft link có thể vượt qua file system.
...
Mỗi file system có một inode namespace riêng, directory entry chỉ có thể tham chiếu tới inode trong file system hiện tại, không thể trực tiếp trỏ tới inode thuộc file system khác. Vì vậy, hard link không thể vượt qua ranh giới file system; nguyên nhân không đơn giản là do inode number bị trùng.

### File type trong Linux

Linux hỗ trợ nhiều file type, trong đó các file type rất quan trọng gồm: **regular file**, **directory file**, **link file**, **device file**, **pipe file**, **Socket file** và các loại khác.

- **Regular file (-)**: Dùng để lưu information và data, Linux user có thể xem, thay đổi và xóa regular file theo permission. Ví dụ: image, sound, PDF, text, video, source code và nhiều loại khác.
- **Directory file (d, directory file)**: Directory cũng là một loại file, dùng để biểu thị và quản lý file trong system; directory file chứa một số file name và subdirectory name. Mở directory thực chất là mở directory file.
- **Symbolic link file (l, symbolic link)**: Lưu chuỗi target path. Khi truy cập symbolic link, kernel sẽ parse lại target theo path này.
...
Mỗi file type có mục đích và attribute khác nhau, có thể dùng các command như `ls`, `file` để xem thông tin file type.
...
Linux sử dụng cấu trúc phân cấp gọi là directory tree để tổ chức file và directory. Directory tree bắt đầu từ root directory (/), mở rộng xuống dưới và tạo thành một loạt directory và subdirectory. Mỗi directory có thể chứa file và subdirectory khác. Cấu trúc phân cấp rõ ràng, giống như một cái cây lộn ngược.

![Cấu trúc directory của Linux](./images/Linux目录树.png)

**Mô tả các directory thường gặp:**

- **/bin:** Chứa binary executable file (ls, cat, mkdir và các file khác), các command thường dùng thường nằm ở đây.
- **/etc:** Chứa system management và configuration file.
- **/home:** Chứa root directory của mọi user file, là điểm bắt đầu của user home directory. Ví dụ, home directory của user `user` là `/home/user`, có thể biểu thị bằng `~user`.
...
- **/mnt:** Mount point để system administrator cài đặt temporary file system; system cung cấp directory này để user tạm thời mount file system khác.
...
## Linux command thường dùng

Dưới đây chỉ liệt kê một số command tương đối thường dùng.

Đề xuất một website tra nhanh Linux command, rất hữu ích. Nếu quên hoặc không hiểu một số command, bạn có thể tìm lời giải tại đây: Sổ tay tra nhanh Linux command online: <https://wangchujiang.com/linux-command/>.
...
Ngoài ra, website [shell.how](https://www.shell.how/) có thể dùng để giải thích ý nghĩa của các command thường gặp, hỗ trợ bạn học Linux basic command và các command thường dùng khác (chẳng hạn Git, NPM).
...
- `cd .. (hoặc cd../)`: Chuyển tới directory ở tầng trước.
- `cd /`: Chuyển tới root directory của system.
- `cd ~`: Chuyển tới user home directory.
- **`cd -`:** Chuyển tới directory của thao tác trước đó.
...
- `ll`: `ll` là alias của `ls -l`; command `ll` có thể xem thông tin chi tiết của toàn bộ directory và file trong directory đó.
...
- `cp [option] source-file/directory target-file/directory`: Copy file hoặc directory (di chuyển). Ví dụ: `cp file.txt /home/file.txt` copy file `file.txt` tới directory `/home` và đổi tên thành `file.txt`. `cp -r source destination` copy directory `source` cùng toàn bộ subdirectory và file bên trong tới directory `destination`, đồng thời giữ nguyên attribute của source file và cấu trúc directory.
- `mv [option] source-file/directory target-file/directory`: Di chuyển file hoặc directory (di chuyển), cũng có thể dùng để rename file hoặc directory. Ví dụ: `mv file.txt /home/file.txt` di chuyển file `file.txt` tới directory `/home` và đổi tên thành `file.txt`. Kết quả của `mv` khác `cp`: `mv` giống như file được “chuyển nhà”, số lượng file không tăng, còn `cp` copy file nên số lượng file tăng.
...
- `cat/more/less/tail file-name`: Xem file (tra cứu). Command `tail -f file` có thể monitor động một file, chẳng hạn log file của Tomcat thay đổi theo quá trình chạy của program; có thể dùng `tail -f catalina-2016-11-11.log` để monitor thay đổi của file.
- `vim file-name`: Sửa nội dung file (sửa). Vim editor là một component mạnh trong Linux và là phiên bản nâng cao của vi editor. Vim editor có rất nhiều command và shortcut, nhưng ở đây không trình bày từng cái; bạn cũng không cần nghiên cứu quá sâu, chỉ cần cơ bản biết cách dùng vim để edit file là được. Trong development thực tế, vim editor chủ yếu dùng để sửa configuration file. Các bước thường dùng là: `vim file------>vào file----->command mode------>nhấn i để vào edit mode----->edit file------->nhấn Esc để vào bottom-line mode----->nhập :wq/q!` (nhập wq nghĩa là ghi nội dung và thoát, tức save; nhập q! nghĩa là force exit không save).
...
- z: Gọi gzip compression command để nén.
- c: Đóng gói file.
- v: Hiển thị quá trình thực hiện.
- f: Chỉ định file name.
...
- `ftp` (File Transfer Protocol): Cung cấp cách đơn giản để connect tới remote FTP server và thực hiện các thao tác upload, download, delete file. Trước khi sử dụng cần connect và login remote FTP server. Sau khi vào FTP command-line interface, có thể dùng command `put` để upload local file tới remote host, dùng `get` để download file từ remote host về local, dùng `delete` để xóa file trên remote host. Phần này không trình diễn.
...
> Tiếp theo sẽ giải thích chi tiết file type, permission trong Linux, file owner, file group và other group là gì.
...
Các Linux distribution phổ biến hiện nay cơ bản sử dụng systemd để quản lý service. Cách khuyến nghị là viết một unit file `zookeeper.service`, sau đó dùng các command dưới đây để thiết lập auto-start khi boot:
...
Nếu sửa service file, trước tiên cần chạy `sudo systemctl daemon-reload` để systemd reload configuration. `chkconfig --add zookeeper`, `chkconfig --list` là cách làm của thời kỳ SysV init, chỉ dùng trong distribution cũ hoặc môi trường compatibility.

### User management

Linux system là một time-sharing operating system đa user, đa task. Bất kỳ user nào muốn sử dụng system resource đều phải xin account từ system administrator trước, sau đó vào system với identity của account này.

Một mặt, user account giúp system administrator theo dõi user sử dụng system và kiểm soát quyền truy cập system resource của họ; mặt khác, account giúp user tổ chức file và cung cấp security protection cho user.

**Command liên quan đến Linux user management:**

- `useradd [option] user-name`: Tạo user account. Account được tạo bằng instruction `useradd` thực tế được lưu trong text file `/etc/passwd`.
...
User group management gồm thêm, xóa và sửa user group. Việc tăng, xóa và sửa group thực tế là cập nhật file `/etc/group`.
...
- `htop [option]`: Tương tự `top` nhưng cung cấp interface interactive và thân thiện hơn, cho phép user thao tác interactive, hỗ trợ color theme, cuộn ngang hoặc dọc để xem process list và hỗ trợ mouse.
- `uptime [option]`: Dùng để xem system đã chạy tổng cộng bao lâu, average load của system và các thông tin khác.
- `vmstat [interval] [repeat-count]`: vmstat (Virtual Memory Statistics) có nghĩa là hiển thị virtual memory status, nhưng cũng có thể báo cáo overall system status về process, memory, I/O và các thành phần khác.
...
- `sar [option] [time-interval] [repeat-count]`: Dùng để thu thập, báo cáo và phân tích performance statistics của system, bao gồm CPU usage, memory usage, disk I/O, network activity và các thông tin chi tiết khác. Điểm đặc biệt là có thể sample system liên tục để thu được lượng lớn sample data. Sample data và kết quả phân tích có thể lưu vào file; khi sử dụng, resource của system tiêu tốn rất ít.
- `ps [option]`: Dùng để xem process information trong system, bao gồm process ID, status, resource usage và các thông tin khác. `ps -ef`/`ps -aux`: Hai command này đều xem process đang chạy trong system hiện tại, khác nhau ở format hiển thị. Nếu muốn xem process cụ thể, có thể dùng format như `ps aux|grep redis` (xem process chứa chuỗi redis), hoặc dùng `pgrep redis -a`.
...
- `journalctl [option]`: Dùng để xem systemd log, rất thường dùng khi kiểm tra service start thất bại hoặc system error. Ví dụ: `journalctl -u nginx -f` realtime xem service log của nginx, `journalctl -xe` xem context của system error gần đây.
...
- `ss [option]`: Dễ dùng hơn `netstat`, cung cấp network connection information nhanh và chi tiết hơn.
...
- `sudo + other command`: Thực thi instruction với identity của system administrator, nghĩa là instruction thực thi qua sudo giống như do root trực tiếp thực thi.
- `grep [option] "search-content" file-path`: Một text search command mạnh và thường dùng. Nó có thể match theo string hoặc regular expression được chỉ định trong file hoặc command output, phù hợp với nhiều scenario như log analysis, text filtering và quick locating. Ví dụ: bỏ qua hoa thường, tìm các dòng chứa error trong syslog: `grep -i "error" /var/log/syslog`; tìm mọi process liên quan đến java: `ps -ef | grep "java"`.
- `kill -9 process-pid`: Kill process (`-9` biểu thị force terminate), trước tiên dùng ps để tìm process, sau đó dùng kill để kill process.
...
Trong Linux system, environment variable dùng để định nghĩa một số parameter của system runtime environment, chẳng hạn home directory khác nhau của mỗi user (HOME).
...
Khuyến nghị cấu hình user-level environment variable trong `~/.bash_profile`, system-level environment variable trong `/etc/profile.d`.
...
Có thể dùng command `export` để sửa environment variable được chỉ định. Tuy nhiên, cách sửa environment variable này chỉ có hiệu lực với shell terminal hiện tại và mất hiệu lực khi đóng shell terminal. Sau khi sửa sẽ có hiệu lực ngay.
...
Có thể dùng command `vim` để sửa environment variable configuration file. Cách này khiến việc sửa environment variable có hiệu lực lâu dài.
...
Sau khi sửa, cần dùng command `source` để áp dụng hoặc đóng shell terminal rồi login lại.
