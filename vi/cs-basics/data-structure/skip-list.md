...
Sorted set ZSet của Redis có cấu trúc bên trong kết hợp skip list và hash table, vì vậy skip list thường xuất hiện cùng Redis trong các buổi phỏng vấn backend.
...
Node của linked list thông thường chỉ có một pointer `next`, còn node của skip list có nhiều forward pointer. Nếu một node xuất hiện ở tầng 3 thì nghĩa là nó có pointer tương ứng ở các tầng 0, 1, 2.
...
Skip list không duy trì balance bằng rotation hay recoloring, mà dựa vào việc tạo các tầng ngẫu nhiên.
...
Kết quả là: node ở tầng càng cao càng ít, node ở tầng càng thấp càng dày. Trong trường hợp lý tưởng, tầng 1 giữ lại khoảng một nửa số node, tầng 2 lại giữ một nửa số đó, tầng 3 tiếp tục giảm. Dù không balance tuyệt đối, xét theo xác suất thì height sẽ duy trì ở mức `O(logn)`.

Đây cũng là nguồn gốc của từ “skip” trong tên skip list: index ở tầng cao cho phép quá trình query bỏ qua từng nhóm element.

## Trọng tâm phỏng vấn

- Giải thích rõ vì sao skip list query nhanh hơn linked list thông thường.
- Mô tả được quy trình cơ bản của tìm kiếm, insert và delete.
- Nêu được complexity trung bình của query, insert và delete trong skip list là `O(logn)`.
...
Nếu target không tồn tại, skip list cũng có thể tìm được vị trí mà nó nên được insert: node cuối cùng ở tầng dưới cùng có giá trị nhỏ hơn target chính là predecessor node của vị trí cần insert.
...
Khi delete node, cũng cần tìm predecessor node ở mỗi tầng, sau đó cho pointer bỏ qua target node.

Skip list không duy trì balance bằng rotation mà dùng số tầng ngẫu nhiên để giữ chiều cao của index trong phạm vi hợp lý. Đây cũng là lý do nó dễ triển khai hơn red-black tree.

Khi viết code insert thực tế, thường sẽ duy trì một array `update`: `update[i]` biểu thị node mà node mới sẽ được insert ngay sau nó ở tầng `i`. Trong quá trình tìm vị trí insert, các predecessor node này được ghi lại luôn; sau khi có số tầng ngẫu nhiên thì có thể lần lượt sửa pointer.

Delete cũng tương tự: trước tiên tìm predecessor node ở mỗi tầng; nếu node tiếp theo ở tầng đó đúng là target node thì cho `forward` của predecessor node trỏ tới node tiếp theo của target node.
...
| Range query | `O(logn + k)`         | Định vị start point trước, sau đó trả về tuần tự k element |

Space complexity ở mức `O(n)`, nhưng sẽ có thêm một số index pointer so với linked list thông thường.

`O(logn)` ở đây là complexity theo nghĩa trung bình, phụ thuộc vào probabilistic balance do số tầng ngẫu nhiên mang lại. Skip list không đưa ra ràng buộc balance nghiêm ngặt trong worst case như red-black tree, nhưng khi random function hoạt động bình thường và parameter được thiết lập hợp lý, performance thường rất ổn định.

Range query là một trường hợp skip list xử lý rất tốt: trước tiên dùng `O(logn)` để định vị start point của range, sau đó duyệt tuần tự qua `k` element theo linked list ở tầng dưới cùng.

## Chọn skip list hay red-black tree?

| Điểm so sánh          | Skip list                                       | Red-black tree                                                    |
| --------------------- | ----------------------------------------------- | ----------------------------------------------------------------- |
| Cách duy trì balance  | Số tầng ngẫu nhiên                              | Rotation và recoloring                                            |
...
| Worst-case complexity | Phụ thuộc tính ngẫu nhiên                       | Có ràng buộc balance nghiêm ngặt                                  |
| Đại diện thực tế      | Redis ZSet                                      | Java `TreeMap`, tree hóa `HashMap`                                |
...
Hash table phù hợp để query score theo member, còn skip list phù hợp để sort và range query theo score. Sau khi kết hợp, ZSet có thể đồng thời hỗ trợ tìm kiếm nhanh và duyệt có thứ tự.
...
- Khi thực hiện các range query như `ZRANGE`, `ZRANGEBYSCORE`, skip list có thể định vị start point trước, sau đó trả về tuần tự các result theo linked list.
- Nếu node của skip list duy trì thông tin span thì còn có thể hỗ trợ các thao tác liên quan đến rank.

Cần lưu ý rằng Redis sẽ sử dụng các internal encoding khác nhau tùy theo data size và config để tiết kiệm memory. Trong phỏng vấn, câu “ZSet sử dụng hash table + skip list” thường nói đến core structure của nó khi xử lý sorted set có quy mô lớn.
