...
description: "Tổng hợp đặc điểm và thao tác của array/linked list/stack/queue, kèm phân tích complexity và ứng dụng điển hình, giúp nắm cách chọn và triển khai linear structure."
...
**Array** là một data structure phổ biến. Nó gồm các element cùng kiểu và được lưu trong một vùng memory liên tục.

Có thể trực tiếp tính địa chỉ lưu trữ của element từ index của nó.
...
**Linked list** tuy là một linear list nhưng không lưu trữ data theo thứ tự tuyến tính và không sử dụng vùng memory liên tục.

Complexity của thao tác chèn và xóa trong linked list là O(1) nếu biết element ngay trước vị trí cần thao tác. Tuy nhiên, tìm một node hoặc truy cập node ở vị trí cụ thể có complexity O(n).

Linked list khắc phục nhược điểm phải biết trước kích thước data của array và tận dụng memory để quản lý động linh hoạt. Tuy nhiên, linked list không tiết kiệm space; so với array, nó chiếm nhiều space hơn vì mỗi node còn lưu pointer trỏ tới node khác. Ngoài ra, linked list không có ưu điểm random access như array.
...
**Singly linked list** chỉ có một hướng; mỗi node có một successor pointer `next` trỏ tới node phía sau. Vì vậy, data structure linked list thường không liên tục trong physical memory. Theo thói quen, node đầu tiên được gọi là head node; linked list thường có một node `head` không lưu value nào, từ head node có thể duyệt toàn bộ linked list. Tail node thường trỏ tới `null`.
...
**Circular linked list** thực ra là một singly linked list đặc biệt. Khác với singly linked list, tail node của circular linked list không trỏ tới `null` mà trỏ tới head node.
...
**Doubly linked list** có hai pointer: một `prev` trỏ tới node trước và một `next` trỏ tới node sau.
...
**Doubly circular linked list** có `next` của node cuối trỏ tới `head`, còn `prev` của `head` trỏ tới node cuối, tạo thành một vòng.
...
- Kích thước của array cố định, còn linked list hỗ trợ dynamic resizing một cách tự nhiên. Nếu array được khai báo quá nhỏ, phải cấp phát một vùng memory lớn hơn, rồi copy các element từ array cũ sang; thao tác này khá tốn thời gian!
...
![Minh họa cấu trúc LIFO của stack](https://oss.javaguide.cn/github/javaguide/cs-basics/data-structure/stack.png)

### 3.2. Các trường hợp sử dụng phổ biến của stack

Khi data chỉ cần được chèn và xóa ở một đầu, đồng thời có đặc tính **last in, first out (LIFO, Last In First Out)**, ta có thể sử dụng data structure stack.

#### 3.2.1. Triển khai chức năng quay lại và tiến tới của browser

Chỉ cần sử dụng hai stack (`Stack1` và `Stack2`) là có thể triển khai chức năng này. Ví dụ, bạn lần lượt xem bốn page 1,2,3,4, ta lần lượt push bốn page này vào `Stack1`. Khi muốn quay lại page 2, bạn nhấn nút quay lại; ta lần lượt pop hai page 4,3 khỏi `Stack1`, sau đó push chúng vào `Stack2`. Nếu lại muốn quay về page 3, bạn nhấn nút tiến tới; ta pop page 3 khỏi `Stack2`, sau đó push page này vào `Stack1`. Hình minh họa như sau:
...
#### 3.2.2. Kiểm tra các symbol có xuất hiện thành cặp hay không
...
1. Trước hết, lưu quy tắc tương ứng giữa các bracket vào `Map`; đây là điều hiển nhiên.
2. Tạo một stack. Duyệt string: nếu character là left bracket thì push trực tiếp vào `stack`; nếu không, so sánh element trên top của `stack` với bracket này. Nếu không bằng nhau thì trả về `false`. Sau khi duyệt xong, nếu `stack` rỗng thì trả về `true`.
...
Dưới đây, ta dùng array để triển khai một stack với các method cơ bản `push()`, `pop()` (trả về element trên top và pop khỏi stack), `peek()` (trả về element trên top nhưng không pop), `isEmpty()` và `size()`.
...
public class MyStack {
    private int[] storage;//array lưu các element của stack
...
Simple queue là queue thông thường; mỗi lần thêm element, element được thêm vào cuối queue. Simple queue được chia thành **sequential queue (triển khai bằng array)** và **linked queue (triển khai bằng linked list)**.
...
Circular queue có thể giải quyết vấn đề false overflow và out of bounds của sequential queue. Cách giải quyết là quay lại từ đầu, tạo thành vòng nối đầu với cuối; đây cũng là nguồn gốc tên gọi circular queue.

Vẫn dùng hình trên làm ví dụ: nếu trỏ pointer `rear` tới vị trí có array index bằng 0 thì sẽ không xảy ra out of bounds. Khi tiếp tục thêm element vào queue, `rear` sẽ di chuyển về phía sau.

![Circular queue](https://oss.javaguide.cn/github/javaguide/cs-basics/data-structure/circular-queue.png)

Trong sequential queue, queue rỗng khi `front==rear`; còn trong circular queue, trạng thái này cũng có thể là queue đầy như hình trên. Có hai cách giải quyết:
...
**Deque** là queue cho phép chèn và xóa ở cả hai đầu, linh hoạt hơn simple queue.

Thông thường, ta có thể thực hiện các thao tác `addFirst`, `addLast`, `removeFirst` và `removeLast` trên deque.

#### 4.2.4. Priority queue

**Priority queue**, xét về cấu trúc bên trong, không phải là linear data structure mà thường được triển khai bằng heap.

1. Mỗi khi một element được enqueue, priority queue sẽ chèn element mới vào heap và điều chỉnh heap.
...
Mặc dù priority queue thường được triển khai bằng cấu trúc heap không tuyến tính, nó cung cấp cho người dùng khả năng dequeue theo priority thông qua queue interface. “Priority” ở đây chỉ mô tả thứ tự dequeue, không có nghĩa là tất cả element trong collection sẽ tự động được sắp xếp.

### 4.3. Trường hợp sử dụng phổ biến của queue

Khi cần xử lý data theo một thứ tự nhất định, có thể cân nhắc dùng queue data structure.

- **Blocking queue:** Blocking queue có thể được xem là queue có thêm thao tác blocking. Khi queue rỗng, thao tác dequeue bị block; khi queue đầy, thao tác enqueue bị block. Sử dụng blocking queue giúp dễ dàng triển khai mô hình “producer - consumer”.
- **Request/task queue trong thread pool:** Khi thread pool không còn thread rảnh, các request cần thread resource cho task mới sẽ được xử lý thế nào? Các task này sẽ được đưa vào task queue, chờ đến khi thread trong thread pool rảnh rồi lấy ra thực thi. Task queue được chia thành unbounded queue (triển khai dựa trên linked list) và bounded queue (triển khai dựa trên array). Unbounded queue có capacity về lý thuyết không bị giới hạn; task có thể liên tục enqueue cho đến khi resource của system cạn kiệt. Ví dụ: blocking queue `LinkedBlockingQueue` mà `FixedThreadPool` sử dụng có capacity mặc định là `Integer.MAX_VALUE`, vì vậy có thể được xem là “unbounded queue”. Bounded queue thì khác: khi queue đầy, nếu tiếp tục submit task mới, do queue không thể chứa thêm task, thread pool sẽ reject các task này và throw exception `java.util.concurrent.RejectedExecutionException`.
- **Stack:** Deque có thể triển khai toàn bộ chức năng của stack (`push`, `pop` và `peek`), đồng thời các method liên quan đã được định nghĩa trong interface `Deque`. `Stack` chưa bị đánh dấu deprecated, nhưng là subclass của `Vector` từ thời kỳ đầu; tài liệu JDK khuyến nghị ưu tiên dùng `Deque` và implementation của nó (như `ArrayDeque`) để thực hiện thao tác stack.
- **Breadth-first search (BFS):** Trong quá trình breadth-first search trên graph, queue được dùng để lưu các node đang chờ truy cập, bảo đảm các node của graph được duyệt theo từng layer.
- Process queue của Linux kernel (xếp theo priority)
- Các bữa tiệc ngoài đời và playlist trên player;
- Message queue
- Và vân vân…

## Trọng tâm ôn tập phỏng vấn

Linear structure là nền tảng của các bài toán về algorithm và Java collection. Trong phỏng vấn, array, linked list, stack và queue thường được đặt cạnh nhau để so sánh.

| Cấu trúc    | Truy vấn          | Chèn/xóa                | Java type điển hình              | Dạng bài thường gặp                                         |
| ----------- | ----------------- | ----------------------- | -------------------------------- | ----------------------------------------------------------- |
| Array       | Theo index `O(1)` | Vị trí giữa `O(n)`      | Underlying array của `ArrayList` | Binary search, two pointers, prefix sum                     |
...
- Array có random access nhanh vì memory liên tục; có thể tính trực tiếp address từ base address và index.
