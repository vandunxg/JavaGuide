---
title: Giải thích chi tiết về heap (max heap, min heap, priority queue)
description: Phân tích tính chất và thao tác của heap, tìm hiểu cách triển khai priority queue và ưu thế về performance của heap sort, nắm được complexity của thao tác insert/delete và các trường hợp sử dụng thực tế.
...
Giá trị của mỗi node trong heap đều lớn hơn hoặc bằng (hoặc nhỏ hơn hoặc bằng) giá trị của tất cả node trong subtree. Nói cách khác, giá trị của bất kỳ node nào cũng lớn hơn hoặc bằng (hoặc nhỏ hơn hoặc bằng) giá trị của tất cả child node.

> Bạn có thể hiểu heap (max heap) như một công ty. Công ty này rất công bằng: ai có năng lực thì làm leader, không có chuyện người yếu lại làm leader; những người dưới quyền leader chắc chắn không thể giỏi hơn leader. Cách hiểu này giúp bạn nắm được các thao tác của heap ở phần sau.

**!!!Lưu ý đặc biệt:**

- Nhiều blog nói heap là complete binary tree, nhưng thực tế không phải vậy, **heap không nhất thiết là complete binary tree**. Chỉ vì thuận tiện cho việc lưu trữ và index, chúng ta thường biểu diễn heap dưới dạng complete binary tree. Trên thực tế, Fibonacci heap và binomial heap được nhiều người biết đến không phải là complete binary tree, thậm chí chúng còn không phải là binary tree.
...
Hình 1 và hình 2 là heap. Hình 1 là max heap, mỗi node đều lớn hơn tất cả node trong subtree. Hình 2 là min heap, mỗi node đều nhỏ hơn tất cả node trong subtree.

Hình 3 không phải heap. Trong hình 3, root node 1 nhỏ hơn 2 và 15, nhưng 15 lại lớn hơn 3, 19 lớn hơn 5, không thỏa mãn tính chất của heap.

## Công dụng của heap

Khi chỉ quan tâm đến max hoặc min trong toàn bộ dữ liệu, cần nhiều lần lấy max hoặc min, nhiều lần insert hoặc delete dữ liệu, bạn có thể sử dụng heap.

Có thể bạn sẽ nghĩ đến ordered array. Complexity khi khởi tạo ordered array là `O(nlog(n))`, complexity khi tìm max hoặc min đều là `O(1)`. Tuy nhiên, khi update (insert hoặc delete) dữ liệu, complexity là `O(n)`. Ngay cả khi dùng binary search có complexity `O(log(n))` để tìm vị trí cần insert hoặc delete, việc di chuyển dữ liệu vẫn cần complexity `O(n)`.

**So với ordered array, ưu thế chính của heap là hiệu quả insert và delete dữ liệu cao hơn.** Vì heap được triển khai dựa trên complete binary tree, khi insert và delete dữ liệu, chỉ cần di chuyển node lên xuống trong binary tree, complexity là `O(log(n))`, hiệu quả hơn `O(n)` của ordered array.

Tuy nhiên, cần lưu ý: dùng Floyd build-heap method, thực hiện sift down lần lượt từ non-leaf node cuối cùng, complexity là `O(n)`; nếu bắt đầu từ empty heap và insert lần lượt n phần tử, complexity là `O(nlogn)`.

## Phân loại heap

Heap được chia thành **max heap** và **min heap**. Điểm khác nhau giữa hai loại nằm ở cách sắp xếp node.

- **Max heap**: giá trị của mỗi node trong heap đều lớn hơn hoặc bằng giá trị của tất cả node trong subtree
- **Min heap**: giá trị của mỗi node trong heap đều nhỏ hơn hoặc bằng giá trị của tất cả node trong subtree
...
Ở phần giới thiệu về tree trước đó, chúng ta đã nói rằng nhờ các tính chất ưu việt của complete binary tree, dùng array để lưu binary tree vừa tiết kiệm không gian, vừa thuận tiện cho việc index (nếu số thứ tự của root node là 1, thì với mọi node i trong tree, số thứ tự của left child node là `2*i`, right child node là `2*i+1`).
...
> Insert element: một nhân viên mới gia nhập công ty, khi mới bắt đầu thì phải làm từ cấp cơ sở.
...
**2. Từ dưới lên trên, nếu parent node nhỏ hơn element đó thì đổi chỗ node và parent node, cho đến khi không thể đổi chỗ**
...
Theo tính chất của heap, heap top của max heap là phần tử lớn nhất trong tất cả phần tử, heap top của min heap là phần tử nhỏ nhất trong tất cả phần tử. Khi cần tìm max hoặc min nhiều lần, có thể dùng heap để thực hiện.
...
So sánh left child node và right child node của root node, tức các phần tử array có index 2 và 3, rồi đưa phần tử lớn hơn vào vị trí của root node (index 1).
...
Sau đó bắt đầu để viên đá này chìm xuống đáy biển, liên tục so sánh giá trị của nó với left child node và right child node, đổi chỗ với child node lớn hơn, cho đến khi không thể đổi chỗ.
...
- **Insert element**: trước tiên đặt element vào cuối array, sau đó heapify từ dưới lên để sift up element ở cuối
...
- Bước đầu tiên là build heap, biến một array không có thứ tự thành heap
- Bước thứ hai là sort, lấy heap top ra, sau đó heapify các element còn lại, lặp lại cho đến khi lấy hết tất cả element.

### Build heap

Nếu đã hiểu đủ về quy trình heapify, bạn sẽ dễ nắm được quy trình build heap hơn. Quy trình build heap chính là heapify từ trên xuống cho tất cả non-leaf node.

Trước tiên cần biết non-leaf node là những node nào. Parent node của node cuối cùng và các element trước nó đều là non-leaf node. Nói cách khác, nếu số node là n, chúng ta cần heapify từ trên xuống (đưa xuống đáy) các node từ n/2 đến 1.
...
Trừu tượng hóa array không có thứ tự ban đầu thành một tree. Số node trong hình là 6, nên node 4, 5, 6 là leaf node, node 1, 2, 3 là non-leaf node. Vì vậy cần heapify từ trên xuống các node 1-3. Lưu ý, thứ tự heapify là từ sau ra trước, bắt đầu từ node 3 cho đến node 1.
...
Vì heap top là phần tử lớn nhất trong tất cả phần tử, nên chỉ cần lặp lại việc lấy heap top, đặt heap top lớn nhất này vào cuối array, rồi heapify các element còn lại.
...
Trước tiên trả lời câu hỏi thứ nhất: cần thực hiện heapify từ trên xuống (đưa xuống đáy). Bước đầu của heapify này là di chuyển element cuối lên heap top, khi đó vị trí cuối sẽ bị bỏ trống. Vì số element trong heap đã giảm, vị trí này sẽ không được dùng lại, nên có thể đặt element đã lấy ra vào cuối.

Bạn đã nhận ra chưa, thực tế đây chính là một thao tác swap: đổi chỗ heap top và element cuối, từ đó gộp việc lấy heap top với bước đầu của heapify (đưa element cuối vào vị trí root node).
...
Không nên viết thành `b - a`, vì với giá trị integer cực trị có thể xảy ra overflow, dẫn đến kết quả so sánh sai.
...
Cần lưu ý, min heap là cách giải tổng quát để tìm phần tử lớn thứ K, nhưng không đáp ứng yêu cầu complexity của mọi bài toán. Ví dụ, [LeetCode 215. Phần tử lớn thứ K trong array](https://leetcode.cn/problems/kth-largest-element-in-an-array/) yêu cầu dùng thuật toán có time complexity `O(n)`, thông thường nên dùng quickselect random, time complexity kỳ vọng là `O(n)`; nếu yêu cầu nghiêm ngặt worst-case time complexity là `O(n)`, cần dùng thuật toán BFPRT. Có thể tham khảo so sánh các phương án cụ thể trong [Tổng hợp câu hỏi phỏng vấn về bài toán Top K](../algorithms/top-k.md#快排分区思路).
...
K phần tử có frequency cao nhất thường dùng “hash table đếm + min heap”:
...
## Minh họa quy trình và các mẫu biên

Khi duy trì min heap có size K, có thể hiểu heap là “candidate pool”:

```text
1. Candidate pool chưa đầy: đưa trực tiếp vào.
2. Candidate pool đã đầy, element mới <= heap top: không thể vào top K, bỏ qua.
...
Nên kiểm tra trước một lượt một số mẫu biên:

- `k == 1`: tìm max.
- `k == nums.length`: tìm min.
