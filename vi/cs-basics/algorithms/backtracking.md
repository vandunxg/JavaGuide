...
description: "Tổng hợp câu hỏi phỏng vấn về Backtracking, giải thích cách nhận diện dạng bài Backtracking, template combination, template permutation, template subset, loại bỏ trùng lặp, pruning, phân tích độ phức tạp và các bài LeetCode thường gặp."
...
Đặc điểm của bài Backtracking rất rõ ràng: đề bài yêu cầu bạn tìm tất cả phương án, tất cả đường đi, tất cả combination, hoặc thử các lựa chọn trong một tập hợp. Nó rất giống DFS, điểm khác biệt là Backtracking nhấn mạnh hơn vào “chọn -> đệ quy -> hủy lựa chọn”.

Khi viết Backtracking trong phỏng vấn, điều quan trọng nhất là trước tiên phải nói rõ ý nghĩa của hàm đệ quy. Khi ý nghĩa hàm đã chắc chắn, tham số, điều kiện kết thúc và thao tác hủy lựa chọn sẽ ít bị rối hơn.

## Trọng tâm phỏng vấn

- Có thể viết ba loại template: combination, permutation và subset.
- Có thể giải thích tác dụng của `path`, `startIndex`, `used`.
- Có thể phán đoán dựa trên đề bài xem có cần loại bỏ trùng lặp hay không.
- Có thể thực hiện pruning đơn giản để tránh tìm kiếm vô ích.
- Có thể nói rõ độ phức tạp liên quan đến quy mô kết quả.

## Suy nghĩ về bài Backtracking như thế nào?

Có thể bắt đầu bằng cách vẽ bài Backtracking thành một “cây lựa chọn”. Mỗi tầng của cây đại diện cho một lần lựa chọn, node gốc đại diện cho trạng thái chưa chọn, node lá đại diện cho một phương án hoàn chỉnh.
...
3. Điều kiện kết thúc là gì? Khi nào đưa `path` vào đáp án.
...
Bài combination không quan tâm đến thứ tự, vì vậy `[1, 2]` và `[2, 1]` là cùng một đáp án. Tác dụng của `start` là đảm bảo các lần chọn sau chỉ chọn những số nằm sau vị trí hiện tại, tránh trùng lặp.
...
Ý nghĩa là: nếu bắt đầu từ `i` mà số lượng số còn lại đã không đủ để tạo thành `k` số, thì không cần tiếp tục liệt kê.
...
Bài permutation quan tâm đến thứ tự, vì vậy ở mỗi tầng đều có thể chọn từ tất cả các số, chỉ là không thể sử dụng lại cùng một số. `used[i]` biểu thị `nums[i]` đã nằm trong path hiện tại hay chưa.

Nếu mảng có các số trùng nhau, việc loại bỏ permutation trùng lặp dễ viết sai hơn combination. Thông thường trước tiên sắp xếp mảng, sau đó ở cùng một tầng bỏ qua trường hợp “số trùng trước đó vẫn chưa được sử dụng”:
...
Câu lệnh này cố định thứ tự lựa chọn của các số trùng nhau ở cùng một tầng, tránh tạo ra permutation trùng lặp.
...
Bài subset rất giống bài combination, nhưng không chỉ thu thập đáp án khi đạt độ dài cố định, mà thu thập một lần ở mỗi node. Vì path có độ dài bất kỳ đều có thể là một subset.
...
- Với các bài như subset và combination, chọn theo chỉ số từ trước đến sau, bỏ qua phần tử trùng lặp ở cùng một tầng, chẳng hạn `i > start && nums[i] == nums[i - 1]`.
...
Lấy bài combination với `n = 3, k = 2` làm ví dụ, cây lựa chọn có thể đơn giản hóa như sau:

| Lựa chọn tầng thứ nhất | Có thể chọn ở tầng thứ hai | Kết quả tạo ra                   |
| ---------------------- | -------------------------- | -------------------------------- |
| Chọn 1                 | 2, 3                       | `[1, 2]`, `[1, 3]`               |
| Chọn 2                 | 3                          | `[2, 3]`                         |
...
| Có phần tử trùng lặp | Có sắp xếp trước và loại bỏ trùng lặp cùng tầng hay không |
...
`path` trong Backtracking là object được dùng lại, nếu không copy thì các list trong đáp án sẽ cùng bị thay đổi bởi các lần đệ quy phía sau.

## Điểm dễ sai

- Khi thêm vào đáp án phải copy `path`, không được đưa trực tiếp reference vào.
