...
- Có thể so sánh complexity của heap, phân vùng quicksort và đếm bằng bucket.
- Có thể xử lý tình huống data stream.
- Có thể viết comparator cho Java `PriorityQueue`.

## Chọn phương án cho bài Top K như thế nào?

Trước tiên, hãy xem 3 điều kiện:

1. Chỉ cần phần tử thứ K hay cần đầy đủ K phần tử đầu tiên?
2. Dữ liệu được cung cấp một lần hay đến liên tục dưới dạng data stream?
3. Có cần kết quả có thứ tự hay không?

Nếu chỉ tìm phần tử lớn thứ K trong một array được cho một lần, phân vùng quicksort trung bình nhanh hơn; nếu dữ liệu liên tục đến, duy trì một heap có kích thước K sẽ tự nhiên hơn; nếu đề bài hỏi K phần tử có tần suất cao nhất, trước tiên phải thống kê frequency, sau đó thực hiện Top K trên frequency.

## So sánh các phương án

| Phương án           | Tình huống phù hợp                                            | Time complexity          | Space complexity               |
| ------------------- | ------------------------------------------------------------- | ------------------------ | ------------------------------ |
| Sorting             | Dữ liệu không lớn, ưu tiên code đơn giản                      | `O(nlogn)`               | Tùy implementation của sorting |
| Min-heap            | Tìm K phần tử lớn nhất hoặc phần tử lớn thứ K                 | `O(nlogk)`               | `O(k)`                         |
| Phân vùng quicksort | Tìm phần tử lớn thứ K, hiệu quả trung bình cao                | Trung bình `O(n)`        | `O(1)` đến `O(logn)`           |
...
- Sorting đơn giản nhất, phù hợp khi dữ liệu không lớn hoặc không yêu cầu complexity tối ưu.
...
Heap luôn giữ lại K số lớn nhất hiện tại; đỉnh heap là phần tử nhỏ nhất trong K số này, cũng chính là phần tử lớn thứ K trong toàn bộ array.

Vì sao là min-heap? Vì cần giữ lại K phần tử lớn nhất trong heap. Khi phần tử mới được thêm vào, nếu kích thước heap vượt quá K thì phải loại bỏ phần tử nhỏ nhất trong K + 1 phần tử. Đỉnh của min-heap chính là giá trị nhỏ nhất.

Nếu tìm phần tử nhỏ thứ K, tư duy sẽ ngược lại: duy trì max-heap có kích thước K, khi vượt quá K thì loại bỏ phần tử lớn nhất.

## Giải thích chi tiết bài tiêu biểu: K phần tử có tần suất cao nhất

[347. K phần tử có tần suất cao nhất](https://leetcode.cn/problems/top-k-frequent-elements/) là bài toán về frequency phổ biến nhất trong nhóm Top K. Đề bài cho một `integer array` và một số nguyên `k`, yêu cầu trả về `k` phần tử có frequency cao nhất, thứ tự kết quả thường không quan trọng.

Không nên sorting trực tiếp array ban đầu trong bài này, vì cần so sánh “frequency”, không phải giá trị phần tử. Cách làm chắc chắn hơn gồm hai bước:
...
Vì sao vẫn dùng min-heap? Vì sau khi heap đầy, khi phần tử mới được thêm vào, chỉ cần kích thước heap vượt quá `k` thì lấy phần tử có frequency thấp nhất hiện tại ra. Như vậy sau khi duyệt hết các phần tử khác nhau, các phần tử còn lại trong heap chính là `k` phần tử có frequency cao nhất.
...
Lấy `nums = [1,1,1,2,2,3]`, `k = 2` làm ví dụ, bảng frequency là `{1=3, 2=2, 3=1}`. Đưa lần lượt `1` và `2` vào heap; sau đó khi đưa `3` vào, kích thước vượt quá 2 nên phần tử có frequency thấp nhất là `3` bị lấy ra, cuối cùng giữ lại `1` và `2`.

Nếu `k` bằng số phần tử khác nhau, cuối cùng heap sẽ giữ lại toàn bộ phần tử; nếu interviewer yêu cầu output theo thứ tự frequency giảm dần thì vẫn cần sorting kết quả thêm một lần.

Nếu interviewer yêu cầu khi frequency bằng nhau thì sắp xếp theo giá trị phần tử hoặc thứ tự từ điển, comparator phải thêm quy tắc sorting thứ hai. Ví dụ, bài K từ có tần suất cao nhất thường yêu cầu từ có frequency cao hơn đứng trước, khi frequency bằng nhau thì từ đứng trước theo thứ tự từ điển.

## Tư duy phân vùng quicksort

Phân vùng quicksort phù hợp để tìm phần tử lớn thứ K khi không yêu cầu K phần tử đầu tiên được sắp xếp. Tư duy là mỗi lần chia array thành hai phía theo pivot, rồi quyết định tiếp tục tìm ở phía nào dựa trên thứ hạng của pivot. Time complexity trung bình là `O(n)`, nhưng trong trường hợp xấu nhất có thể suy biến thành `O(n^2)`; cách viết thực tế thường chọn pivot ngẫu nhiên.

Ưu điểm của phân vùng quicksort là không cần duy trì heap, time complexity trung bình thấp; hạn chế là phù hợp hơn với dữ liệu nằm trong memory và được cung cấp một lần. Nếu data stream liên tục đến hoặc dữ liệu quá lớn không thể đưa toàn bộ vào memory, phương án heap dễ triển khai hơn.

## Tình huống data stream

Với bài toán data stream, không thể sorting lại mỗi khi có phần tử mới. Cách làm thường gặp là liên tục duy trì một data structure:
...
Lấy array `[3, 2, 1, 5, 6, 4]` để tìm phần tử lớn thứ 2 làm ví dụ, duy trì min-heap có kích thước 2. Trong bảng, để dễ đọc, các phần tử trong heap được hiển thị theo thứ tự tăng dần của giá trị, không đại diện cho thứ tự trong internal array của Java `PriorityQueue`.
...
Comparator này có thể xảy ra overflow với các giá trị integer ở biên. Cách viết an toàn hơn là:
...
- Nếu cần output có thứ tự, sau khi dùng heap hoặc phân vùng quicksort vẫn cần sorting bổ sung.
