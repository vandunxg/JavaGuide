---
title: "Tổng hợp câu hỏi phỏng vấn về two pointers và sliding window: template thường gặp cho array, linked list, string"
description: Tổng hợp câu hỏi phỏng vấn về two pointers và sliding window, giải thích left/right pointer, fast/slow pointer, read/write pointer, fixed window, variable window, Java template và các bài LeetCode thường gặp.
...
Two pointers là cách tiếp cận rộng hơn: chỉ cần dùng hai pointer phối hợp tiến lên thì đều có thể gọi là two pointers. Sliding window cụ thể hơn, nó duy trì một đoạn liên tiếp `[left, right]`; trong window thường có một nhóm state, chẳng hạn character count, element sum, maximum hoặc matching count.

| Đặc điểm bài toán                                                      | Thường dùng        |
| ---------------------------------------------------------------------- | ------------------ |
...
| Tìm subarray/substring liên tiếp dài nhất, ngắn nhất hoặc đếm số lượng | Sliding window     |
...
Nếu array chưa được sắp xếp, thông thường hãy sort trước rồi dùng left/right pointer. Sau khi sort, cần nhớ time complexity trở thành `O(nlogn)`.
...
Three sum cũng theo cùng một cách, chỉ khác là cố định trước một số rồi thực hiện two sum trong phần còn lại. Điểm khó là loại duplicate: cần bỏ qua giá trị trùng ở số cố định, và sau khi left/right pointer tìm thấy đáp án cũng phải bỏ qua các giá trị trùng nhau.
...
Điểm mấu chốt của bài linked list không phải code dài hay ngắn, mà là ý nghĩa của pointer phải nhất quán. Thứ tự của `fast != null && fast.next != null` cũng không được đảo ngược.

Fast/slow pointer thường có hai loại chênh lệch tốc độ:

- `fast` đi 2 bước mỗi lần, `slow` đi 1 bước mỗi lần: dùng để phát hiện cycle và tìm middle của linked list.
- Một pointer đi trước `k` bước, sau đó hai pointer cùng đi: dùng để tìm node thứ `k` từ cuối.

Khi tìm node thứ `k` từ cuối, khoảng cách giữa hai pointer luôn là `k` node. Khi pointer phía trước đi đến cuối linked list, pointer phía sau vừa dừng ở vị trí mục tiêu. Khi xóa node thứ `N` từ cuối, thông thường sẽ thêm dummy head để tránh phải xử lý riêng trường hợp xóa head node.

## Read/write pointer

Read/write pointer thường được dùng để chỉnh sửa array tại chỗ:
...
`read` chịu trách nhiệm scan array gốc, còn `write` trỏ tới vị trí tiếp theo có thể ghi. Khi phỏng vấn, tốt nhất hãy nói rõ ý nghĩa của hai biến này trước.

Điểm cốt lõi của read/write pointer là “đọc hết array, chỉ ghi các phần cần giữ lại về phía trước”. Dạng bài này thường yêu cầu chỉnh sửa tại chỗ và trả về length mới, thay vì tạo array mới.
...
Lấy bài “substring dài nhất không có ký tự lặp” làm ví dụ:
...
2. Khi window không thỏa điều kiện, liên tục di chuyển left pointer và đồng thời cập nhật state.
3. Khi window thỏa yêu cầu bài toán, cập nhật đáp án.

Thời điểm cập nhật của bài toán tìm longest và shortest không giống nhau:

- Tìm window hợp lệ dài nhất: thường cập nhật đáp án sau khi window trở nên hợp lệ.
- Tìm window ngắn nhất thỏa điều kiện: thường cập nhật đáp án khi window đã thỏa điều kiện, sau đó tiếp tục thu hẹp left boundary.

Ví dụ, trong bài “minimum window substring”, ngay khi window bao phủ đủ character mục tiêu, cần cập nhật đáp án trước rồi mới thử thu nhỏ window; trong bài “longest substring without repeating characters”, khi window có duplicate character, cần thu hẹp về trạng thái hợp lệ trước rồi mới cập nhật đáp án.
...
Điểm cốt lõi của fixed window là thêm một element ở bên phải và loại một element ở bên trái.
...
- Loại bỏ element cũ rời khỏi window.
- Cập nhật đáp án của window hiện tại.

Nếu cần duy trì maximum hoặc minimum trong window, biến thông thường là chưa đủ; thường phải dùng monotonic queue. Ví dụ, trong bài “sliding window maximum”, queue lưu các index có khả năng trở thành maximum, còn phần tử đầu queue là maximum của window hiện tại.

## Cách viết khi phỏng vấn

Với bài two pointers và sliding window, lỗi dễ mắc nhất khi phỏng vấn là làm thay đổi ý nghĩa của pointer giữa chừng. Nên viết theo thứ tự sau:

1. Xác định dạng bài trước: thu hẹp hai đầu, fast/slow đuổi nhau, ghi đè tại chỗ hay window liên tiếp.
...
5. Xác định thời điểm cập nhật đáp án: sau khi hợp lệ thì cập nhật longest, khi thỏa điều kiện thì cập nhật shortest.
...
[76. Minimum Window Substring](https://leetcode.cn/problems/minimum-window-substring/) là một trong những bài đòi hỏi xử lý nhiều chi tiết nhất về sliding window. Bài toán yêu cầu tìm substring ngắn nhất trong `s` sao cho bao phủ tất cả character và số lần xuất hiện tương ứng trong `t`.
...
- `valid--` phải xảy ra trước khi giảm `window[out]`, vì lúc đó window vẫn vừa đủ thỏa điều kiện.
- Cập nhật đáp án phải đặt bên trong `while (valid == need.size())`, vì chỉ khi window hiện tại đã bao phủ `t` thì nó mới đủ điều kiện để so sánh với đáp án ngắn nhất.

## Minh họa quá trình và các trường hợp biên

Lấy “longest substring without repeating characters” làm ví dụ, diễn biến của window trong string `abba` như sau:

| Character tại right pointer | Window sau khi thêm | Hợp lệ không | Left pointer di chuyển thế nào                      | Longest hiện tại |
| --------------------------- | ------------------- | ------------ | --------------------------------------------------- | ---------------- |
...
Với sliding window, ít nhất nên kiểm tra các trường hợp biên sau:

| Input                         | Điểm trọng tâm                                  |
| ----------------------------- | ----------------------------------------------- |
| Empty string hoặc empty array | Có trả về 0 ngay không                          |
| Mọi character đều giống nhau  | Left boundary có liên tục thu hẹp không         |
| Không có duplicate character  | Đáp án có thể cập nhật tới toàn bộ độ dài không |
...
Khi thu hẹp variable window, thông thường phải dùng `while` cho tới khi window trở lại thỏa điều kiện. Chỉ di chuyển một lần sẽ dễ sai với các input như `abba`, `aaabc`.
...
- [26. Xóa phần tử trùng trong sorted array](https://leetcode.cn/problems/remove-duplicates-from-sorted-array/)
- [15. Three Sum](https://leetcode.cn/problems/3sum/)
- [141. Cycle trong linked list](https://leetcode.cn/problems/linked-list-cycle/)
- [3. Substring dài nhất không có ký tự lặp](https://leetcode.cn/problems/longest-substring-without-repeating-characters/)
- [76. Substring ngắn nhất bao phủ](https://leetcode.cn/problems/minimum-window-substring/)
