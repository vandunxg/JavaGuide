...
_Năng lực cá nhân có hạn. Nếu bài viết còn điểm nào cần bổ sung/hoàn thiện/chỉnh sửa, hoan nghênh bạn chỉ ra trong phần bình luận để cùng tiến bộ!_

## Lời nói đầu

I/O luôn là một kiến thức khó hiểu đối với nhiều bạn, trong bài viết này tôi sẽ trình bày những gì mình hiểu về I/O, hy vọng có thể giúp ích cho bạn.
...
Thiết bị input (chẳng hạn bàn phím) và thiết bị output (chẳng hạn màn hình) đều là thiết bị bên ngoài. Card mạng và ổ cứng vừa có thể là thiết bị input, vừa có thể là thiết bị output.
...
Các application chúng ta thường chạy đều hoạt động trong user space, chỉ kernel space mới có thể thực hiện các thao tác tài nguyên ở system mode, chẳng hạn quản lý file, giao tiếp giữa các process, quản lý memory, v.v. Nói cách khác, muốn thực hiện thao tác IO, nhất định phải dựa vào năng lực của kernel space.

Ngoài ra, chương trình trong user space không thể trực tiếp truy cập kernel space.

Khi muốn thực hiện thao tác IO, do không có quyền thực hiện các thao tác này, chương trình chỉ có thể gửi system call để yêu cầu hệ điều hành hoàn tất.

Vì vậy, nếu user process muốn thực hiện thao tác IO thì phải thông qua **system call** để gián tiếp truy cập kernel space.

Trong quá trình phát triển hằng ngày, chúng ta tiếp xúc nhiều nhất với **disk IO (đọc và ghi file)** và **network IO (request và response mạng)**.

**Xét từ góc độ application, application của chúng ta gửi lời gọi IO (system call) đến kernel của hệ điều hành, còn hệ điều hành chịu trách nhiệm để kernel thực hiện thao tác IO cụ thể. Nói cách khác, application thực tế chỉ gửi lời gọi thực hiện thao tác IO; việc thực thi IO cụ thể do kernel của hệ điều hành hoàn tất.**

Lấy thao tác đọc truyền thống làm ví dụ, sau khi application gửi lời gọi I/O, thường sẽ trải qua hai bước:
...
Trong mô hình synchronous blocking IO, sau khi application gọi read, nó sẽ bị block liên tục cho đến khi kernel copy dữ liệu vào user space.
...
NIO trong Java được giới thiệu từ Java 1.4, tương ứng với package `java.nio`, cung cấp các abstraction như `Channel`, `Selector`, `Buffer`. Chữ N trong NIO là New. NIO đồng thời cung cấp channel blocking và non-blocking; trong đó selectable channel (`SelectableChannel`) có thể được cấu hình ở non-blocking mode. Đây là phương thức thao tác I/O hỗ trợ buffer-oriented và channel-based.

Trong Java NIO, phương thức lập trình network dựa trên `SelectableChannel` non-blocking và `Selector` tương ứng với **mô hình I/O multiplexing**. Tuy nhiên, không thể đồng nhất toàn bộ package `java.nio` với I/O multiplexing, vì package này còn bao gồm file I/O, blocking channel và các API khác.

Hãy tiếp tục theo dõi, bạn sẽ tìm được câu trả lời!
...
> Với synchronous non-blocking IO, khi thực hiện một lời gọi read, nếu dữ liệu chưa sẵn sàng thì application có thể không block để chờ mà chuyển sang thực hiện một số tác vụ tính toán nhỏ, sau đó nhanh chóng quay lại tiếp tục thực hiện lời gọi read, tức là polling. Việc
> polling này không diễn ra liên tục mà có khoảng ngắt; tận dụng khoảng ngắt đó chính là điểm khiến synchronous non-blocking IO hiệu quả hơn synchronous blocking IO.

Tuy nhiên, mô hình IO này cũng tồn tại vấn đề: **quá trình application liên tục thực hiện system call I/O để polling xem dữ liệu đã sẵn sàng hay chưa tiêu tốn rất nhiều tài nguyên CPU.**
...
Trong mô hình I/O multiplexing, thread trước tiên thực hiện lời gọi select để hỏi kernel xem dữ liệu đã sẵn sàng hay chưa. Sau khi kernel chuẩn bị xong dữ liệu, user thread mới thực hiện lời gọi read. Quá trình gọi read (dữ liệu từ kernel space -> user space) vẫn là blocking.

> Các system call hiện hỗ trợ I/O multiplexing gồm select, epoll, v.v. System call select hiện được gần như mọi hệ điều hành hỗ trợ.
>
...
> - **Lời gọi epoll**: thuộc kernel Linux 2.6, là phiên bản cải tiến của lời gọi select, tối ưu hiệu suất thực thi IO.
...
AIO thường chỉ asynchronous channel API do NIO.2 được giới thiệu trong Java 7. Ngoài asynchronous channel, NIO.2 còn bao gồm API file system mới.
...
Cuối cùng là một hình ảnh tóm tắt đơn giản về BIO, NIO và AIO trong Java.
...
- Bạn biết bao nhiêu về mô hình IO | Phần lý thuyết: <https://www.cnblogs.com/sheng-jie/p/how-much-you-know-about-io-models.html>
