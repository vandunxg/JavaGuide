---
title: "Tổng hợp các design pattern trong Java IO"
description: "Phân tích chuyên sâu các design pattern trong Java IO: giải thích chi tiết ứng dụng Decorator trong BufferedInputStream, cách triển khai Adapter bằng InputStreamReader, thiết kế Template Method của InputStream, tìm hiểu kiến trúc thư viện class Java IO."
category: Java
tag:
  - Java IO
  - Java Basics
...
Nếu `InputStream` có ít subclass thì làm như vậy không có vấn đề gì. Tuy nhiên, `InputStream` có quá nhiều subclass và quan hệ inheritance cũng quá phức tạp. Nếu tùy chỉnh một buffered input stream tương ứng cho từng subclass thì chẳng phải sẽ rất phiền phức sao?
...
Có rất nhiều ví dụ về việc áp dụng Decorator pattern trong IO stream, không cần cố ghi nhớ riêng! Sau khi nắm được cốt lõi của Decorator pattern, khi sử dụng bạn sẽ tự nhiên nhận ra những nơi nào đã áp dụng Decorator pattern.
