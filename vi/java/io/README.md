...
description: "Lộ trình học Java IO và NIO, bao quát BIO, NIO, AIO, blocking/non-blocking, synchronous/asynchronous, I/O multiplexing, mô hình Reactor và các design pattern cho IO."
category: Java
tag:
  - Java
  - Java IO
...
- Người đang chuẩn bị các câu hỏi phỏng vấn liên quan đến BIO, NIO, AIO, I/O multiplexing và Reactor.
- Người muốn tiếp tục học các network communication framework như Netty, RPC, message queue và database driver.
- Engineer dễ nhầm lẫn giữa các khái niệm blocking/non-blocking, synchronous/asynchronous, Selector, Channel và Buffer.

## Trọng tâm học

- Hệ thống IO stream của Java, byte stream, character stream, buffered stream và các thao tác file thường gặp.
- Ứng dụng của các design pattern như Decorator pattern, Adapter pattern trong IO.
- Khác biệt về model, use case và ưu/nhược điểm của BIO, NIO, AIO.
...
2. [Tổng hợp design pattern trong Java IO](./io-design-patterns.md): hiểu cách Decorator pattern, Adapter pattern và các design pattern khác được áp dụng vào Java IO API.
3. [Giải thích chi tiết Java IO model](./io-model.md): làm rõ BIO, NIO, AIO, synchronous/asynchronous, blocking/non-blocking và multiplexing.
...
- Vì sao Java IO sử dụng Decorator pattern với tần suất cao?
...
- Trong NIO, Buffer, Channel và Selector lần lượt đảm nhiệm trách nhiệm gì?
- Vì sao Netty được xây dựng dựa trên NIO thay vì sử dụng trực tiếp BIO truyền thống?

## Chuyên đề liên quan

- [Hệ thống kiến thức Java](../)
- [Chuyên đề lập trình Java concurrency](../concurrent/)
- [Chuyên đề JVM](../jvm/)
