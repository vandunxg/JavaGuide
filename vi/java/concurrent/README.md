...
description: "Lộ trình học và câu hỏi phỏng vấn Java Concurrency, bao quát thread, lock, synchronized, ReentrantLock, JMM, CAS, AQS, ThreadLocal, thread pool, CompletableFuture và virtual thread."
category: Java
tag:
  - Java
  - Java Concurrency
...
Java Concurrency là một trong những module quan trọng nhất và cũng dễ gây nhầm lẫn nhất trong phát triển backend và phỏng vấn. Học concurrency không thể chỉ học thuộc API, mà cần đặt vòng đời thread, cơ chế lock, memory model, atomic operation, thread pool và các lớp công cụ concurrency trên cùng một mạch để hiểu.

## Dành cho ai

- Backend developer muốn học Java Concurrency một cách có hệ thống.
- Bạn đọc đang chuẩn bị các câu hỏi phỏng vấn về thread, lock, JMM, CAS, AQS, thread pool, v.v.
- Bạn đọc đã sử dụng multithreading trong project nhưng chưa nắm chắc các vấn đề như deadlock, tham số thread pool, rò rỉ ThreadLocal.
- Engineer muốn hiểu năng lực thực tiễn khi làm việc với concurrent collections, CompletableFuture, virtual thread.

## Trọng tâm học

- Tạo thread, vòng đời, context switch, thread safety và các vấn đề concurrency thường gặp.
- Phạm vi áp dụng của `synchronized`, `volatile`, `ReentrantLock`, mutual exclusion lock, read-write lock, optimistic lock, pessimistic lock.
- JMM, happens-before, instruction reordering, visibility, atomicity và ordering.
- Tư duy bên trong của CAS, atomic class Atomic, AQS, concurrent collections và blocking queue.
- Core parameter, rejection policy, task queue, cấu hình tham số và thực tiễn production của thread pool.
- Cách sử dụng và rủi ro của CompletableFuture, ThreadLocal, virtual thread trong project thực tế.

## Thứ tự đọc đề xuất

1. [Tổng hợp câu hỏi phỏng vấn Java Concurrency thường gặp (phần 1)](./java-concurrent-questions-01.md): Trước tiên xây dựng danh sách các vấn đề cơ bản về thread, lock và thread safety.
...
3. [Giải thích chi tiết về Java lock](./java-lock.md), [Giải thích chi tiết về optimistic lock và pessimistic lock](./optimistic-lock-and-pessimistic-lock.md), [Giải thích chi tiết về CAS](./cas.md), [Giải thích chi tiết về JMM (Java Memory Model)](./jmm.md): Trước tiên xây dựng hệ thống lock, sau đó hiểu semantic bên trong của concurrency control.
...
5. [Giải thích chi tiết về Java thread pool](./java-thread-pool-summary.md) và [Best practice cho Java thread pool](./java-thread-pool-best-practices.md): Nắm vững hạ tầng concurrency cơ bản được sử dụng phổ biến nhất trong production.
...
- [Giải thích chi tiết về Java lock](./java-lock.md): Từ mutual exclusion lock, read-write lock, spin lock đến `synchronized`, `ReentrantLock`, AQS, xây dựng hệ thống Java lock.
...
### Công cụ concurrency và thực tiễn engineering

- [Giải thích chi tiết về Java thread pool](./java-thread-pool-summary.md): Hiểu core parameter, task queue, rejection policy và execution flow.
- [Best practice cho Java thread pool](./java-thread-pool-best-practices.md): Tổng hợp các đề xuất về thread pool isolation, cấu hình tham số và monitoring trong production environment.
- [Tổng hợp các concurrent collections thường gặp của Java](./java-concurrent-collections.md): Hệ thống hóa các collection như ConcurrentHashMap, CopyOnWriteArrayList, BlockingQueue.
- [Tổng hợp các atomic class](./atomic-classes.md): Hiểu việc atomic update primitive type, array, reference và field.
...
- [Tổng hợp các câu hỏi thường gặp về virtual thread](./virtual-thread.md): Hiểu định vị, trường hợp sử dụng và giới hạn của virtual thread.

## Câu hỏi thường gặp

- Thread và process khác nhau như thế nào? Thread có những state nào?
...
- Cấu hình core parameter của thread pool như thế nào? Chọn rejection policy ra sao?
...
- [Operating System](../../cs-basics/operating-system/)
