---
title: "Tổng hợp câu hỏi phỏng vấn JVM mới nhất năm 2026: runtime data area, class loading, garbage collection và điều tra sự cố production"
category: Java
description: "Tổng hợp câu hỏi phỏng vấn JVM mới nhất năm 2026, bao quát runtime data area, object creation, class file và class loading, thuật toán và collector của garbage collection, tham số JVM, công cụ chẩn đoán JDK, OOM, Full GC thường xuyên và CPU tăng vọt cùng các trọng tâm thường gặp khác."
tag:
  - Java
  - JVM
  - Câu hỏi phỏng vấn
...
Phỏng vấn JVM hiếm khi chỉ dừng ở câu hỏi “heap và stack khác nhau thế nào”. Sau khi trả lời về vùng nhớ, interviewer thường hỏi tiếp object được cấp phát thế nào, object nào có thể được thu hồi, vì sao một lần GC lại gây pause, cũng như cách điều tra khi hệ thống production gặp OOM, Full GC thường xuyên hoặc CPU tăng vọt.

Bài viết này là điểm bắt đầu ôn tập phỏng vấn cho chuyên đề JVM của JavaGuide. Các câu hỏi được chia thành năm phần: memory và object, class file và class loading, garbage collection, tham số và công cụ chẩn đoán, điều tra sự cố production. Câu trả lời đầy đủ cho từng câu hỏi nằm trong bài viết chuyên đề tương ứng.
...
| Class file và class loading | File `.class` đi vào JVM thế nào, class hoàn tất initialization khi nào                 | constant pool, quá trình class loading, class loader, parent delegation, class isolation |
...
| Tham số và công cụ          | Dựa vào JDK, collector và hiện tượng runtime để chọn parameter thế nào                  | `-Xms`, `-Xmx`, GC log, Heap Dump, jstat, jstack, JFR                                    |
| Điều tra production         | Sau khi nhận cảnh báo, giữ lại bằng chứng, thu hẹp phạm vi và xác nhận việc sửa thế nào | CPU, Load, OOM, memory leak, GC thường xuyên, thread bị block                            |

Hầu hết object instance được cấp phát trên heap; thread đang hoạt động, static field và các vị trí khác có thể giữ reference đến object. GC bắt đầu từ GC Roots để xác định object có thể reach được hay không, sau đó collector cụ thể thực hiện việc thu hồi. Khi ứng dụng tăng memory hoặc bị pause, còn phải kết hợp GC log, thread stack, heap dump và system metrics để phán đoán nguyên nhân, không thể thấy Full GC rồi lập tức tăng kích thước heap.

## JVM memory area và object

Trả lời về runtime data area chỉ nêu tên là chưa đủ. Những thread nào chia sẻ từng area, area lưu trữ gì, vòng đời ra sao và có thể ném exception nào đều có thể trở thành câu hỏi tiếp theo. Method area là vùng logic do đặc tả JVM định nghĩa; permanent generation và metaspace là các implementation của HotSpot trong những version khác nhau. Khi trả lời, không nên gộp chúng thành cùng một khái niệm.

Nội dung liên quan: [Giải thích chi tiết Java memory area](./memory-area.md)
...
- Stack frame của một Java method thay đổi thế nào từ lúc method được gọi đến khi trả về?
- `StackOverflowError` và `OutOfMemoryError` lần lượt có thể phát sinh thế nào?
- Java heap chủ yếu lưu trữ gì? Có phải mọi object đều chắc chắn được cấp phát trên heap không?
...
- Những trường hợp nào kích hoạt initialization của class? Truy cập compile-time constant có kích hoạt không?
- `<clinit>` và constructor `<init>` khác nhau thế nào?
- Bootstrap class loader, platform class loader và application class loader lần lượt load những class nào? Extension class loader của JDK 8 khác gì?
- Parent delegation model là gì? Nó tránh việc core class bị load hoặc thay thế nhiều lần thế nào?
- Parent delegation được thực thi ra sao? Khi parent loader không thể hoàn tất việc load thì chuyện gì xảy ra?
...
Trả lời về parent delegation chỉ như một chain tìm kiếm cố định hướng lên trên là chưa đầy đủ. Identity của class được xác định đồng thời bởi fully qualified name và class loader đã load nó. Các tình huống như SPI, class isolation của application server và hot deploy sử dụng những cách load khác nhau, mục đích cũng không hoàn toàn giống nhau.

## Garbage collection

Câu hỏi về garbage collection cần đi từ việc cấp phát object và xác định object còn sống đến collector cụ thể. Throughput, pause time và memory usage có sự đánh đổi; khi chọn collector còn phải cân nhắc JDK version, kích thước heap và yêu cầu latency theo nghiệp vụ, không thể chỉ so sánh tên thuật toán.

Nội dung liên quan: [Giải thích chi tiết JVM garbage collection](./jvm-garbage-collection.md)

Câu hỏi phỏng vấn thường gặp:

- Young generation và old generation thường được phân chia thế nào? Object thường được cấp phát và promote ra sao?
- Object lớn và object sống lâu sẽ đi vào old generation thế nào?
...
- Vì sao generational collection cần chọn thuật toán khác nhau cho young generation và old generation?
...
- ZGC giảm các pause dài như thế nào? Collector có pause thấp phải đánh đổi những gì?
- Những tình huống nào có thể kích hoạt Full GC? Khi Full GC xảy ra thường xuyên, nên bắt đầu điều tra từ dữ liệu nào?

Tên event, phân vùng memory và parameter trong GC log thay đổi theo JDK và collector. Khi phỏng vấn, có thể nêu runtime environment của mình trước, sau đó giải thích nguyên nhân kích hoạt một lần collection, phạm vi collection, phase pause và kết quả. Học thuộc kết luận cố định mà không xét version rất dễ dẫn đến mâu thuẫn khi bị hỏi tiếp.

## Tham số JVM và công cụ chẩn đoán

Câu hỏi về parameter kiểm tra cơ sở để đưa ra cấu hình. Kích thước heap, tỷ lệ young generation, collector và parameter ghi log cần được thảo luận cùng memory khi deploy, tốc độ cấp phát object, mục tiêu latency và JDK version. Đặc biệt với G1, thông thường không nên sao chép nguyên xi cấu hình cũ lấy kích thước young generation cố định làm trung tâm.
...
- Vì sao ứng dụng phía server thường đặt `-Xms` và `-Xmx` bằng nhau?
- `-XX:MetaspaceSize` và `-XX:MaxMetaspaceSize` khác nhau thế nào?
- Chọn garbage collector cho ứng dụng thế nào? Trước khi đổi collector cần thu thập dữ liệu gì?
- Cấu hình GC log trong JDK 8 và JDK 9 trở đi thế nào?
- Khi xảy ra OOM, làm thế nào để tự động tạo Heap Dump? Vì sao cần xác nhận dung lượng lưu trữ trước?
- `jps`, `jstat`, `jinfo`, `jmap`, `jstack` và `jcmd` lần lượt phù hợp để kiểm tra gì?
- Dùng `jstat` để theo dõi tần suất GC, capacity của từng area và tình trạng object được promote như thế nào?
...
Việc tạo Heap Dump, class histogram hoặc sampling tần suất cao đều có thể làm tăng tải trên production; Dump file cũng có thể chứa business data. Trước khi thực thi lệnh chẩn đoán, cần xác nhận phạm vi ảnh hưởng, dung lượng đĩa và vị trí lưu file. Trước khi restart khẩn cấp, ít nhất phải giữ lại thời điểm cảnh báo, các metric quan trọng, thread stack và GC information cần thiết.

## Điều tra sự cố JVM trong production

Điều tra production thường không có câu trả lời kiểu “một command có thể định vị ngay”. Sau khi nhận cảnh báo, trước hết kiểm tra thời gian, phạm vi ảnh hưởng, thay đổi gần đây và tài nguyên hệ thống, rồi quyết định thu thập thread stack, GC log hay heap dump. Nếu service đã ảnh hưởng đến user, cần đồng thời sắp xếp việc giảm tác động và giữ bằng chứng; không thể vì muốn thu thập đầy đủ bằng chứng mà để sự cố tiếp tục lan rộng.
...
- Khi memory liên tục tăng, làm thế nào xác định đó là business cache tăng, object được cấp phát quá nhanh hay memory leak?
...
- Khi Full GC xảy ra thường xuyên, quan sát old generation tăng, object được promote và hiệu quả collection thế nào?
...
Kết quả điều tra cần hình thành được chuỗi bằng chứng. Ví dụ khi CPU tăng vọt, trước tiên tìm Java process và thread có CPU cao, chuyển thread ID sang format dùng trong thread dump, rồi kiểm tra nhiều thread stack có liên tục nằm ở cùng một đoạn code hay không. Với vấn đề memory, cần kết hợp GC log, object histogram, Heap Dump và native memory; chỉ xem heap usage của JVM thì không thể bao quát mọi trường hợp OOM.
...
| 3～7 ngày         | Bổ sung JVM parameter, GC log và công cụ chẩn đoán JDK; lần lượt trình bày bằng lời quy trình điều tra CPU tăng vọt, OOM và Full GC thường xuyên                                                                         | Chọn được metric và tool từ hiện tượng cảnh báo, đồng thời nêu được rủi ro thao tác            |
| Hơn 1 tuần        | Đọc toàn bộ bài chuyên đề JVM, phân tích một lần GC log, thread dump hoặc Heap Dump trên máy local hay môi trường test; kết hợp với project để ghi lại JDK, collector, cấu hình heap và runtime metric thực tế           | Kết nối được nguyên lý, cấu hình, hiện tượng runtime và phương pháp xác minh                   |

Các vị trí dành cho ứng viên đã có kinh nghiệm và vị trí cấp trung, cấp cao thường tiếp tục hỏi về cơ sở chọn parameter và bằng chứng điều tra. Nếu chưa từng trực tiếp xử lý sự cố JVM production, có thể trả lời dựa trên quan sát ở môi trường test và tài liệu chuyên đề, nêu rõ mình sẽ thu thập gì trước và thu hẹp phạm vi thế nào; không nên biến case học tập thành sự cố production từng trải qua.
