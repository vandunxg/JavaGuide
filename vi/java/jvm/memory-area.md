...
description: "Giải thích chi tiết JVM Memory Area: phân tích chuyên sâu Java runtime data area (heap, method area, Java Virtual Machine Stack, native method stack, program counter), quy trình tạo object, chiến lược cấp phát bộ nhớ và cách định vị object khi truy cập."
category: Java
tag:
  - JVM
head:
...
Direct memory thường được thảo luận cùng các vùng này, nhưng nó không phải runtime data area do _Java Virtual Machine Specification_ định nghĩa, cũng không nên được xem là “thread-shared runtime data area” trong specification.
...
- **`StackOverflowError`:** khi stack space cần cho thread execution vượt quá kích thước JVM cho phép, ném lỗi `StackOverflowError`.
- **`OutOfMemoryError`:** nếu stack có thể mở rộng động, khi JVM mở rộng stack nhưng không thể xin đủ bộ nhớ thì ném exception `OutOfMemoryError`.
...
Khi thực thi native method, implementation của JVM có thể dùng native stack truyền thống (thường gọi là C stack). _Java Virtual Machine Specification_ không quy định native method stack phải sử dụng local variable table, operand stack và cấu trúc dynamic linking giống stack frame của JVM.
...
> **🐛 Đính chính (tham khảo: [issue552](https://github.com/Snailclimb/JavaGuide/issues/552))**: “Khi HotSpot duyệt qua mọi object, nó cộng dồn kích thước mà chúng chiếm theo thứ tự age tăng dần. Khi tổng kích thước đến một age nào đó vượt quá một nửa Survivor area, nó lấy giá trị nhỏ hơn giữa age đó và `MaxTenuringThreshold` làm promotion age threshold mới”.
>
...
1. **`java.lang.OutOfMemoryError: GC Overhead Limit Exceeded`**: lỗi này xảy ra khi JVM dành quá nhiều thời gian thực hiện garbage collection nhưng chỉ thu hồi được rất ít heap space.
2. **`java.lang.OutOfMemoryError: Java heap space`**: khi tạo object mới, nếu heap memory không đủ chỗ chứa object thì lỗi này sẽ xảy ra (liên quan đến dung lượng heap tối đa được cấu hình và chịu giới hạn bởi bộ nhớ vật lý. Dung lượng heap tối đa có thể cấu hình bằng parameter `-Xmx`; nếu không cấu hình đặc biệt thì sẽ dùng giá trị mặc định, xem [Default Java 8 max heap size](https://stackoverflow.com/questions/28272923/default-xmxsize-in-java-8-max-heap-size)).
3. ……
...
_Java Virtual Machine Specification_ chỉ quy định khái niệm method area và chức năng của nó; cách triển khai cụ thể là việc JVM tự quyết định. Nói cách khác, các JVM implementation khác nhau có cách triển khai method area khác nhau.
...
1. **Class metadata**: gồm cấu trúc đầy đủ của class như class name, parent class, interface đã implement, access modifier và thông tin chi tiết của field, method (name, type, modifier...).
2. **Method bytecode**: chuỗi instruction gốc của mỗi method.
...
- **Static variable (Static Variables)**: từ JDK 7, static variable của class được lưu trong Java Heap cùng với object `java.lang.Class` tương ứng.
...
1. Dung lượng Permanent Generation chịu giới hạn trên `-XX:MaxPermSize`; giới hạn này có thể cấu hình. Metaspace chuyển sang dùng native memory và có thể giới hạn bằng `-XX:MaxMetaspaceSize`. Cả hai đều có thể bị tràn; rủi ro thực tế phụ thuộc configuration và tốc độ tăng của class metadata.

> Khi Metaspace bị tràn sẽ nhận được lỗi sau: `java.lang.OutOfMemoryError: Metaspace`
...
4. Permanent Generation tạo ra độ phức tạp không cần thiết cho GC và hiệu quả thu hồi tương đối thấp.
...
Tương đối mà nói, garbage collection trong vùng này khá ít xảy ra, nhưng không phải dữ liệu đã vào method area sẽ “tồn tại vĩnh viễn”.
...
Khác biệt lớn với Permanent Generation là nếu không chỉ định `MaxMetaspaceSize`, Metaspace không có giới hạn trên cố định do parameter này đặt. Việc class metadata tiếp tục tăng có thể tiêu thụ lượng lớn native memory.
...
Giải thích về symbol reference và direct reference trong mục 7.34, bản 3 của _Understanding the JVM_ như sau:
...
Implementation của string constant pool trong HotSpot JVM có thể xem tại `src/hotspot/share/classfile/stringTable.cpp`. `StringTable` native của HotSpot hiện đại dùng concurrent hash table và dùng weak handle để lưu string object trên heap, hỗ trợ expansion, rehash và cleanup các entry không còn hiệu lực; `-XX:StringTableSize` dùng để đặt số bucket ban đầu. Vì vậy, cách nói “fixed-length array + linked list” chỉ áp dụng cho một số implementation cũ, không thể dùng để khái quát HotSpot hiện tại.
...
Cuối cùng, dưới đây là một đoạn lời của thầy Zhou Zhiming trong [sample code và errata của _Understanding the JVM_ (bản 3)](https://github.com/fenixsoft/jvm_book), tại [issue#112](https://github.com/fenixsoft/jvm_book/issues/112) của GitHub repository:
