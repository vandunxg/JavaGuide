---
title: "Chuyên đề JVM: vùng nhớ, class loading, garbage collection, tinh chỉnh tham số và xử lý sự cố production"
description: "Lộ trình học câu hỏi phỏng vấn và performance tuning JVM, bao quát vùng nhớ Java, cấu trúc class file, class loading, garbage collection, tham số JVM, công cụ monitoring JDK và xử lý sự cố production."
category: Java
tag:
  - Java
  - JVM
...
- Tham số JVM, GC log, heap dump, thread stack và các công cụ monitoring, chẩn đoán JDK thường gặp.
- Lộ trình cơ bản để xử lý sự cố production: quan sát hiện tượng, thu thập chỉ số, phân tích bằng công cụ, xác định nguyên nhân và kiểm chứng tối ưu hóa.

## Thứ tự đọc đề xuất

1. Khi chuẩn bị phỏng vấn, hãy đọc trước [Tổng hợp câu hỏi phỏng vấn JVM thường gặp](./jvm-interview-questions.md) để tìm ra các module bạn chưa trả lời đầy đủ; nếu học có hệ thống, bạn có thể bắt đầu từ bước tiếp theo.
2. [Giải thích JVM bằng ngôn ngữ dễ hiểu](./jvm-intro.md): trước tiên xây dựng nhận thức tổng thể về JVM.
3. [Giải thích chi tiết vùng nhớ Java (trọng tâm)](./memory-area.md): hiểu runtime data area và các tình huống OOM thường gặp.
4. [Giải thích chi tiết cấu trúc class file](./class-file-structure.md), [Giải thích chi tiết quá trình class loading](./class-loading-process.md), [Giải thích chi tiết class loader (trọng tâm)](./classloader.md): nắm được quá trình class đi từ `.class` đến object có thể chạy.
5. [Giải thích chi tiết garbage collection của JVM (trọng tâm)](./jvm-garbage-collection.md): học có hệ thống kiến thức cơ bản, thuật toán GC và garbage collector.
6. [Tổng hợp các tham số JVM quan trọng nhất](./jvm-parameters-intro.md), [Tổng hợp công cụ monitoring và xử lý sự cố JDK](./jdk-monitoring-and-troubleshooting-tools.md), [Xử lý sự cố production của Java backend](./jvm-in-action.md): bắt đầu đi vào cấu hình tham số và thực tiễn production.
...
- [Giải thích JVM bằng ngôn ngữ dễ hiểu](./jvm-intro.md): hiểu vị trí và thành phần của JVM từ góc nhìn tổng thể.
...
- [Xử lý sự cố production của Java backend](./jvm-in-action.md): bắt đầu từ xác nhận cảnh báo và thu thập bằng chứng để xử lý lần lượt CPU, memory, GC, thread pool, connection pool, slow SQL, Redis và message backlog.
...
- Object được tạo như thế nào? Có những cách nào để định vị việc truy cập object?
