...
- [JEP 338: Vector API (Incubator) (Vector API, lần incubate đầu tiên)](https://openjdk.java.net/jeps/338)
- [JEP 376: ZGC: Concurrent Thread-Stack Processing (ZGC xử lý thread stack đồng thời)](https://openjdk.java.net/jeps/376)
...
- [JEP 396: Strongly Encapsulate JDK Internals by Default (mặc định encapsulate chặt các thành phần bên trong JDK)](https://openjdk.java.net/jeps/396)
- [JEP 397: Sealed Classes (sealed class, lần preview thứ hai)](https://openjdk.java.net/jeps/397)
...
## JEP 338: Vector API (Vector API, lần incubate đầu tiên)

Vector API lần đầu được đề xuất trong [JEP 338](https://openjdk.java.net/jeps/338) và được tích hợp vào Java 16 dưới dạng [incubator API](http://openjdk.java.net/jeps/11). Lần incubate thứ hai được đề xuất trong [JEP 414](https://openjdk.java.net/jeps/414) và tích hợp vào Java 17, lần incubate thứ ba được đề xuất trong [JEP 417](https://openjdk.java.net/jeps/417) và tích hợp vào Java 18, lần thứ tư được đề xuất trong [JEP 426](https://openjdk.java.net/jeps/426) và tích hợp vào Java 19.

Incubator API này cung cấp một vòng lặp ban đầu của API để biểu diễn một số phép tính vector. Các phép tính này được biên dịch đáng tin cậy tại runtime thành các instruction phần cứng vector tối ưu trên kiến trúc CPU được hỗ trợ, nhờ đó đạt performance tốt hơn phép tính scalar tương đương và tận dụng đầy đủ kỹ thuật Single Instruction Multiple Data (SIMD), một loại instruction có thể sử dụng trên hầu hết CPU hiện đại. Dù HotSpot hỗ trợ vectorization tự động, tập hợp các phép toán scalar có thể chuyển đổi còn hạn chế và dễ bị ảnh hưởng bởi thay đổi code. API này giúp developer dễ dàng viết các thuật toán vector có performance cao và khả chuyển bằng Java.
...
## JEP 376: ZGC: Concurrent Thread-Stack Processing (ZGC xử lý thread stack đồng thời)

Java 16 chuyển việc xử lý thread stack của ZGC từ safe point sang một giai đoạn concurrent, cho phép tạm dừng tại safe point của GC trong vài mili giây ngay cả trên heap lớn. Việc loại bỏ nguồn latency cuối cùng trong garbage collector ZGC có thể cải thiện đáng kể performance và hiệu quả của ứng dụng.

## JEP 387: Elastic Metaspace (Metaspace co giãn)

Kể từ khi Metaspace được giới thiệu, theo phản hồi nhận được, Metaspace thường chiếm quá nhiều memory ngoài heap, gây lãng phí memory. Tính năng Metaspace co giãn có thể trả memory metadata của class HotSpot chưa sử dụng, tức memory metaspace, về operating system nhanh hơn, từ đó giảm dung lượng Metaspace.
...
Ngay từ Java 9, các nhà thiết kế Java đã nâng cấp annotation `@Deprecated`, bổ sung hai element mới là `since` và `forRemoval`. Element since dùng để chỉ định phiên bản API được đánh dấu bằng annotation `@Deprecated` bị deprecated, còn `forRemoval` làm rõ hơn ngữ nghĩa khi API được đánh dấu bằng annotation `@Deprecated`: nếu `forRemoval=true`, điều đó có nghĩa API chắc chắn sẽ bị xóa trong phiên bản tương lai và developer nên dùng API mới để thay thế, không còn dễ gây hiểu nhầm như trước (trước Java 9, API được đánh dấu bằng annotation `@Deprecated` có thể mang nhiều ngữ nghĩa, chẳng hạn có rủi ro khi sử dụng, có thể phát sinh lỗi tương thích trong tương lai, có thể bị xóa trong phiên bản tương lai hoặc nên dùng phương án thay thế tốt hơn).
...
Tiếp theo, khi sử dụng instance của value-based class trong block `synchronized`, compiler của JDK 16 sẽ sinh cảnh báo. HotSpot cũng có thể ghi log hoặc ngăn chặn kiểu synchronization này tại runtime thông qua diagnostic parameter. Ngay cả khi không bật diagnostic tại runtime, cũng không nên dùng instance của value-based class làm lock, ví dụ:
...
`Integer` là immutable object. `count++` sẽ thực hiện unboxing, tăng một đơn vị rồi boxing lại, đồng thời đổi field reference sang một instance `Integer` khác. Vì vậy, các thread khác nhau có thể lock các object khác nhau, không thể cung cấp mutual exclusion ổn định cho phép toán kết hợp này. Nếu cần tự tăng đồng thời, nên dùng một lock object cố định hoặc `AtomicInteger`.

## JEP 392: Packaging Tool (công cụ đóng gói, chính thức)

Trong Java 14, JEP 343 giới thiệu công cụ đóng gói với command `jpackage`. Trong Java 15, công cụ này tiếp tục ở giai đoạn incubate, còn trong Java 16 cuối cùng đã trở thành tính năng chính thức.

Công cụ đóng gói này cho phép đóng gói ứng dụng Java tự chứa. Nó hỗ trợ các định dạng đóng gói native, cung cấp trải nghiệm cài đặt tự nhiên cho end user. Các định dạng gồm msi và exe trên Windows, pkg và dmg trên macOS, cùng deb và rpm trên Linux. Công cụ cũng cho phép chỉ định parameter khi khởi động trong lúc đóng gói, có thể được gọi trực tiếp từ command line hoặc gọi bằng lập trình thông qua ToolProvider API. Lưu ý tên module của jpackage đã đổi từ jdk.incubator.jpackage thành jdk.jpackage. Điều này sẽ cải thiện trải nghiệm của end user khi cài đặt ứng dụng và đơn giản hóa việc deploy theo mô hình “app store”.
...
## JEP 393: Foreign Memory Access API (Foreign Memory Access API, lần incubate thứ ba)
...
Java 14 ([JEP 370](https://openjdk.org/jeps/370)) lần đầu incubate Foreign Memory Access API, Java 15 thực hiện lần incubate thứ hai ([JEP 383](https://openjdk.org/jeps/383)), còn Java 16 thực hiện lần incubate thứ ba.
...
Từ Java 16, bạn có thể sửa giá trị của variable trong `instanceof`.
...
## JEP 396: Strongly Encapsulate JDK Internals by Default (mặc định encapsulate chặt các thành phần bên trong JDK)

Tính năng này mặc định encapsulate chặt tất cả thành phần bên trong JDK, ngoại trừ các internal API quan trọng như `sun.misc.Unsafe`. Theo mặc định, code truy cập internal API của JDK vốn biên dịch thành công trên phiên bản cũ có thể không còn hoạt động. Developer được khuyến khích chuyển từ việc sử dụng thành phần bên trong sang API chuẩn, để họ và user có thể nâng cấp liền mạch lên các phiên bản Java tương lai. Encapsulation chặt được điều khiển bởi launcher option `–illegal-access` từ JDK 9; đến JDK 15, mặc định được đổi thành warning; từ JDK 16, mặc định là deny. Hiện tại vẫn có thể dùng một command-line option để nới lỏng encapsulation đối với tất cả package, nhưng trong tương lai chỉ có thể dùng `–add-opens` để mở các package cụ thể.

## JEP 397: Sealed Classes (sealed class, lần preview thứ hai)
...
- **JEP 369: Migration sang GitHub**: Tương ứng với thay đổi trong JEP 357 từ Mercurial sang Git, sau khi chuyển version control sang Git, cộng đồng OpenJDK đã chọn lưu trữ Git repository trên GitHub. Tuy nhiên, chỉ JDK 11 và các phiên bản JDK cao hơn được migration.
- **JEP 386: Port Alpine Linux**: Alpine Linux là một Linux distribution độc lập, phi thương mại, có kích thước rất nhỏ: một container không cần quá 8 MB dung lượng, bản cài đặt tối thiểu trên disk chỉ cần khoảng 130 MB storage và rất đơn giản, đồng thời vẫn bảo đảm tính an toàn. Đề xuất này port JDK sang Alpine Linux. Vì Alpine Linux là Linux distribution nhẹ dựa trên musl lib, các Linux distribution khác dùng musl lib trên kiến trúc x64 và AArch64 cũng được hỗ trợ.
