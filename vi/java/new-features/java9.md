...
head:
  - - meta
    - name: keywords
      content: Java 9,JDK9,module,JPMS,jlink,collection factory methods,new API
---

**Java 9** được phát hành vào ngày 21 tháng 9 năm 2017. Là phiên bản được phát hành sau Java 8 ba năm rưỡi, Java 9 mang đến nhiều thay đổi lớn. Trong đó, thay đổi quan trọng nhất là việc giới thiệu Java Platform Module System; ngoài ra còn có các thay đổi về collection, `Stream`...
...
- [JEP 193: Variable Handles (Variable handles)](https://openjdk.org/jeps/193)
...
1. Hạ thấp rào cản để viết chương trình Java đầu tiên kiểu `"Hello World!"`, giúp người mới hứng thú hơn với việc học.
...
**Code của JShell khác code thông thường có thể biên dịch ở điểm nào?**

1. Sau khi nhập xong một câu lệnh, JShell sẽ biên dịch và thực thi code trong background, sau đó trả về kết quả ngay, không cần người dùng tự chạy `javac` và `java`.
...
Chúng ta có thể dùng keyword `exports` để kiểm soát package nào được công khai cho bên ngoài sử dụng và công khai cho module nào.
...
module my.module {
    // exports ... to công khai package được chỉ định cho module được chỉ định
...
Variable handle là tham chiếu tới một biến hoặc một nhóm biến, bao gồm static field, non-static field, phần tử của array và các thành phần trong cấu trúc dữ liệu ngoài heap.
...
Sự xuất hiện của `VarHandle` thay thế một phần thao tác của `java.util.concurrent.atomic` và `sun.misc.Unsafe`. Đồng thời, nó cung cấp một loạt thao tác memory barrier tiêu chuẩn để kiểm soát memory ordering ở mức chi tiết hơn. Về tính an toàn, khả dụng và performance, nó đều tốt hơn API hiện có.
...
Method `ofNullable()` trong Java 9 có thể tạo `Stream` một phần tử hoặc rỗng từ một giá trị có thể là `null`. Java 8 đã có thể dùng `Stream.empty()` để tạo stream rỗng, nhưng chưa có method tiện lợi để chuyển trực tiếp giá trị có thể `null` thành stream.
...
Overload mới của method `iterate()` cung cấp một tham số `Predicate` (điều kiện kiểm tra) để quyết định khi nào kết thúc việc lặp.
...
Cách sử dụng hai method được so sánh dưới đây; overload mới của `iterate()` linh hoạt hơn.
...
Trong Java 8 và các phiên bản trước, `String` luôn được lưu trữ bằng `char[]`. Từ Java 9, implementation của `String` chuyển sang dùng array `byte[]` để lưu trữ chuỗi, giúp tiết kiệm dung lượng.
...
**Biến effectively-final là gì?** Nói đơn giản, đó là biến không được khai báo với `final` nhưng chưa từng thay đổi giá trị sau khi khởi tạo.
...
Trong class `java.util.concurrent.Flow` của Java 9, các interface cốt lõi của đặc tả Reactive Streams được bổ sung.
...
Để tìm hiểu chi tiết hơn về Reactive Streams trong Java 9, bạn nên đọc bài viết [Giải mã Java 9 (17. Reactive Streams ) - Lin Bản Thác](https://www.cnblogs.com/IcanFixIt/p/7245377.html).

## Khác

- **Cải tiến platform logging API**: Java 9 cho phép JDK và ứng dụng sử dụng cùng một logging implementation. Bổ sung `System.LoggerFinder` để quản lý logging implementation mà JDK sử dụng. Trong runtime, JVM chỉ có một instance `LoggerFinder` trên toàn hệ thống. Chúng ta có thể thêm implementation `System.LoggerFinder` của riêng mình để JDK và ứng dụng sử dụng các logging framework khác như SLF4J.
...
- **Cải thiện khả năng bảo mật của ứng dụng**: Java 9 bổ sung 4 thuật toán hash SHA-3: SHA3-224, SHA3-256, SHA3-384 và SHA3-512.
