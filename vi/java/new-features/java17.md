---
title: "Tính năng mới trong Java 17 (JDK 17): sealed class, pattern matching cho switch và Random API"
...
Java 17 (JDK 17) được phát hành chính thức vào ngày 14 tháng 9 năm 2021, là phiên bản được Oracle xác định là phiên bản hỗ trợ dài hạn (LTS).
...
Khi nâng cấp từ JDK 11 lên JDK 17, bạn nên đặc biệt chú ý đến sealed class, pattern matching cho `switch`, Random API giả ngẫu nhiên mới và các thay đổi về tính tương thích do việc đóng gói mạnh các API nội bộ của JDK. Phiên bản Java tối thiểu của Spring 6.x và Spring Boot 3.x cũng là Java 17.
...
- [JEP 398: Deprecate the Applet API for Removal (Đánh dấu Applet API deprecated để loại bỏ)](https://openjdk.java.net/jeps/398)
...
- [JEP 410: Remove the Experimental AOT and JIT Compiler (Loại bỏ AOT và JIT compiler thử nghiệm)](https://openjdk.java.net/jeps/410)
- [JEP 411: Deprecate the Security Manager for Removal (Đánh dấu Security Manager deprecated để loại bỏ)](https://openjdk.java.net/jeps/411)
- [JEP 412: Foreign Function & Memory API (Incubator) (Foreign Function & Memory API, incubator lần đầu)](https://openjdk.java.net/jeps/412)
- [JEP 414: Vector API (Second Incubator) (Vector API, incubator lần hai)](https://openjdk.java.net/jeps/414)
...
Java 17 bổ sung các loại interface và implementation mới cho bộ sinh số giả ngẫu nhiên (pseudorandom number generator, PRNG, còn gọi là bộ sinh bit ngẫu nhiên xác định), giúp developer dễ dàng thay đổi qua lại giữa các thuật toán PRNG khác nhau trong ứng dụng.
...
// Sử dụng timestamp làm seed cho số ngẫu nhiên
RandomGenerator randomGenerator = l128X256MixRandom.create(System.currentTimeMillis());
// Tạo số ngẫu nhiên
randomGenerator.nextInt(10);
```

## JEP 398: Deprecate the Applet API for Removal (Đánh dấu Applet API deprecated để loại bỏ)
...
Applet API được đánh dấu deprecated từ Java 9 ([JEP 289](https://openjdk.java.net/jeps/289)), nhưng khi đó chưa nhằm mục đích xóa bỏ.

## JEP 406: Pattern Matching for switch (Pattern matching cho switch, preview)

Tương tự `instanceof`, `switch` cũng được bổ sung khả năng tự động chuyển đổi khi matching type.
...
Trong [Tổng quan tính năng mới của Java 14 & 15](./java14-15.md), sealed class đã được giới thiệu chi tiết, nên ở đây không giới thiệu thêm.

## JEP 410: Remove the Experimental AOT and JIT Compiler (Loại bỏ AOT và JIT compiler thử nghiệm)

Trong Java 9, [JEP 295](https://openjdk.java.net/jeps/295) đã giới thiệu AOT compiler thử nghiệm, dùng để compile class Java thành native code trước khi khởi động virtual machine.

Java 17 loại bỏ AOT và JIT compiler thử nghiệm vì compiler này ít được sử dụng kể từ khi ra mắt, trong khi công sức cần thiết để bảo trì lại rất lớn. JVM Compiler Interface (JVMCI) thử nghiệm ở cấp Java vẫn được giữ lại, để developer có thể tiếp tục sử dụng các phiên bản compiler được build bên ngoài cho JIT compilation.

## JEP 411: Deprecate the Security Manager for Removal (Đánh dấu Security Manager deprecated để loại bỏ)

Đánh dấu Security Manager deprecated để loại bỏ trong các phiên bản tương lai.

Security Manager có từ Java 1.0. Trong nhiều năm, nó không còn là phương thức chính để bảo vệ code Java phía client và cũng hiếm khi được dùng để bảo vệ code phía server. Để thúc đẩy Java phát triển, Java 17 đánh dấu Security Manager deprecated để loại bỏ cùng với Applet API cũ ([JEP 398](https://openjdk.java.net/jeps/398)).

## JEP 412: Foreign Function & Memory API (Foreign Function & Memory API, incubator)

Java program có thể dùng API này để tương tác với code và data bên ngoài Java runtime. Thông qua việc gọi hiệu quả các foreign function (tức code bên ngoài JVM) và truy cập an toàn vào foreign memory (tức memory không do JVM quản lý), API này cho phép Java program gọi native library và xử lý native data mà không nguy hiểm, mong manh như JNI.
...
Trong [Tổng quan tính năng mới của Java 19](./java19.md), Foreign Function & Memory API đã được giới thiệu chi tiết, nên ở đây không giới thiệu thêm.

## JEP 414: Vector API (Second Incubator) (Vector API, incubator lần hai)
...
Incubator API này cung cấp một iteration ban đầu của API để biểu diễn một số phép tính vector. Các phép tính này được compile một cách đáng tin cậy khi runtime thành các instruction phần cứng vector tối ưu trên kiến trúc CPU được hỗ trợ, từ đó đạt performance cao hơn so với phép tính scalar tương đương và tận dụng kỹ thuật Single Instruction, Multiple Data (SIMD), một loại instruction có trên hầu hết CPU hiện đại. Mặc dù HotSpot hỗ trợ vectorization tự động, tập hợp các phép toán scalar có thể chuyển đổi còn hạn chế và dễ bị ảnh hưởng bởi thay đổi code. API này giúp developer dễ dàng viết các thuật toán vector có performance cao và portable bằng Java.

Trong [Tổng quan tính năng mới của Java 18](./java18.md), Vector API đã được giới thiệu chi tiết, nên ở đây không giới thiệu thêm.
