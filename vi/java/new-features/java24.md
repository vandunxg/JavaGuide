...
description: Tổng hợp các tính năng mới và thay đổi của JDK 24, giúp theo dõi quá trình phát triển của Java.
category: Java
tag:
...
- [JEP 478: Key Derivation Function API (API dẫn xuất khóa, preview)](https://openjdk.org/jeps/478)
...
- [JEP 485: Stream Gatherers (các Stream Gatherer)](https://openjdk.org/jeps/485)
- [JEP 486: Disable the Security Manager (vô hiệu hóa vĩnh viễn Security Manager)](https://openjdk.org/jeps/486)
- [JEP 487: Scoped Values (giá trị theo phạm vi, preview lần thứ tư)](https://openjdk.org/jeps/487)
- [JEP 495: Simplified Source Files and Instance Main Methods (đơn giản hóa Source File và Instance Main Method, preview lần thứ tư)](https://openjdk.org/jeps/495)
...
## JEP 478: Key Derivation Function API (API dẫn xuất khóa, preview)

Key Derivation Function API cung cấp một nhóm interface tiêu chuẩn để dẫn xuất thêm các khóa từ khóa ban đầu và dữ liệu khác. API này có thể tạo nhiều khóa khác nhau cho các mục đích mã hóa khác nhau, chẳng hạn như mã hóa và xác thực, tránh trực tiếp sử dụng cùng một khóa nhiều lần. API này đang ở giai đoạn preview trong JDK 24.
...
Trong JVM truyền thống, ứng dụng phải tải và liên kết class một cách động mỗi khi khởi động. Tính năng này cache các class đã được tải và liên kết, giảm công việc lặp lại trong những lần khởi động sau. Trong benchmark Spring PetClinic được nêu trong JEP 483, thời gian khởi động sau khi sử dụng AOT cache được rút ngắn tối đa khoảng 42%; lợi ích thực tế phụ thuộc vào ứng dụng và môi trường chạy.

Tối ưu hóa này không yêu cầu sửa code của ứng dụng, library hoặc framework, nhưng trước tiên cần ghi lại một lần chạy dùng cho training và tạo AOT cache, sau đó nạp cache này khi khởi động chính thức. Các parameter liên quan gồm `-XX:AOTMode=record`, `-XX:AOTConfiguration`, `-XX:AOTMode=create` và `-XX:AOTCache`.

## JEP 484: Class File API (API Class File)

Class File API đã có preview lần đầu trong JDK 22 ([JEP 457](https://openjdk.org/jeps/457)), preview lần thứ hai trong JDK 23 và được hoàn thiện thêm ([JEP 466](https://openjdk.org/jeps/466)). Cuối cùng, tính năng này đã trở thành tính năng chuẩn trong JDK 24.

Mục tiêu của Class File API là cung cấp một nhóm API tiêu chuẩn hóa để phân tích, tạo và chuyển đổi Class File của Java, thay thế sự phụ thuộc trước đây vào các thư viện bên thứ ba như ASM trong việc xử lý Class File.

```java
// Tạo một đối tượng ClassFile, đây là điểm bắt đầu để thao tác với Class File.
...
byte[] newBytes = cf.build(classModel.thisClass().asSymbol(),
        classBuilder -> {
            ...
            for (ClassElement ce : classModel) {
                // Kiểm tra element có phải method và tên method có bắt đầu bằng "debug" hay không
...
## JEP 485: Stream Gatherers (các Stream Gatherer)
...
Khác với các operation tích hợp sẵn như `filter`, `map` hoặc `distinct`, `Stream::gather` cho phép developer thực hiện những task khó hoàn thành bằng các operation Stream tiêu chuẩn. Ví dụ, có thể dùng `Stream::gather` để thực hiện sliding window, loại bỏ element trùng lặp theo rule tùy chỉnh hoặc chuyển đổi và aggregate state phức tạp hơn. Tính linh hoạt này mở rộng đáng kể phạm vi ứng dụng của Stream API, giúp developer xử lý các scenario dữ liệu phức tạp hơn.
...
JDK 24 không còn cho phép bật `Security Manager`. Ngay cả khi sử dụng lệnh `java -Djava.security.manager` cũng không thể bật tính năng này. Đây là một bước quan trọng trong quá trình loại bỏ dần tính năng này. Mặc dù `Security Manager` từng là công cụ quan trọng trong Java để hạn chế quyền của code, chẳng hạn quyền truy cập file system hoặc network, đọc hoặc ghi file nhạy cảm và thực thi system command, cộng đồng Java quyết định loại bỏ hoàn toàn do tính phức tạp cao, mức độ sử dụng thấp và chi phí bảo trì lớn.

## JEP 487: Scoped Values (giá trị theo phạm vi, preview lần thứ tư)

Scoped Values có thể chia sẻ dữ liệu immutable giữa các thread và trong phạm vi một thread, có ưu điểm hơn ThreadLocal, đặc biệt khi sử dụng nhiều virtual thread.
...
Scoped Values cho phép chia sẻ dữ liệu an toàn và hiệu quả giữa các component trong chương trình lớn mà không cần truyền qua parameter của method.

## JEP 491: Virtual Threads Synchronization Without Pinning (đồng bộ hóa virtual thread mà không pin platform thread)
...
Code Java hiện có sử dụng `synchronized` không cần sửa vẫn có thể hưởng lợi từ khả năng mở rộng của virtual thread. Ví dụ, một ứng dụng I/O-intensive nếu sử dụng platform thread truyền thống có thể bị giảm khả năng concurrency do thread bị block. Ngược lại, khi sử dụng virtual thread, ngay cả khi xảy ra blocking trong block `synchronized`, platform thread cũng không bị pin, cho phép platform thread tiếp tục phục vụ các virtual thread khác và cải thiện performance tổng thể của concurrency.
...
## JEP 495: Simplified Source Files and Instance Main Methods (đơn giản hóa Source File và Instance Main Method, preview lần thứ tư)

Tính năng này chủ yếu đơn giản hóa khai báo method `main`. Đối với người mới học Java, khai báo method `main` này đòi hỏi quá nhiều khái niệm cú pháp Java, không thuận lợi cho việc bắt đầu nhanh.

Định nghĩa method `main` trước khi áp dụng tính năng này:
...
Định nghĩa method `main` sau khi áp dụng tính năng mới:
...
JDK 24 bổ sung hỗ trợ triển khai thuật toán chữ ký số dựa trên module lattice có khả năng kháng lượng tử (Module-Lattice-Based Digital Signature Algorithm, **ML-DSA**), nhằm chuẩn bị đối phó với các mối đe dọa có thể do máy tính lượng tử trong tương lai gây ra.
...
## JEP 498: Warnings When Using `sun.misc.Unsafe` Memory Access Methods (cảnh báo khi sử dụng các method truy cập memory của `sun.misc.Unsafe`)

JDK 23 ([JEP 471](https://openjdk.org/jeps/471)) đề xuất đánh dấu các method truy cập memory trong `sun.misc.Unsafe` là deprecated; những method này sẽ bị loại bỏ trong các phiên bản tương lai. Trong JDK 24, runtime sẽ phát cảnh báo khi một method truy cập memory bất kỳ của `sun.misc.Unsafe` được gọi lần đầu.
...
class OffHeapIntBuffer {
    ...
    // Constructor, cấp phát không gian cho số integer được chỉ định
...
Structured Concurrency coi nhiều task chạy trong các thread khác nhau là một work unit duy nhất, từ đó đơn giản hóa việc xử lý lỗi, tăng độ tin cậy và nâng cao khả năng quan sát. Nói cách khác, Structured Concurrency giữ lại tính dễ đọc, khả năng bảo trì và khả năng quan sát của code single-thread.
...
```java
    try (var scope = new StructuredTaskScope<Object>()) {
        ...
        scope.join();
        // Xử lý result có thể bao gồm xử lý hoặc ném lại exception
        ... process results/exceptions ...
    } // close
```

Structured Concurrency đặc biệt phù hợp với virtual thread, là loại thread nhẹ do JDK triển khai. Nhiều virtual thread dùng chung một thread của hệ điều hành, nhờ đó cho phép sử dụng số lượng virtual thread rất lớn.
