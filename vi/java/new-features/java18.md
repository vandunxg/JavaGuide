...
description: "Tổng quan các cập nhật và tính năng preview của JDK 18, tìm hiểu các cải tiến do API mới mang lại."
category: Java
tag:
  - Java New Features
head:
...
- [JEP 400: UTF-8 by Default (UTF-8 làm charset mặc định)](https://openjdk.java.net/jeps/400)
- [JEP 408: Simple Web Server (static Web server đơn giản)](https://openjdk.java.net/jeps/408)
...
## JEP 400: UTF-8 by Default (UTF-8 làm charset mặc định, finalized)

JDK cuối cùng đã đặt UTF-8 làm charset mặc định.

Trong Java 17 và các phiên bản trước, charset mặc định chỉ được xác định khi JVM chạy, phụ thuộc vào các yếu tố như hệ điều hành, locale và các yếu tố khác, nên tồn tại rủi ro tiềm ẩn. Ví dụ, một chương trình Java in văn bản ra console chạy bình thường trên Mac nhưng lại hiển thị ký tự lỗi trên Windows nếu bạn không tự thay đổi charset.

## JEP 408: Simple Web Server (static Web server đơn giản, finalized)

Từ Java 18, bạn có thể dùng lệnh `jwebserver` để khởi động một static Web server đơn giản.
...
Server này không hỗ trợ CGI và Servlet, chỉ phục vụ static file.
...
Cách tạo code snippet bằng `<pre>{@code ...}</pre>` cho kết quả tương đối bình thường.
...
Java 18 cải thiện logic triển khai của `java.lang.reflect.Method` và `Constructor`, giúp performance tốt hơn và tốc độ nhanh hơn. Thay đổi này không sửa các API liên quan. Điều đó có nghĩa là trong quá trình phát triển, bạn không cần sửa code reflection liên quan mà vẫn có thể trải nghiệm reflection với performance tốt hơn.

OpenJDK cung cấp kết quả benchmark performance reflection của triển khai mới và cũ.

![Kết quả benchmark performance reflection của triển khai mới và cũ](https://oss.javaguide.cn/github/javaguide/java/new-features/JEP416Benchmark.png)
...
Phép tính vector gồm một chuỗi thao tác trên vector. Vector API dùng để biểu đạt phép tính vector, phép tính này có thể được compile đáng tin cậy thành các instruction vector tối ưu trên kiến trúc CPU được hỗ trợ tại runtime, từ đó đạt performance tốt hơn phép tính scalar tương đương.

Vector API hướng đến việc cung cấp cho người dùng một cách biểu đạt đơn giản, dễ dùng và độc lập với platform cho nhiều loại phép tính vector.
...
Java 18 định nghĩa một SPI (service-provider interface) hoàn toàn mới, chủ yếu dùng để phân giải tên và địa chỉ, để `java.net.InetAddress` có thể sử dụng resolver bên thứ ba ngoài platform.

## JEP 419: Foreign Function & Memory API (Foreign Function & Memory API, incubator lần thứ hai)

Java program có thể dùng API này để tương tác với code và data bên ngoài Java runtime. Bằng cách gọi foreign function hiệu quả (tức code bên ngoài JVM) và truy cập foreign memory an toàn (tức memory không do JVM quản lý), API này cho phép Java program gọi native library và xử lý native data mà không nguy hiểm và dễ hỏng như JNI.
