---
title: "Hệ thống kiến thức Java: Basics, Collection, Concurrency, JVM, IO và tính năng mới"
description: Lộ trình học kiến thức và phỏng vấn Java, bao quát Java Basics, source code Collection, lập trình Concurrency, JVM, IO/NIO và các tính năng mới của Java, phù hợp để ôn tập phỏng vấn Java backend cho sinh viên mới tốt nghiệp và người đã đi làm.
category: Java
tag:
  - Java
  - Java Basics
...
Hệ thống kiến thức Java này hướng đến việc học Java backend và ôn tập phỏng vấn, sắp xếp các bài viết Java trên website theo thứ tự “cú pháp cơ bản -> collection container -> lập trình concurrency -> IO/NIO -> JVM -> tính năng mới”.

Nếu có ít thời gian, bạn nên đọc trước các bài tổng hợp câu hỏi phỏng vấn về Java Basics, Collection, Concurrency và JVM để nhanh chóng xây dựng danh sách câu hỏi thường gặp; nếu muốn củng cố kiến thức cơ bản một cách có hệ thống, bạn có thể đọc theo thứ tự chuyên đề bên dưới.

## Dành cho ai

- Backend developer đang học Java một cách có hệ thống.
- Người chuẩn bị phỏng vấn Java backend khi mới tốt nghiệp, đã đi làm hoặc tại các công ty công nghệ vừa và lớn.
- Độc giả muốn ôn tập liền mạch Java Basics, Collection, Concurrency, JVM, IO và các tính năng mới.
- Kỹ sư đã từng viết dự án Java nhưng chưa hiểu có hệ thống về nguyên lý bên trong, thiết kế source code và thực tiễn kỹ thuật.
...
- Phạm vi sử dụng, triển khai source code và câu hỏi phỏng vấn thường gặp của List, Map, Queue và các collection hỗ trợ concurrency.
- Thread, lock, JMM, CAS, AQS, thread pool, CompletableFuture và virtual thread trong Java.
- Vùng nhớ JVM, class loading, garbage collection, cấu hình tham số, công cụ monitoring và xử lý sự cố trên môi trường production.
- BIO, NIO, AIO, các mô hình IO và design pattern liên quan đến IO như decorator, adapter.
- Các tính năng mới quan trọng từ Java 8 đến Java 26, cùng những tính năng thực sự ảnh hưởng đến việc phát triển hằng ngày.

## Thứ tự đọc đề xuất

1. [Chuyên đề Java Basics](./basis/): Nắm vững cú pháp, lập trình hướng đối tượng, generic, reflection, proxy, serialization và các năng lực cơ bản khác trước.
...
4. [Chuyên đề JVM](./jvm/): Hiểu vùng nhớ, class loading, garbage collection, tham số JVM và xử lý sự cố trên môi trường production.
...
- [Tổng hợp câu hỏi phỏng vấn Java Basics (phần 3)](./basis/java-basic-questions-03.md): Bổ sung kiến thức nâng cao trong Java Basics và các điểm dễ mắc lỗi thường gặp.
...
- [Chuyên đề Java Collection](./collection/): Kết nối collection framework, các lưu ý khi sử dụng và phân tích source code thường gặp.
...
- [Giải thích chi tiết về CAS](./concurrent/cas.md), [Giải thích chi tiết về AQS](./concurrent/aqs.md), [Giải thích chi tiết về Java thread pool](./concurrent/java-thread-pool-summary.md): Nắm vững các chủ đề quan trọng về tầng bên trong của concurrency.
...
- [Chuyên đề JVM](./jvm/): Tập trung vào memory, class loading, GC, tham số, công cụ và xử lý sự cố trên môi trường production.
...
- [Giải thích chi tiết về quá trình class loading](./jvm/class-loading-process.md) và [Giải thích chi tiết về class loader (trọng tâm)](./jvm/classloader.md): Nắm vững vòng đời class và mô hình delegation cho parent.
- [Chuyên đề Java IO](./io/): Trình bày từ BIO, NIO, AIO đến mô hình IO và design pattern về IO.
- [Tổng hợp kiến thức cơ bản về Java IO](./io/io-basis.md), [Tổng hợp kiến thức cốt lõi về Java NIO](./io/nio-basis.md), [Giải thích chi tiết về mô hình Java IO](./io/io-model.md): Bổ sung kiến thức nền tảng cần thiết cho việc học network programming và middleware.
...
- [Thực hành các tính năng mới của Java 8](./new-features/java8-common-new-features.md): Nắm vững Lambda, Stream, Optional, default method của interface và API date mới.
- [Tổng quan tính năng mới của Java 11 (quan trọng)](./new-features/java11.md), [Tổng quan tính năng mới của Java 17 (quan trọng)](./new-features/java17.md), [Tổng quan tính năng mới của Java 21 (quan trọng)](./new-features/java21.md): Ưu tiên chú ý đến các tính năng có khả năng sử dụng lâu dài trong những phiên bản LTS.
