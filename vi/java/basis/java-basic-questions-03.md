...
- **`Exception`**: exception mà bản thân chương trình có thể xử lý, có thể bắt bằng `catch`. `Exception` lại được chia thành Checked Exception (checked exception, bắt buộc xử lý) và Unchecked Exception (unchecked exception, có thể không xử lý).
- **`Error`**: `Error` thuộc nhóm lỗi mà chương trình không thể xử lý, ~~chúng ta không thể bắt bằng `catch`~~ không khuyến nghị bắt bằng `catch`. Ví dụ: lỗi khi JVM chạy (`Virtual MachineError`), lỗi JVM không đủ memory (`OutOfMemoryError`), lỗi định nghĩa class (`NoClassDefFoundError`), v.v. Khi các lỗi này xảy ra, JVM thường sẽ chọn kết thúc thread.

### Sự khác nhau giữa `ClassNotFoundException` và `NoClassDefFoundError`

- `ClassNotFoundException` là `Exception`, xảy ra khi không tìm thấy class trong lúc dynamic loading như reflection; đây là tình huống có thể dự đoán và bắt để xử lý.
...
**Checked Exception** tức checked exception. Trong quá trình compile code Java, nếu checked exception không được xử lý bằng `catch` hoặc keyword `throws`, code sẽ không thể compile.
...
- `IllegalArgumentException` (lỗi argument, ví dụ kiểu tham số truyền vào method không đúng)
...
- `ArithmeticException` (lỗi arithmetic)
- `SecurityException` (lỗi security, ví dụ không đủ quyền)
- `UnsupportedOperationException` (lỗi operation không được hỗ trợ, ví dụ tạo trùng cùng một user)
- …
...
- `String getLocalizedMessage()`: trả về thông tin localized của exception object. Subclass của `Throwable` có thể override method này để tạo thông tin localized. Nếu subclass không override method này, thông tin method trả về giống với kết quả của `getMessage()`
...
- Block `try`: dùng để bắt exception. Sau nó có thể là không hoặc nhiều block `catch`; nếu không có block `catch` thì bắt buộc phải có block `finally`.
...
**Lưu ý: Không dùng `return` trong block `finally`!** Khi cả câu lệnh `try` và `finally` đều có `return`, câu lệnh `return` trong block `try` sẽ bị bỏ qua. Nguyên nhân là giá trị trả về của `return` trong câu lệnh `try` trước tiên được tạm lưu vào một local variable; khi thực thi đến `return` trong câu lệnh `finally`, giá trị của local variable này sẽ trở thành giá trị trả về của `return` trong `finally`.
...
### Code trong `finally` có nhất định được thực thi không?

Không nhất định! Trong một số tình huống, code trong `finally` sẽ không được thực thi.

Ví dụ, nếu JVM bị kết thúc trước `finally`, code trong `finally` sẽ không được thực thi.
...
> Với resource bắt buộc phải đóng, luôn nên ưu tiên `try-with-resources` thay vì `try-finally`. Code tạo ra sẽ ngắn hơn, rõ ràng hơn, và exception tạo ra cũng hữu ích hơn. Câu lệnh `try-with-resources` giúp viết code phải đóng resource dễ hơn; nếu dùng `try-finally` thì gần như không thể làm tốt điều này.
...
- Không định nghĩa exception thành static variable, vì điều này khiến thông tin exception stack bị sai. Mỗi lần chủ động throw exception, cần tự new một exception object để throw.
...
Compiler có thể kiểm tra generic parameter, đồng thời generic parameter cho phép chỉ định type của object được truyền vào. Ví dụ dòng `ArrayList<Person> persons = new ArrayList<Person>()` chỉ rõ object `ArrayList` này chỉ nhận object `Person`; nếu truyền object type khác vào sẽ báo lỗi.

```java
ArrayList<E> extends AbstractList<E>
```

Ngoài ra, return type của `List` raw là `Object`, cần tự chuyển type mới có thể sử dụng; sau khi dùng generic, compiler sẽ tự động chuyển type.
...
```java
// T ở đây có thể viết tùy ý thành bất kỳ identifier nào; các parameter thường gặp như T, E, K, V thường dùng để biểu thị generic
...
Implement generic interface, không chỉ định type:
...
Implement generic interface, chỉ định type:
...
- Interface custom trả về kết quả generic `CommonResult<T>` có thể chỉ định dynamic data type của result theo return type cụ thể thông qua parameter `T`
- Định nghĩa class xử lý `Excel` là `ExcelUtil<T>` để dynamic chỉ định data type được `Excel` export
...
Nói đơn giản, Java reflection (Reflection) là khả năng **dynamic lấy thông tin về class và thao tác với class hoặc object (method, field) trong lúc chương trình runtime**.

Thông thường, type của code ta viết đã được xác định lúc compile; cần gọi method nào, truy cập field nào đều rất rõ ràng. Nhưng reflection cho phép đến **runtime** mới tìm hiểu một class có những method, property nào, constructor ra sao, đồng thời dynamic tạo object, gọi method hoặc sửa property khi access control và module boundary cho phép.

Chính khả năng “quan sát chính mình” và thao tác lúc runtime này khiến reflection trở thành **nền tảng của nhiều framework và library dùng chung**. Nó giúp code linh hoạt hơn và có thể xử lý type chưa biết lúc compile.

### Ưu và nhược điểm của reflection?

**Ưu điểm:**

1. **Tính linh hoạt và dynamic:** reflection cho phép chương trình dynamic load class, tạo object, gọi method và truy cập field lúc runtime; từ đó dynamic thích ứng và mở rộng behavior của chương trình theo nhu cầu thực tế (như config file, user input, annotation, v.v.). Nhiều Java framework hiện đại (như Spring, Hibernate, MyBatis) dựa vào tính năng này để triển khai các chức năng cốt lõi như dependency injection (DI), aspect-oriented programming (AOP), object-relational mapping (ORM), xử lý annotation; có thể nói reflection là nền tảng không thể thiếu của framework development.
2. **Giảm coupling và tính dùng chung:** thông qua reflection, có thể viết code dùng chung, tái sử dụng và highly decoupled hơn, giảm dependency giữa các module. Ví dụ có thể dùng reflection để triển khai object copy dùng chung, serialization, Bean utility, v.v.

**Nhược điểm:**

1. **Performance overhead:** reflection operation thường chậm hơn gọi trực tiếp trong code. Nguyên nhân gồm dynamic type resolution, tìm method và việc tối ưu của JIT compiler bị hạn chế. Tuy nhiên, với phần lớn trường hợp dùng trong framework, performance loss này thường có thể chấp nhận được hoặc bản thân framework sẽ cache để tối ưu.
2. **Vấn đề security:** khi đáp ứng các điều kiện như access check và quan hệ mở module, reflection có thể giảm bớt một số access check của ngôn ngữ Java (như truy cập field và method `private`), có thể phá vỡ encapsulation. Ngoài ra, reflection còn có thể bypass kiểm tra generic ở compile time, gây rủi ro về type safety. Từ Java 9 trở đi, module boundary có thể từ chối deep reflection access như vậy và throw `InaccessibleObjectException`.
3. **Khả năng đọc và maintainability của code:** lạm dụng reflection khiến code phức tạp, khó hiểu và debug. Error thường chỉ bộc lộ lúc runtime, không dễ phát hiện như compile-time error.
...
Khi viết business code, có thể ta ít trực tiếp làm việc với Java reflection (Reflection). Nhưng có thể bạn chưa nhận ra rằng mình đang tận hưởng sự tiện lợi do reflection mang lại mỗi ngày! **Nhiều framework phổ biến như Spring/Spring Boot, MyBatis đều sử dụng rất nhiều reflection ở tầng dưới**, nhờ đó chúng mới linh hoạt và mạnh mẽ như vậy.

Dưới đây là một số use case phổ biến để dễ hình dung.

**1. Dependency injection và inversion of control (IoC)**

Các IoC framework tiêu biểu như Spring/Spring Boot sẽ scan các class có annotation cụ thể (như `@Component`, `@Service`, `@Repository`, `@Controller`) lúc startup, dùng reflection để instantiate object (Bean), rồi inject dependency bằng reflection (như `@Autowired`, constructor injection, v.v.).

**2. Xử lý annotation**

Annotation bản thân chỉ là một “marker”; phải có thành phần đọc marker đó thì mới biết cần làm gì. Reflection chính là “reader” đó. Framework dùng reflection để kiểm tra class, method, field có annotation cụ thể hay không, sau đó thực thi logic tương ứng theo thông tin annotation. Ví dụ khi thấy `@Value`, nó dùng reflection đọc nội dung annotation, tìm value tương ứng trong config file, rồi dùng reflection set value cho field.

**3. Dynamic proxy và AOP**

Muốn tự động thêm một số xử lý trước hoặc sau khi gọi method (như ghi log, mở transaction, kiểm tra quyền)? AOP (aspect-oriented programming) được dùng cho việc này, còn dynamic proxy là một cách phổ biến để triển khai AOP. Dynamic proxy có sẵn trong JDK (`Proxy` và `InvocationHandler`) không thể thiếu reflection. Khi proxy object gọi method của real object bên trong, nó thực hiện thông qua `Method.invoke` của reflection.
...
public class DebugInvocationHandler implements InvocationHandler {
    ...
    // proxy: proxy object, method: method được gọi, args: method arguments
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        System.out.println("Logic aspect: trước khi gọi method " + method.getName());
        // Gọi method cùng tên của real object bằng reflection
        Object result = method.invoke(target, args);
        System.out.println("Logic aspect: sau khi gọi method " + method.getName());
        return result;
    }
}
```
...
Các framework như MyBatis, Hibernate có thể giúp bạn tự động biến từng dòng data lấy từ database thành từng Java object. Làm sao chúng biết database field tương ứng với Java property nào? Vẫn là nhờ reflection. Chúng dùng reflection lấy danh sách property của Java class, ghép query result theo name hoặc config, sau đó dùng reflection gọi setter hoặc sửa trực tiếp field value. Ngược lại, khi lưu object vào database, chúng cũng dùng reflection đọc property value để ghép SQL.
...
Dynamic proxy là một design pattern rất mạnh, cho phép **enhance chức năng của method thuộc một class hoặc object mà không sửa source code**.
...
Đây là cách Java cung cấp chính thức; yêu cầu cốt lõi là target class phải implement một hoặc nhiều interface. Khi runtime, JDK dynamic proxy dùng method `Proxy.newProxyInstance()` để dynamic tạo instance của một proxy class implement các interface này. Proxy class được generate trong memory nên bạn không nhìn thấy file `.java` hoặc `.class` của nó.

Khi gọi bất kỳ method nào của proxy object, lời gọi đó sẽ được forward tới method `invoke` của interface `InvocationHandler` mà ta cung cấp. Trong method `invoke`, có thể thêm enhancement logic trước hoặc sau khi gọi method gốc (target method).

**Cách thứ hai: CGLIB dynamic proxy**

CGLIB là một code generation library bên thứ ba. Nguyên lý của nó hoàn toàn khác JDK: không yêu cầu class được proxy implement interface. Khi runtime, nó dynamic generate subclass của target class làm proxy class (thông qua kỹ thuật thao tác bytecode ASM). Sau đó, nó override mọi method không phải `final`, `private` và `static` trong parent class (tức class được proxy).

Khi gọi bất kỳ method nào của proxy object, lời gọi đó sẽ bị method `intercept` của interface `MethodInterceptor` trong CGLIB intercept. Tương tự method `invoke` của `InvocationHandler`, có thể thêm enhancement logic trong method `intercept`, trước hoặc sau khi gọi method của parent class gốc.

### Static proxy và dynamic proxy khác nhau thế nào?

Khác biệt cốt lõi giữa static proxy và dynamic proxy nằm ở **thời điểm xác định quan hệ proxy, tính linh hoạt khi triển khai và chi phí maintainability**.
...
| Cách triển khai                  | Tự viết proxy class trước khi compile, thường gọi target object bằng composition và delegation                                                                | Không cần tự viết proxy class cụ thể, dùng `Handler`/`Interceptor` đóng gói enhancement logic                                                               |
...
2. Về performance, trong phần lớn trường hợp JDK dynamic proxy tốt hơn; khi version JDK được nâng cấp, ưu thế này càng rõ rệt.

### Hãy giới thiệu use case thực tế của dynamic proxy trong framework

Use case điển hình nhất của dynamic proxy là **Spring AOP**.

AOP (Aspect-Oriented Programming: aspect-oriented programming) có thể đóng gói các logic hoặc responsibility không liên quan business nhưng được các business module cùng gọi (như transaction processing, log management, permission control), giúp giảm code trùng lặp trong system, giảm coupling giữa các module, đồng thời có lợi cho khả năng mở rộng và maintainability sau này.
...
`Annotation` là một tính năng mới được giới thiệu từ Java 5, có thể xem là một dạng comment đặc biệt, chủ yếu dùng để sửa class, method hoặc variable, cung cấp thông tin cho chương trình sử dụng lúc compile hoặc runtime.
...
### Có những cách parse annotation nào?

Annotation chỉ có hiệu lực sau khi được parse; các cách parse thường gặp gồm hai loại:

- **Scan trực tiếp lúc compile:** compiler scan và xử lý annotation tương ứng khi compile code Java. Ví dụ method dùng annotation `@Override`, compiler sẽ kiểm tra method hiện tại có override method tương ứng của parent class lúc compile hay không.
...
Nhiều framework sử dụng cơ chế SPI của Java, như Spring framework, load database driver, logging interface và các extension implementation của Dubbo.
...
Ví dụ dễ hiểu: công ty H là một công ty công nghệ, vừa thiết kế một chip mới và hiện cần mass production, trong khi thị trường có vài công ty sản xuất chip. Khi công ty H chỉ định tiêu chuẩn sản xuất chip (định nghĩa interface standard), các công ty chip hợp tác (service provider) sẽ giao chip có đặc điểm riêng theo tiêu chuẩn (cung cấp implementation của các solution khác nhau nhưng cho ra cùng một kết quả).

### Ưu và nhược điểm của SPI?

Cơ chế SPI có thể tăng đáng kể tính linh hoạt của interface design, nhưng cũng có một số nhược điểm, chẳng hạn:
...
Khi cần persist Java object, chẳng hạn lưu Java object vào file hoặc truyền Java object qua network, đều cần dùng serialization.
...
Với ngôn ngữ lập trình hướng object như Java, đối tượng được serialize là object, tức class đã instantiate (Class); nhưng trong C++, một ngôn ngữ bán hướng object, `struct` (struct) định nghĩa data structure type, còn class tương ứng với object type.
...
Như hình trên, trong OSI 7-layer protocol model, presentation layer chủ yếu xử lý và chuyển user data của application layer thành binary stream. Ngược lại, nó chuyển binary stream thành user data của application layer. Điều này tương ứng với serialization và deserialization, đúng không?

Vì application layer, presentation layer và session layer trong OSI 7-layer protocol model đều tương ứng với application layer trong TCP/IP 4-layer model, serialization protocol thuộc một phần của application layer trong TCP/IP protocol.

### Nếu có field không muốn serialize thì làm thế nào?

Với variable không muốn serialize, dùng keyword `transient` để đánh dấu.
...
Cách serialization đi kèm JDK thường không được dùng vì hiệu suất serialization thấp và có vấn đề security. Các serialization protocol được dùng phổ biến hơn gồm Hessian, Kryo, Protobuf, ProtoStuff; tất cả đều là binary serialization protocol.
...
Ta ít, gần như không bao giờ, trực tiếp dùng cách serialization đi kèm JDK, chủ yếu vì các nguyên nhân sau:

- **Không hỗ trợ gọi cross-language**: nếu gọi service được viết bằng ngôn ngữ khác thì không hỗ trợ.
...
- **Có vấn đề security**: bản thân serialization và deserialization không có vấn đề. Nhưng khi input của data deserialization có thể bị user kiểm soát, attacker có thể tạo input độc hại để deserialization sinh ra object ngoài dự kiến và thực thi arbitrary code được tạo ra trong quá trình này. Đọc thêm: [Application security: nỗi đau từ lỗ hổng Java deserialization](https://cryin.github.io/blog/secure-development-java-deserialization-vulnerability/).
...
IO là `Input/Output`, tức input và output. Quá trình data input vào memory của computer là input; ngược lại, quá trình output ra external storage (như database, file, remote host) là output. Quá trình truyền data tương tự dòng nước nên được gọi là IO stream. Trong Java, IO stream được chia thành input stream và output stream; theo cách xử lý data lại chia thành byte stream và character stream.

Hơn 40 class của Java IO đều được dẫn xuất từ 4 abstract base class sau.
...
Bản chất câu hỏi là: **dù đọc ghi file hay gửi nhận qua network, đơn vị lưu trữ nhỏ nhất của thông tin đều là byte, vậy tại sao I/O stream operation lại chia thành byte stream operation và character stream operation?**

Theo tôi, chủ yếu có hai nguyên nhân:

- Character stream được JVM chuyển đổi từ byte; quá trình này tương đối tốn thời gian;
- Nếu không biết encoding type, khi dùng byte stream rất dễ phát sinh lỗi mojibake.
...
**Syntactic sugar (Syntactic sugar)** là một dạng syntax đặc biệt được ngôn ngữ lập trình thiết kế để developer thuận tiện hơn khi viết chương trình; syntax này không ảnh hưởng đến chức năng của ngôn ngữ lập trình. Khi triển khai cùng một chức năng, code viết dựa trên syntactic sugar thường đơn giản, ngắn gọn và dễ đọc hơn.
...
Tuy nhiên, JVM thực ra không thể nhận diện syntactic sugar. Để được thực thi đúng, Java syntactic sugar trước hết phải được compiler desugar, tức chuyển thành syntax cơ bản mà JVM nhận diện trong giai đoạn compile chương trình. Điều này cũng cho thấy thành phần thực sự hỗ trợ syntactic sugar trong Java là Java compiler chứ không phải JVM. Nếu xem source code của `com.sun.tools.javac.main.JavaCompiler`, bạn sẽ thấy trong `compile()` có một bước gọi `desugar()`; method này chịu trách nhiệm triển khai việc desugar syntax.
