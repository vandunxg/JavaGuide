...
description: "Lộ trình học SQL và kiến thức cơ bản về database cho phỏng vấn, bao quát truy vấn SQL, lọc, sắp xếp, tổng hợp, phân nhóm, kết nối, truy vấn con, thêm sửa xoá, constraint, transaction và các câu hỏi phỏng vấn SQL thường gặp."
category: Database
tag:
  - SQL
  - Database
...
SQL là kiến thức nền tảng về database mà backend developer không thể bỏ qua. Dù học tiếp về index của MySQL, execution plan hay tối ưu SQL chậm, bạn đều cần nắm vững các khái niệm cơ bản như truy vấn, lọc, tổng hợp, kết nối, truy vấn con và thay đổi dữ liệu.

## Dành cho ai

- Backend developer đang học kiến thức cơ bản về database và cú pháp SQL.
- Người chuẩn bị cho các câu hỏi phỏng vấn về kiến thức nền tảng SQL, truy vấn SQL và CRUD database.
- Người đã viết SQL đơn giản nhưng chưa nắm chắc JOIN, GROUP BY, HAVING, truy vấn con và thứ tự thực thi.
- Kỹ sư muốn củng cố kiến thức nền tảng SQL trước khi học index của MySQL và tối ưu SQL.

## Trọng tâm học

- Nên hiểu vai trò và thứ tự thực thi của SELECT, WHERE, ORDER BY, LIMIT, GROUP BY, HAVING như thế nào?
- INNER JOIN, LEFT JOIN, RIGHT JOIN, UNION và truy vấn con phù hợp với những trường hợp sử dụng nào?
- Kết hợp hàm tổng hợp, thống kê theo nhóm và lọc theo điều kiện như thế nào?
- Cách viết INSERT, UPDATE, DELETE có những điểm nào dễ bị bỏ sót?
- Nên phân tích câu hỏi SQL trong phỏng vấn theo quan hệ giữa các bảng, điều kiện lọc, tiêu chí tổng hợp, sắp xếp và phân trang như thế nào?

## Thứ tự đọc đề xuất

1. [Tổng hợp kiến thức nền tảng cú pháp SQL](./sql-syntax-summary.md): trước tiên nắm vững cú pháp cơ bản và các thao tác thường gặp của SQL một cách có hệ thống.
...
- [Tổng hợp câu hỏi phỏng vấn SQL thường gặp (5)](./sql-questions-05.md): tiếp tục củng cố khả năng vận dụng tổng hợp vào các bài truy vấn SQL.
...
- Nên chọn truy vấn con hay JOIN trong trường hợp nào?
...
- Nên phân tích câu hỏi SQL dựa trên quan hệ giữa các bảng như thế nào?

## Chuyên đề liên quan

- [Hệ thống kiến thức về database](../)
