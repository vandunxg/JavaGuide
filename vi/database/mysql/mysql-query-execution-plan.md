...
description: Giải thích chi tiết ý nghĩa các cột trong execution plan EXPLAIN của MySQL, gồm id, select_type, type, key, rows, Extra và các trường quan trọng khác, giúp bạn phân tích bottleneck performance của SQL và tối ưu có mục tiêu.
category: Database
tag:
  - MySQL
head:
  - - meta
    - name: keywords
      content: execution plan MySQL,EXPLAIN,query optimizer,phân tích performance SQL,index hit,type access,Extra,tối ưu slow query
---

Bước đầu tiên để tối ưu SQL là đọc hiểu execution plan của SQL. Trong bài viết này, chúng ta cùng tìm hiểu kiến thức liên quan đến execution plan `EXPLAIN` của MySQL.
...
Execution plan thường được dùng trong các trường hợp như phân tích và tối ưu performance SQL. Thông qua kết quả của `EXPLAIN`, bạn có thể biết thứ tự query của các table, loại thao tác query dữ liệu, index nào có thể được sử dụng, index nào thực tế sẽ được sử dụng, mỗi table có bao nhiêu record được query và các thông tin khác.

## Lấy execution plan như thế nào?

MySQL cung cấp command `EXPLAIN` để lấy thông tin liên quan đến execution plan.

Cần lưu ý rằng câu lệnh `EXPLAIN` tiêu chuẩn không thực sự thực thi câu lệnh tương ứng, mà phân tích câu lệnh thông qua query optimizer, tìm ra phương án query tối ưu và hiển thị thông tin tương ứng.
...
Ngoài ra, `EXPLAIN FORMAT=JSON` có thể xuất dữ liệu cost model của optimizer (`query_cost`), phản ánh cost thực tế của từng bước tốt hơn dạng bảng, đặc biệt hữu ích khi tối ưu JOIN nhiều table hoặc subquery:
...
Execution plan `EXPLAIN` hỗ trợ các câu lệnh `SELECT`, `DELETE`, `INSERT`, `REPLACE` và `UPDATE`. Thông thường, chúng ta chủ yếu dùng nó để phân tích câu lệnh query `SELECT`. Cách sử dụng rất đơn giản, cú pháp như sau:

```sql
EXPLAIN SELECT query statement;
```

Hãy xem execution plan của một câu lệnh query:

**Ví dụ 1: Query một table (sử dụng index)**
...
| **Tên cột**   | **Ý nghĩa**                                                               |
| ------------- | ------------------------------------------------------------------------- |
| id            | Sequence identifier của query `SELECT`                                    |
...
| ref           | Khi query bằng giá trị index, column hoặc constant được so sánh với index |
...
Loại query, chủ yếu dùng để phân biệt query thông thường, query hợp nhất, subquery và các query phức tạp khác. Các giá trị thường gặp:
...
- **`<derivedN>`**: Row này tham chiếu đến kết quả derived table do table có id là N tạo ra. Derived table có thể được tạo từ subquery trong câu lệnh FROM.
- **`<subqueryN>`**: Row này tham chiếu đến kết quả materialized subquery do table có id là N tạo ra.
...
- **const**: Table có nhiều nhất một record khớp, có thể tìm thấy record đó chỉ bằng một query. Thường dùng khi tất cả field của primary key hoặc unique index được dùng làm điều kiện query.
...
- **ref**: Dùng ordinary index làm điều kiện query, kết quả có thể tìm thấy nhiều row phù hợp (khác với eq_ref ở chỗ một driving row có thể match nhiều driven row).
- **index_merge**: Khi mệnh đề WHERE chứa nhiều range condition và mỗi condition có thể dùng một index khác nhau, MySQL sẽ hợp nhất kết quả scan của nhiều index. Cột key liệt kê các index được sử dụng, cột Extra hiển thị thuật toán hợp nhất:

  - `Using union(...)`: Lấy union của nhiều kết quả index (điều kiện OR).
  - `Using sort_union(...)`: Sắp xếp kết quả index trước rồi lấy union (điều kiện OR, các index column không có thứ tự).
...
- **range**: Thực hiện range query trên index column; cột key trong execution plan cho biết index nào được sử dụng.
- **index**: Full Index Scan, query duyệt toàn bộ index tree. Tương tự ALL (full table scan) nhưng thường có cost thấp hơn: kích thước record trong index nhỏ hơn nhiều so với full row data, số I/O page cần đọc để lấy cùng số row ít hơn; nếu đồng thời thỏa điều kiện covering index thì còn có thể tránh đọc lại table. Tuy nhiên, trên table cực lớn (từ hàng trăm triệu row trở lên), full index scan cũng có thể tạo ra lượng I/O lớn. Không nên bỏ qua cost của nó chỉ vì cấp độ type cao hơn ALL.
- **ALL**: Full table scan.

### possible_keys

Cột possible_keys cho biết các index mà MySQL có thể sử dụng khi thực thi query. Nếu cột này là NULL, nghĩa là không có index nào có khả năng được sử dụng. Khi đó, cần kiểm tra các column được sử dụng trong câu lệnh WHERE, xem có thể cải thiện performance query bằng cách thêm index cho một hoặc nhiều column đó hay không.
...
Cột key_len cho biết độ dài tối đa của index MySQL thực tế sử dụng; khi sử dụng composite index, giá trị này có thể là tổng độ dài của nhiều column. Với điều kiện đáp ứng được nhu cầu, giá trị càng ngắn càng tốt. Nếu cột key hiển thị NULL thì cột key_len cũng hiển thị NULL.

### rows

Cột rows cho biết số row **ước tính** cần đọc để tìm record cần thiết, dựa trên thống kê table và việc lựa chọn index. Giá trị càng nhỏ càng tốt.

Cần lưu ý rằng đây là giá trị ước tính, không phải giá trị chính xác. Thống kê của InnoDB dựa trên việc random sample các index page:

- Số page được sample do `innodb_stats_persistent_sample_pages` kiểm soát (mặc định 20 page).
...
Khi execution plan không khớp với performance thực tế, có thể thực thi `ANALYZE TABLE` để sample lại, sau đó quan sát thay đổi của execution plan.

### filtered

Cột filtered cho biết tỷ lệ record **ước tính** còn lại sau khi data do storage engine trả về được lọc bằng điều kiện WHERE ở Server layer (phần trăm, 0～100). Công thức tính là: `filtered = (số row sau khi lọc điều kiện / số row do storage engine trả về) × 100`.
...
Cột này chứa thông tin bổ sung về cách MySQL parse query. Thông qua thông tin này, có thể hiểu chính xác hơn MySQL thực sự thực thi query như thế nào. Các giá trị thường gặp:
...
- **Using where**: Server layer của MySQL áp dụng filter WHERE bổ sung lên các row do storage engine trả về. Ngay cả khi đã hit index (chẳng hạn `type=ref`), nếu index chỉ đáp ứng được một phần condition query thì các condition còn lại vẫn phải được filter ở Server layer; khi đó cũng sẽ xuất hiện `Using where`.
...
Cần lưu ý rằng khi cột Extra chứa Using filesort hoặc Using temporary, performance của MySQL có thể gặp vấn đề và nên cố gắng tránh các trường hợp này.
