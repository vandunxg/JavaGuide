...
description: Giải thích chi tiết toàn bộ quy trình thực thi câu lệnh SQL trong MySQL, từ xác thực danh tính của connector, query cache, phân tích cú pháp của analyzer, optimizer tạo execution plan đến toàn bộ quá trình executor gọi storage engine.
category: Database
tag:
  - MySQL
head:
...
Trước khi phân tích, tôi sẽ cùng bạn xem qua kiến trúc cơ bản của MySQL. Biết MySQL gồm những component nào và vai trò của chúng sẽ giúp chúng ta hiểu và giải quyết các vấn đề này.
...
- **Executor:** thực thi câu lệnh, sau đó trả dữ liệu về từ storage engine. -
...
- **Server layer:** chủ yếu bao gồm connector, query cache, analyzer, optimizer, executor... Tất cả chức năng dùng chung cho các storage engine đều được triển khai ở layer này, chẳng hạn stored procedure, trigger, view, function và một module log dùng chung là binlog.
...
Sau khi connection được thiết lập, khi thực thi câu lệnh truy vấn, trước tiên sẽ kiểm tra query cache. MySQL trước tiên kiểm tra câu lệnh SQL này đã từng được thực thi hay chưa, rồi cache trong memory theo dạng key-value. Key là câu lệnh truy vấn, value là result set. Nếu cache key được hit, result set sẽ được trả trực tiếp về client. Nếu không hit, các thao tác tiếp theo sẽ được thực hiện, sau đó kết quả cũng được cache để thuận tiện cho lần gọi tiếp theo. Tất nhiên, khi thực sự thực thi truy vấn từ cache, hệ thống vẫn kiểm tra quyền của người dùng và họ có quyền truy vấn bảng đó hay không.

Không nên sử dụng query cache cho truy vấn MySQL vì trong các business scenario thực tế, query cache có thể bị invalidation rất thường xuyên. Ví dụ, nếu bạn cập nhật một bảng, toàn bộ query cache của bảng đó sẽ bị xóa. Với dữ liệu không thường xuyên cập nhật, sử dụng cache vẫn có thể chấp nhận được.

Vì vậy, trong hầu hết trường hợp, chúng tôi không khuyến nghị sử dụng query cache.

Từ MySQL 8.0, tính năng cache đã bị xóa. Phía chính thức cũng cho rằng tính năng này ít được sử dụng trong các application scenario thực tế nên đã loại bỏ hoàn toàn.

#### 3) Analyzer

Nếu MySQL không hit cache, câu lệnh sẽ đi vào analyzer. Analyzer chủ yếu dùng để phân tích câu lệnh SQL muốn thực hiện việc gì, và cũng được chia thành một số bước:
...
Vai trò của optimizer là chọn execution plan mà nó cho là tối ưu nhất để thực thi. Ví dụ, khi có nhiều index thì nên chọn index nào, khi truy vấn nhiều bảng thì nên chọn thứ tự join như thế nào...

Có thể nói, sau khi đi qua optimizer, cách thực thi cụ thể của câu lệnh đã được quyết định.

#### 5) Executor

Sau khi execution plan được chọn, MySQL chuẩn bị bắt đầu thực thi. Trước hết, hệ thống kiểm tra người dùng có quyền hay không. Nếu không có quyền, hệ thống trả về error message; nếu có quyền, hệ thống gọi API của engine và trả về kết quả thực thi từ API đó.

## II. Phân tích câu lệnh

### 2.1 Câu lệnh truy vấn

Sau khi đã nói nhiều như trên, một câu lệnh SQL thực sự được thực thi như thế nào? SQL của chúng ta có thể được chia thành hai loại: truy vấn và cập nhật (thêm, sửa, xóa). Trước tiên, hãy phân tích câu lệnh truy vấn. Câu lệnh như sau:
...
Hãy thử sửa tuổi của Trương Tam. Trong database thực tế chắc chắn sẽ không đặt field là tuổi, nếu không sẽ bị technical lead đánh. Về cơ bản, câu lệnh này cũng đi theo flow truy vấn ở trên, chỉ khác là khi thực thi cập nhật chắc chắn phải ghi log, từ đó đưa module log vào flow. Module log đi kèm MySQL là **binlog (archive log)**, mọi storage engine đều có thể sử dụng. Storage engine InnoDB thường dùng của chúng ta còn có module log **redo log (redo log)**. Hãy lấy InnoDB làm ví dụ để phân tích flow thực thi câu lệnh này. Flow như sau:

- Trước tiên tìm bản ghi của Trương Tam. Query cache sẽ không được sử dụng vì rule thiết kế của query cache là chỉ phục vụ các câu lệnh truy vấn.
...
- Sau khi nhận được thông báo, executor ghi binlog rồi xóa query cache của bảng. Xóa ở thời điểm này đảm bảo các câu lệnh SELECT tiếp theo không đọc cache cũ — vì transaction sắp được commit cuối cùng, dữ liệu sắp chuyển sang trạng thái mới nhất, thời điểm cache bị invalidation vừa khớp với thời điểm dữ liệu thực sự được cập nhật.
- Executor gọi API của engine, commit redo log sang trạng thái commit.
- Hoàn tất cập nhật.

**Chắc chắn sẽ có bạn thắc mắc: tại sao phải dùng hai module log, dùng một module log không được sao?**

Đó là vì ban đầu MySQL chưa có engine InnoDB (engine InnoDB do một công ty khác tích hợp vào MySQL dưới dạng plugin). Engine đi kèm MySQL là MyISAM. Tuy nhiên, redo log là tính năng riêng của engine InnoDB, các storage engine khác đều không có, dẫn đến không có khả năng crash-safe (khả năng crash-safe nghĩa là ngay cả khi database restart bất thường, các record đã commit trước đó cũng không bị mất). Binlog chỉ có thể dùng để archive.

Không phải chỉ dùng một module log là không được, mà engine InnoDB hỗ trợ transaction thông qua redo log. Vậy sẽ có bạn hỏi tiếp: đã dùng hai module log nhưng không làm phức tạp như vậy được không? Tại sao redo log phải có trạng thái prepare? Hãy dùng phương pháp phản chứng để giải thích vì sao phải làm như vậy.
...
- MySQL chủ yếu được chia thành Server layer và engine layer. Server layer chủ yếu bao gồm connector, query cache, analyzer, optimizer, executor, đồng thời có một module log (binlog). Module log này có thể được dùng chung bởi mọi engine, còn redo log chỉ có ở InnoDB.
- Engine layer có dạng plugin, hiện chủ yếu bao gồm MyISAM, InnoDB, Memory...
