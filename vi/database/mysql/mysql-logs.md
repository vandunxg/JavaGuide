---
title: Giải thích chi tiết về ba loại log lớn của MySQL (binlog, redo log và undo log)
description: Phân tích chuyên sâu vai trò và nguyên lý của ba loại log lớn trong MySQL là binlog, redo log và undo log, giải thích chi tiết cơ chế two-phase commit bảo đảm tính nhất quán dữ liệu, cùng ứng dụng của log trong crash recovery và master-slave replication.
category: Database
tag:
  - MySQL
head:
...
Log MySQL chủ yếu gồm một số loại: error log, query log, slow query log, transaction log và binary log. Trong đó, quan trọng hơn cả là binary log binlog (archive log), transaction log redo log (redo log) và undo log (rollback log).

![](https://oss.javaguide.cn/github/javaguide/01.png)

Hôm nay chúng ta sẽ tìm hiểu về redo log (redo log), binlog (archive log), two-phase commit và undo log (rollback log).

## redo log

redo log (redo log) là loại log chỉ có trong storage engine InnoDB, giúp MySQL có khả năng crash recovery.
...
Trong MySQL, dữ liệu được tổ chức theo page. Khi bạn truy vấn một record, hệ thống sẽ tải một page dữ liệu từ ổ đĩa, dữ liệu được tải lên này gọi là data page và được đưa vào `Buffer Pool`.

Các lần truy vấn sau sẽ tìm trong `Buffer Pool` trước; nếu không hit mới tải từ ổ đĩa, nhờ đó giảm chi phí IO ổ đĩa và cải thiện performance.
...
Sau đó, thao tác “đã thực hiện thay đổi gì trên một data page nào đó” sẽ được ghi vào redo log buffer (`redo log buffer`), rồi flush xuống file redo log.
...
Trong trường hợp lý tưởng, transaction sẽ flush ngay khi commit, nhưng trên thực tế, thời điểm flush phụ thuộc vào strategy.

> Mẹo nhỏ: mỗi record redo gồm “số tablespace + số data page + offset + độ dài dữ liệu thay đổi + dữ liệu thay đổi cụ thể”.

### Thời điểm flush

Trong storage engine InnoDB, **redo log buffer** (redo log buffer) là một vùng memory dùng để tạm lưu redo log. Để bảo đảm tính bền vững của transaction và tính nhất quán của dữ liệu, InnoDB sẽ flush dữ liệu log trong buffer này xuống file redo log trên disk vào những thời điểm nhất định. Các thời điểm đó có thể được tổng hợp thành sáu trường hợp sau:
...
2. **Khi không đủ dung lượng trong redo log buffer**: đây là strategy quản lý capacity chủ động của InnoDB, nhằm tránh việc user thread bị block do buffer đầy.
   - Khi dung lượng đã dùng của redo log buffer vượt quá **một nửa (50%)** tổng capacity, background thread sẽ **chủ động** flush phần log này xuống disk để giải phóng dung lượng cho các lần ghi log tiếp theo, đây là một optimization “phòng xa”.
   - Nếu buffer bị **ghi đầy hoàn toàn** do transaction lớn hoặc I/O bận, mọi user thread cố ghi log mới sẽ bị **block**, đồng thời bắt buộc thực hiện một lần flush đồng bộ cho đến khi có dung lượng trống. Tình huống này ảnh hưởng đến performance database và nên được hạn chế.
...
4. **Background thread flush định kỳ**: InnoDB có một master thread chạy background, khoảng mỗi giây thực hiện một lần các task định kỳ, trong đó có flush log trong redo log buffer xuống disk. Cơ chế này là biện pháp bảo đảm persistence chủ yếu khi `innodb_flush_log_at_trx_commit` được đặt là 0 hoặc 2.
...
6. **Khi binlog chuyển file**: khi binlog được bật, trong cấu hình double-one `innodb_flush_log_at_trx_commit=1` và `sync_binlog=1` của MySQL, để bảo đảm state giữa redo log và binlog nhất quán (dùng cho crash recovery hoặc master-slave replication), khi file binlog đầy hoặc khi chuyển file bằng cách thực hiện thủ công `flush logs`, thao tác flush redo log sẽ được trigger.

Tóm lại, InnoDB sẽ flush redo log trong nhiều tình huống để bảo đảm tính bền vững và tính nhất quán của dữ liệu.

Chúng ta cần chú ý thiết lập strategy flush đúng cho `innodb_flush_log_at_trx_commit`. Tùy strategy flush được cấu hình cho MySQL, sau khi MySQL dừng đột ngột có thể xảy ra mất dữ liệu nhẹ.

`innodb_flush_log_at_trx_commit` có 3 giá trị, tương ứng với 3 strategy flush:

- **0**: khi đặt là 0, mỗi lần commit transaction sẽ không thực hiện flush. Cách này có performance cao nhất nhưng cũng kém an toàn nhất, vì nếu MySQL gặp lỗi hoặc dừng đột ngột, transaction trong 1 giây gần nhất có thể bị mất.
...
File redo log được lưu trên disk không chỉ có một file mà xuất hiện dưới dạng một **log file group**; mỗi file `redo` có cùng kích thước.

Ví dụ, có thể cấu hình một group gồm `4` file, mỗi file có kích thước `1GB`, khi đó toàn bộ log file group redo log có thể ghi `4G` dữ liệu.
...
Trong **log file group** này còn có hai property quan trọng là `write pos` và `checkpoint`.

- **write pos** là vị trí hiện tại đang ghi, vừa ghi vừa dịch về phía sau.
...
Về thay đổi này, chúng ta có thể kiểm chứng như sau.

Trước tiên, tạo một file cấu hình, trong đó cấu hình giá trị của `innodb_log_files_in_group` và `innodb_log_file_size`:
...
Điều này cũng cho thấy hai variable `innodb_log_files_in_group` và `innodb_log_file_size` được dùng để tính `innodb_redo_log_capacity`, đồng thời đã bị deprecated.
...
Tin rằng mọi người đều đã biết vai trò, thời điểm flush và hình thức lưu trữ của redo log.
...
Nếu ghi redo log, một record có thể chỉ chiếm vài chục `Byte`, chỉ bao gồm số tablespace, số data page, offset trong file trên disk và giá trị update, hơn nữa còn là sequential write nên tốc độ flush rất nhanh.
...
> Thực ra data page trong memory cũng được flush vào những thời điểm nhất định, chúng ta gọi là page merge. Phần này sẽ được giải thích chi tiết khi nói về `Buffer Pool`.
...
Bất kể dùng storage engine nào, chỉ cần xảy ra update dữ liệu trong table thì đều tạo ra log binlog.

Vậy binlog rốt cuộc dùng để làm gì?

Có thể nói **data backup, master-backup, master-master và master-slave** của database MySQL đều không thể thiếu binlog; cần dựa vào binlog để đồng bộ dữ liệu và bảo đảm tính nhất quán của dữ liệu.

![](https://oss.javaguide.cn/github/javaguide/01-20220305234724956.png)

binlog ghi lại mọi thao tác logic liên quan đến update dữ liệu và thực hiện sequential write.

### Format record

Log binlog có ba format, có thể chỉ định bằng parameter `binlog_format`.
...
Khi đồng bộ dữ liệu, hệ thống sẽ thực thi câu lệnh `SQL` được ghi lại. Nhưng có một vấn đề: `update_time=now()` sẽ lấy thời gian hiện tại của system; thực thi trực tiếp sẽ khiến dữ liệu không nhất quán với database gốc.
...
Tuy nhiên, format này cần dung lượng lớn hơn để ghi dữ liệu, chiếm nhiều space hơn; khi recovery và đồng bộ cũng tiêu tốn nhiều tài nguyên IO hơn, ảnh hưởng đến execution speed.

Vì vậy mới có phương án trung gian là chỉ định `mixed`, trong đó nội dung ghi lại là sự kết hợp của hai format trước.

MySQL sẽ phán đoán câu lệnh `SQL` này có thể gây ra dữ liệu không nhất quán hay không. Nếu có, nó dùng format `row`; nếu không, nó dùng format `statement`.

### Cơ chế ghi

Thời điểm ghi binlog cũng rất đơn giản: trong quá trình transaction thực thi, trước tiên log được ghi vào `binlog cache`; khi transaction commit, `binlog cache` mới được ghi vào file binlog.
...
- **`write` trong hình trên nghĩa là ghi log vào page cache của filesystem, chưa persistence dữ liệu xuống disk nên tốc độ khá nhanh.**
- **`fsync` trong hình trên mới là thao tác persistence dữ liệu xuống disk.**
...
Để an toàn, có thể đặt là `1`, nghĩa là mỗi lần commit transaction đều thực hiện `fsync`, giống với **quy trình flush log redo log**.

Cuối cùng còn một cách trung gian khác: có thể đặt là `N(N>1)`, nghĩa là mỗi lần commit transaction đều `write`, nhưng chỉ `fsync` sau khi tích lũy `N` transaction.
...
Tương tự, nếu máy dừng đột ngột thì log binlog của `N` transaction gần nhất sẽ bị mất.
...
Quay lại vấn đề chính, nếu logic giữa hai log redo log và binlog không nhất quán thì sẽ xảy ra vấn đề gì?
...
Giả sử trong quá trình thực thi, sau khi ghi xong redo log thì xảy ra lỗi trong lúc ghi log binlog. Tình huống sẽ thế nào?

![](https://oss.javaguide.cn/github/javaguide/02-20220305234828662.png)

Vì xảy ra lỗi trước khi ghi xong binlog, nên binlog không có record thay đổi tương ứng. Do đó, sau này khi dùng log binlog để recovery dữ liệu, lần update này sẽ bị thiếu; giá trị `c` của record được recovery là `0`, trong khi database gốc khôi phục bằng redo log lại có giá trị `c` là `1`, cuối cùng dữ liệu không nhất quán.
...
Xét một tình huống khác: nếu xảy ra lỗi khi redo log được đặt sang phase `commit` thì transaction có rollback không?
...
Mọi thay đổi dữ liệu của mỗi transaction đều được ghi vào undo log. Nếu xảy ra lỗi trong quá trình thực thi transaction hoặc cần thực hiện rollback, MySQL có thể dùng undo log để khôi phục dữ liệu về state trước khi transaction bắt đầu.
...
undo log được ghi theo dạng segment (segment). Mỗi thao tác undo khi được ghi sẽ chiếm một **undo log segment** (undo log segment), undo log segment nằm trong **rollback segment** (rollback segment). Khi transaction bắt đầu, hệ thống cần cấp phát một rollback segment cho nó. Mỗi rollback segment có 1024 undo log segment, giúp quản lý nhu cầu rollback của nhiều transaction concurrency.
...
Ngoài ra, implementation của **MVCC** phụ thuộc vào **hidden field, Read View và undo log**. Trong implementation nội bộ, InnoDB dựa vào `DB_TRX_ID` của row dữ liệu và `Read View` để phán đoán visibility của dữ liệu. Nếu không visible, InnoDB tìm version lịch sử trong undo log thông qua `DB_ROLL_PTR` của row dữ liệu. Version dữ liệu mà mỗi transaction đọc được có thể khác nhau. Trong cùng một transaction, user chỉ có thể nhìn thấy các thay đổi đã commit trước khi transaction đó tạo `Read View` và các thay đổi do chính transaction đó thực hiện.

## Tổng kết

> Phần nội dung này do JavaGuide bổ sung:

MySQL InnoDB engine sử dụng **redo log (redo log)** để bảo đảm **persistence** của transaction, sử dụng **undo log (rollback log)** để bảo đảm **atomicity** của transaction.

**Data backup, master-backup, master-master và master-slave** của database MySQL đều không thể thiếu binlog; cần dựa vào binlog để đồng bộ dữ liệu và bảo đảm tính nhất quán của dữ liệu.
