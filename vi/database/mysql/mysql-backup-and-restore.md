...
Sự cố database đáng sợ nhất trong production nhiều khi không phải là process MySQL bị dừng.

Process bị dừng còn có thể restart, primary database bị dừng còn có thể chuyển sang replica. Điều thực sự rắc rối là dữ liệu bị xóa, bị script sai sửa nhầm, disk bị hỏng hoặc phát hiện thiếu một nhóm table trong lúc migration. Khi đó, master-slave replication, redo log và undo log đều không đủ dùng; thứ thường có thể xử lý tình huống cuối cùng là backup và restore.
...
- **Crash recovery**: Sau khi MySQL shutdown bất thường, InnoDB dựa vào redo log, undo log và các cơ chế khác để khôi phục database về trạng thái nhất quán. Đây là vấn đề nhất quán của storage engine sau khi process crash hoặc máy restart.
- **Master-slave replication / chuyển đổi high availability**: Khi primary database không khả dụng, chuyển traffic sang replica hoặc primary database mới. Đây là vấn đề service availability, nhưng các write sai thường cũng được đồng bộ sang replica.
- **Backup và restore**: Khôi phục từ một bản sao dữ liệu trong lịch sử, sau đó replay binlog đến một thời điểm cụ thể. Đây là cách xử lý việc mất dữ liệu, xóa hoặc sửa nhầm, disk hỏng, migration giữa các môi trường và lưu trữ phục vụ audit.
...
Nếu business có thể chấp nhận mất dữ liệu 1 ngày, full backup mỗi ngày một lần có lẽ là đủ. Nếu dữ liệu order, payment, inventory chỉ được phép mất tối đa vài phút, chỉ full backup là chưa đủ; cần giữ lại binlog để restore tăng dần. Nếu database có vài trăm GB, việc restore file SQL có thể chạy rất lâu, trong khi RTO yêu cầu khôi phục trong 30 phút, thì cần nghiêm túc cân nhắc physical backup, warm-up replica, diễn tập restore và quy trình chuyển đổi.
...
Theo việc database có cung cấp service trong lúc backup hay không:
...
`mysqldump` là logical backup tool đi kèm MySQL. Nó export một loạt câu lệnh SQL, có thể dùng các câu lệnh này để dựng lại schema và dữ liệu của database, table. Ưu điểm của nó là đơn giản, phổ biến, dễ xem và cũng phù hợp với migration giữa các môi trường. Nhược điểm cũng rất rõ: khi data volume lớn, backup chậm và restore còn chậm hơn, vì quá trình restore phải thực thi lại SQL, ghi dữ liệu và tạo index.
...
- `--source-data=2`: Ghi tên và position của file binlog hiện tại vào file dump, đồng thời giữ lại dưới dạng SQL comment. `--master-data` thường gặp trong tài liệu cũ hiện đã là alias deprecated của `--source-data` trong tài liệu mới.
...
`mysqldump` có một số vấn đề thường gặp:
...
Với database nhỏ, môi trường test, migration khác version hoặc export một lượng dữ liệu nhỏ, `mysqldump` rất tiện. Nếu production database hàng trăm GB vẫn chỉ dựa vào nó để restore, thời gian restore thường sẽ khiến mọi người khó chịu.

## MySQL Shell Dump Utilities: logical backup song song

Nếu muốn giữ khả năng migrate của logical backup nhưng thấy export và import đơn thread của `mysqldump` quá chậm, có thể xem xét MySQL Shell Dump Utilities.
...
Nó vẫn được định vị là logical backup, nhưng hỗ trợ dump/load song song, compression, thông tin tiến độ, checksum và output đến object storage. Trong tài liệu chính thức, giá trị mặc định của `threads` là 4; có thể tăng tùy theo tải của instance, network và khả năng ghi của đầu đích.
...
binlog ghi lại các event có thể thay đổi dữ liệu, đồng thời là nền tảng của master-slave replication và point-in-time restore. PITR (Point-in-Time Recovery) thường bắt đầu bằng việc restore một full backup, sau đó replay log từ binlog position tương ứng với backup cho đến thời gian hoặc position đích.
...
`mysql --binary-mode` không phải parameter trang trí. Tài liệu chính thức đề cập rằng nếu output binlog chứa ký tự null như `\0`, không thêm `--binary-mode` có thể khiến client `mysql` không parse đúng.
...
Command này sẽ lấy binlog ở định dạng binary raw và tiếp tục chờ event mới sau khi đến cuối file log mới nhất hiện tại. Nó khác với replica: khi connection bị ngắt, nó không tự reconnect như replica, vì vậy production script còn phải kết hợp process supervisor, alert và resume từ điểm dừng.
...
Thứ tự prepare, cách merge và các bước restore của incremental chain dễ viết sai hơn. Trước khi team diễn tập thành thạo, không nên ngay lập tức đặt năng lực restore vào một incremental chain phức tạp. Cách chắc chắn hơn là bảo đảm trước tiên “physical full backup + binlog” có thể restore, sau đó tùy theo data volume và áp lực về window mà bổ sung incremental backup.
...
Không cần thần bí hóa việc lựa chọn tool. Database nhỏ dùng `mysqldump` không có vấn đề gì; script đơn giản và cũng dễ điều tra khi có lỗi. Khi data volume tăng, vấn đề restore file SQL chậm sẽ ngày càng rõ, lúc đó chuyển sang physical backup thực tế hơn.

Phương án tệ nhất thường là chỉ thực hiện một loại backup và không bao giờ verify restore.

## Nên diễn tập restore thế nào?

Backup script chạy thành công chỉ chứng minh đã tạo ra file. File có dùng được hay không phải nhờ restore drill trả lời.
...
5. Verify số lượng database, table quan trọng, SQL nghiệp vụ quan trọng, stored procedure, event, trigger, account, role và permission.
...
7. Dùng application connection vào restore instance và chạy một nhóm read-only smoke API.
...
Replica có thể tiếp nhận traffic read/write, nhưng không ngăn được việc xóa hoặc sửa nhầm. Sau khi SQL sai được đồng bộ sang, replica cũng chuyển thành trạng thái sai. Replica có delay có thể tranh thủ thêm một chút thời gian xử lý, nhưng vẫn không thể thay thế offline backup.
...
Khi disk hỏng, phòng máy gặp sự cố hoặc directory bị xóa nhầm, data và backup có thể cùng biến mất. Ít nhất phải copy sang storage độc lập; với business quan trọng còn cần cân nhắc khác phòng máy hoặc object storage.
...
Backup không có restore drill thường trông như phương án ít tốn công nhất, nhưng lại đắt nhất khi có sự cố. Các bước restore càng lâu không chạy thì càng dễ bị chặn bởi version, permission, path, disk space và parameter của tool.

**Hiểu lầm 6: Checksum pass nghĩa là restore được.**
...
Nếu backup dùng chung permission với production account, hoặc script vận hành thông thường có thể trực tiếp overwrite, delete, thì khi gặp xóa nhầm, ransomware hay script bug, backup có thể cùng mất hiệu lực. Business quan trọng ít nhất phải có một bản copy cross-account, cross-failure-domain, có version hoặc immutable policy, đồng thời hạn chế permission delete.
...
- Sau khi backup được ghi xuống disk, tính checksum rồi copy sang storage độc lập; business quan trọng nên giữ thêm một bản copy encryption, cross-account, có version hoặc immutable policy.
