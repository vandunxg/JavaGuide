...
description: "Giải thích chi tiết nguyên lý của character set và character encoding, phân tích sâu các character encoding thường gặp như ASCII, GB2312, GBK, UTF-8, UTF-16, giải thích sự khác biệt giữa utf8 và utf8mb4 trong MySQL cùng giải pháp lưu trữ emoji."
category: Database
tag:
  - Database Basics
head:
...
Trong MySQL có hai cách triển khai UTF-8: **`utf8`** và **`utf8mb4`**.

Nếu dùng **`utf8`**, việc lưu ký hiệu emoji, một số chữ phức tạp và chữ phồn thể sẽ xảy ra lỗi.

Tại sao lại như vậy? Bài viết này sẽ giải thích vấn đề từ gốc rễ.

## Character set là gì?

Character là tên gọi chung cho chữ và ký hiệu, bao gồm chữ viết của các quốc gia, dấu câu, biểu tượng cảm xúc, chữ số, v.v. **Character set** là một tập hợp các character. Có khá nhiều loại character set; phạm vi character mà mỗi character set biểu diễn được cũng khác nhau. Chẳng hạn, một số character set không thể biểu diễn chữ Hán.
...
ASCII character set đã định nghĩa tổng cộng 128 character, trong đó có 33 control character (chẳng hạn carriage return, ký tự xóa) không thể hiển thị.

Độ dài một mã ASCII là một byte, tức 8 bit. Ví dụ, mã ASCII tương ứng với “a” là “01100001”. Tuy nhiên, bit cao nhất bằng 0 chỉ được dùng làm parity bit, 7 bit còn lại được tổ hợp từ 0 và 1. Vì vậy, ASCII character set có thể định nghĩa 128 (2^7) character.
...
GB2312 character set là một character set khá phù hợp với chữ Hán, bao gồm hơn 6.700 chữ Hán, về cơ bản bao quát phần lớn chữ Hán thường dùng. Tuy nhiên, GB2312 character set không hỗ trợ phần lớn chữ hiếm gặp và chữ phồn thể.

Với character tiếng Anh, GB2312 encoding giống ASCII code và chỉ cần 1 byte để encode. Với character không phải tiếng Anh, cần 2 byte để encode.

### GBK

GBK character set có thể xem là phần mở rộng của GB2312 character set và tương thích với GB2312 character set, bao gồm hơn 20.000 chữ Hán.

Chữ K trong GBK là chữ cái đầu của “Kuo” trong Hanyu Pinyin Kuo Zhan (mở rộng).

### GB18030

GB18030 hoàn toàn tương thích với GB2312 và GBK character set, bổ sung chữ viết của một số dân tộc thiểu số ở Trung Quốc và chữ Hán của Nhật Bản, Hàn Quốc. Đây là character set hỗ trợ chữ Hán toàn diện nhất hiện nay, bao gồm hơn 70.000 chữ Hán.

### BIG5

BIG5 chủ yếu dành cho tiếng Trung phồn thể, bao gồm hơn 13.000 chữ Hán.

### Unicode & UTF-8

Để phù hợp hơn với ngôn ngữ của mỗi quốc gia, nhiều loại character set đã ra đời.

Như đã nói ở trên, các character set khác nhau có phạm vi character biểu diễn và quy tắc encoding khác nhau. Điều này dẫn đến một vấn đề rất nghiêm trọng: **mở một file chứa character bằng encoding không phù hợp sẽ tạo ra hiện tượng mojibake**.

Ví dụ, nếu mở file có encoding GB2312 bằng UTF-8 encoding, file sẽ xuất hiện mojibake. Cụ thể, character mang nghĩa “bò” có giá trị hệ thập lục phân sau khi được encode bằng GB2312 là “C5A3”, nhưng khi “C5A3” được decode bằng UTF-8, kết quả lại là “ţ”.
...
UTF-8 dùng từ 1 đến 4 byte để encode mỗi character, UTF-16 dùng 2 hoặc 4 byte để encode mỗi character, còn UTF-32 luôn dùng 4 byte để encode mỗi character.
...
Ở trên đã đề cập các cấp độ của character set, vốn liên quan đến việc lưu trữ. Connection character set lại liên quan đến việc giao tiếp với MySQL server.
...
- `character_set_connection`: mô tả character set được server dùng để chuyển đổi câu lệnh SQL sau khi nhận được.
- `character_set_results`: mô tả character set của kết quả mà server trả về client.
...
Thông thường, nên dùng UTF-8 làm character encoding mặc định.

Tuy nhiên, có một điểm dễ nhầm ở đây.

Trong MySQL có hai cách triển khai UTF-8:

- **`utf8`**: `utf8` chỉ hỗ trợ `1-3` byte cho mỗi character. Trong `utf8` encoding, chữ Hán chiếm 3 byte, còn chữ số, chữ tiếng Anh và ký hiệu chiếm 1 byte. Tuy nhiên, ký hiệu emoji chiếm 4 byte, một số chữ phức tạp và chữ phồn thể cũng chiếm 4 byte.
- **`utf8mb4`**: triển khai đầy đủ UTF-8, phiên bản hoàn chỉnh! Hỗ trợ tối đa 4 byte để biểu diễn một character, nên có thể dùng để lưu ký hiệu emoji.
...
Cùng xem ví dụ (môi trường: MySQL 5.7+).
