...
description: "Tổng hợp câu hỏi phỏng vấn Redis mới nhất (Phần 1): giải thích chuyên sâu về Redis cơ bản, năm data structure thường dùng, nguyên lý single-thread model, cơ chế persistence, chính sách memory eviction và expiration, distributed lock và cách triển khai message queue. Phù hợp với developer chuẩn bị phỏng vấn backend!"
category: Database
tag:
  - Redis
head:
...
Để đáp ứng các business scenario khác nhau, Redis tích hợp nhiều data type (ví dụ String, Hash, Sorted Set, Bitmap, HyperLogLog, GEO). Redis cũng hỗ trợ transaction, persistence, Lua script, publish-subscribe model và nhiều cluster solution dùng ngay (Redis Sentinel, Redis Cluster).
...
Redis không có external dependency. Linux và OS X là hai operating system được dùng nhiều nhất để phát triển và test Redis; chính thức khuyến nghị deploy Redis trong production trên Linux.
...
Redis đã thực hiện rất nhiều performance optimization nội bộ, trong đó 4 điểm quan trọng là:

1. **Pure memory operation (Memory-Based Storage)**: Đây là nguyên nhân chính. Mọi thao tác đọc ghi dữ liệu Redis đều diễn ra trong memory, tốc độ truy cập ở cấp nanosecond, trong khi tốc độ database truyền thống thường xuyên đọc ghi disk ở cấp millisecond; hai bên chênh nhau vài bậc độ lớn.
2. **I/O model hiệu quả (I/O Multiplexing & Single-Threaded Event Loop)**: Redis sử dụng event loop single-thread kết hợp kỹ thuật I/O multiplexing, cho phép một thread đồng thời xử lý nhiều I/O event trên nhiều network connection (như đọc ghi), tránh context switch và lock contention trong multi-thread model. Dù là single-thread, nhờ hiệu quả của memory operation và I/O multiplexing, Redis vẫn dễ dàng xử lý lượng lớn concurrent request (Redis thread model sẽ được giới thiệu chi tiết ở phần sau).
...
Vậy nếu đã nhanh như thế, tại sao không dùng Redis trực tiếp làm primary database? Chủ yếu vì chi phí memory quá cao, đồng thời persistence mà Redis cung cấp vẫn có rủi ro mất data.

### Ngoài Redis, bạn còn biết distributed cache solution nào khác không?
...
Nếu bạn biết các solution khác và có thể giải thích vì sao cuối cùng chọn Redis (tiến thêm một bước nữa!), điều này sẽ giúp phần thể hiện trong phỏng vấn được cộng điểm đáng kể!

Dưới đây là trao đổi ngắn về việc chọn distributed cache technology phổ biến.

Trong distributed cache, các technology lâu đời và được sử dụng nhiều vẫn là **Memcached** và **Redis**. Tuy nhiên hiện nay hầu như không còn thấy project nào dùng **Memcached** làm cache, mà đều dùng trực tiếp **Redis**.

Memcached khá phổ biến khi distributed cache mới bắt đầu phát triển. Sau đó, cùng với sự phát triển của Redis, mọi người dần chuyển sang Redis mạnh mẽ hơn.

Một số công ty lớn cũng open source distributed high-performance KV storage database tương tự Redis, ví dụ [**Tendis**](https://github.com/Tencent/Tendis) do Tencent open source. Tendis sử dụng project mã nguồn mở nổi tiếng [RocksDB](https://github.com/facebook/rocksdb) làm storage engine, tương thích 100% với Redis protocol và mọi data model của Redis 4.0. Về so sánh Redis và Tendis, Tencent từng đăng một bài viết: [Redis vs Tendis: Giải mã architecture của cold-hot mixed storage](https://mp.weixin.qq.com/s/MeYkfOIdnU6LYlsGb24KjQ), có thể tham khảo sơ lược.

Tuy nhiên, từ commit record trên Github của project Tendis có thể thấy bản open source của Tendis hầu như đã không còn được maintain và update. Thêm vào đó, mức độ quan tâm không cao, số công ty sử dụng cũng ít. Vì vậy, không khuyến nghị dùng Tendis để triển khai distributed cache.

Hiện tại, hai distributed cache open source được industry công nhận hơn để thay thế Redis là:
...
Tuy nhiên, cá nhân tôi vẫn khuyên ưu tiên Redis cho distributed cache. Dù sao Redis đã được kiểm chứng qua nhiều năm, ecosystem rất tốt và tài liệu cũng đầy đủ!

PS: Do giới hạn dung lượng bài viết, tôi không giới thiệu và so sánh chi tiết các lựa chọn distributed cache nêu trên. Nếu quan tâm, bạn có thể tự nghiên cứu thêm.

### Hãy nói về điểm khác biệt và điểm chung giữa Redis và Memcached

Hiện nay các công ty thường dùng Redis để triển khai cache, bản thân Redis cũng ngày càng mạnh hơn! Tuy nhiên, hiểu điểm khác biệt và điểm chung giữa Redis và Memcached giúp chúng ta có cơ sở rõ ràng khi thực hiện technical selection tương ứng!
...
1. **Data type**: Redis hỗ trợ data type phong phú hơn (hỗ trợ application scenario phức tạp hơn). Redis không chỉ hỗ trợ data type k/v đơn giản mà còn cung cấp storage cho các data structure như list, set, zset, hash; còn Memcached chỉ hỗ trợ data type k/v đơn giản nhất.
...
4. **Thread model**: Memcached là network model multi-thread, non-blocking I/O multiplexing; Redis sử dụng model multi-way I/O multiplexing single-thread (Redis 6.0 đã thêm multi-thread cho đọc ghi network data).
...
6. **Xóa data hết hạn**: Policy xóa expired data của Memcached chỉ dùng lazy deletion, còn Redis đồng thời dùng lazy deletion và scheduled deletion.
...
Data của database truyền thống được lưu trên disk, còn Redis dựa trên memory; tốc độ truy cập memory nhanh hơn disk rất nhiều. Sau khi đưa Redis vào, chúng ta có thể đặt một số data được truy cập thường xuyên vào Redis, lần sau đọc trực tiếp từ memory, tốc độ có thể tăng vài chục, thậm chí hơn một trăm lần.

**2. High concurrency**

Thông thường database như MySQL có QPS khoảng 4k (4 core 8g), nhưng sau khi dùng Redis cache rất dễ đạt 5w+, thậm chí 10w+ (trong trường hợp Redis đơn máy; Redis cluster còn cao hơn).
...
Ngoài làm cache, Redis còn có thể dùng cho distributed lock, rate limiting, message queue, delayed queue và các scenario khác; feature rất mạnh!

### ⭐️Tại sao dùng Redis thay vì local cache?

| Feature              | Local cache                                                     | Redis                                                     |
| -------------------- | --------------------------------------------------------------- | --------------------------------------------------------- |
...
Mỗi người đều có thể dựa trên Redis để custom development Module của riêng mình, ví dụ triển khai search engine, custom distributed lock và distributed rate limiting.
...
Về giới thiệu chi tiết Redis module, có thể xem official document: <https://redis.io/modules>.
...
Đây là một câu hỏi khá quan trọng, cũng có thể áp dụng cho technical selection. Tôi đã viết riêng một bài giới thiệu và phân tích chi tiết, khuyến nghị bạn có thời gian thì đọc kỹ vài lần và bookmark: [Redis có thể làm message queue không? Triển khai thế nào?](https://javaguide.cn/database/redis/redis-stream-mq.html).
...
Chức năng delayed task dựa trên Redis về cơ bản chỉ có hai solution sau:
...
1. **Giảm khả năng mất message**: Message trong DelayedQueue sẽ được persist. Ngay cả khi Redis down, theo cơ chế persistence cũng chỉ có thể mất một ít message, ảnh hưởng không lớn. Dĩ nhiên, bạn cũng có thể dùng cách scan database làm compensation mechanism.
...
Về giới thiệu chi tiết 5 basic data type và 3 special data type của Redis, hãy xem hai bài viết dưới đây và [Redis official document](https://redis.io/docs/data-types/):
...
- **5 basic data type**: String, List, Set, Hash, Zset.
- **3 special data type**: HyperLogLog, Bitmap, Geospatial.

Ngoài các type nêu trên, còn có một số type khác như [Bloom filter](https://javaguide.cn/cs-basics/data-structure/bloom-filter.html), Bitfield.
...
### Implementation bên dưới của String là gì?

Redis được viết bằng ngôn ngữ C, nhưng implementation bên dưới của String type trong Redis không phải là string trong C (tức character array kết thúc bằng null character `\0`), mà là [SDS](https://github.com/antirez/sds) (Simple Dynamic String) do Redis tự viết.
...
Từ source code có thể thấy SDS có năm implementation: SDS_TYPE_5 (không được sử dụng), SDS_TYPE_8, SDS_TYPE_16, SDS_TYPE_32, SDS_TYPE_64; chỉ bốn loại sau được sử dụng thực tế. Redis quyết định sử dụng type nào dựa trên length khởi tạo, từ đó giảm memory usage.
...
Bốn implementation sau đều chứa 4 property:

- `len`: length của string, tức số byte đã sử dụng.
...
### Lưu thông tin shopping cart bằng String hay Hash thì tốt hơn?

Vì sản phẩm trong shopping cart thường xuyên được sửa đổi và thay đổi, thông tin shopping cart được khuyến nghị lưu bằng Hash:

- user id là key
- product id là field, số lượng sản phẩm là value

![Duy trì shopping cart đơn giản bằng Hash](https://oss.javaguide.cn/github/javaguide/database/redis/hash-shopping-cart.png)

Vậy cụ thể phải duy trì thông tin shopping cart của user như thế nào?

- User thêm sản phẩm tức là thêm field và value mới vào Hash;
- Query thông tin shopping cart tức là duyệt Hash tương ứng;
...
- Xóa toàn bộ shopping cart tức là xóa key tương ứng.

Đây chỉ là ví dụ trong shopping cart scenario tương đối đơn giản; trong e-commerce scenario thực tế, chỉ lưu product id vào field là không đủ đáp ứng yêu cầu.

### Triển khai ranking bằng Redis như thế nào?

Redis có một data type tên là `Sorted Set` thường được dùng trong nhiều ranking scenario, ví dụ ranking tặng quà trong livestream, ranking số bước chân WeChat trong friend circle, ranking tier trong Honor of Kings, ranking độ hot của topic, v.v.

Một số Redis command liên quan: `ZRANGE` (sort từ nhỏ đến lớn), `ZREVRANGE` (sort từ lớn đến nhỏ), `ZREVRANK` (ranking của element được chỉ định).

![](https://oss.javaguide.cn/github/javaguide/database/redis/2021060714195385.png)

Trong [《Java Interview Guide》](https://javaguide.cn/zhuanlan/java-mian-shi-zhi-bei.html), phần “Technical Interview Questions” có một bài viết giới thiệu chi tiết cách dùng Sorted Set để thiết kế và tạo ranking; nếu quan tâm bạn có thể xem.

![](https://oss.javaguide.cn/github/javaguide/database/redis/image-20220719071115140.png)

### Vì sao implementation bên dưới của Redis sorted set dùng skiplist thay vì balanced tree, red-black tree hoặc B+ tree?
...
Application scenario phổ biến của `Set`:
...
Bitmap lưu các số binary liên tiếp (0 và 1). Thông qua Bitmap, chỉ cần một bit để biểu thị value hoặc state tương ứng của một element, key chính là element tương ứng. Ta biết 8 bit tạo thành một byte, vì vậy bản thân Bitmap tiết kiệm storage space rất lớn.

Bạn có thể xem Bitmap như một array lưu các số binary (0 và 1), index của mỗi element trong array gọi là offset.
...
HyperLogLog (HLL) là một probabilistic data structure rất tinh tế, chuyên giải quyết một loại big data problem khó: dùng memory cực nhỏ để ước tính số element không trùng trong một set giữa lượng data khổng lồ, tức cardinality (Cardinality) thường được nhắc đến.
...
- **Thống kê UV (Unique Visitor) của website/app:** Ví dụ thống kê mỗi ngày có bao nhiêu IP hoặc user ID khác nhau đã truy cập homepage.
...
Cuối cùng, implementation của Redis cũng rất thông minh: bên trong sẽ tự động chuyển đổi giữa **sparse matrix** (chiếm ít space hơn) và **dense matrix** (cố định 12KB) theo cardinality, tiếp tục tối ưu memory usage. Tóm lại, khi cần deduplicate count trên lượng data khổng lồ và có thể chấp nhận sai số nhỏ, HyperLogLog là lựa chọn không thể thay thế.
...
## ⭐️Cơ chế Redis persistence (quan trọng)

Có khá nhiều câu hỏi liên quan đến Redis persistence mechanism (RDB persistence, AOF persistence, hybrid persistence giữa RDB và AOF), đồng thời cũng khá quan trọng. Vì vậy tôi đã tách riêng một bài để tổng hợp knowledge point và câu hỏi về Redis persistence mechanism: [Giải thích chi tiết Redis persistence mechanism](https://javaguide.cn/database/redis/redis-persistence.html).

## ⭐️Redis thread model (quan trọng)

Đối với read/write command, Redis luôn sử dụng single-thread model. Tuy nhiên sau Redis 4.0 đã thêm multi-thread để thực hiện một số thao tác async delete trên large key-value pair; từ Redis 6.0 đã thêm multi-thread để xử lý network request (tăng network I/O read/write performance).

### Bạn có hiểu Redis single-thread model không?

**Redis dựa trên Reactor pattern để thiết kế và phát triển một event processing model hiệu quả** (thread model của Netty cũng dựa trên Reactor pattern; Reactor pattern quả không hổ danh là nền tảng của high-performance I/O). Event processing model này tương ứng với file event handler trong Redis. Vì file event handler chạy theo cách single-thread nên chúng ta thường nói Redis là single-thread model.
...
>
> - File event handler dùng chương trình I/O multiplexing để đồng thời lắng nghe nhiều socket, đồng thời liên kết các event handler khác nhau với socket theo task mà socket hiện đang thực hiện.
...
Redis dùng **I/O multiplexing program** để lắng nghe lượng lớn connection từ client (hay nói cách khác là lắng nghe nhiều socket), đăng ký event và type được quan tâm (read, write) vào kernel rồi lắng nghe xem từng event có xảy ra hay không.

Lợi ích rất rõ ràng: **việc sử dụng I/O multiplexing giúp Redis không cần tạo thêm thread để lắng nghe lượng lớn connection của client, giảm resource consumption** (khá giống component `Selector` trong NIO).
...
Dù nói Redis là single-thread model, nhưng trên thực tế **từ các version sau Redis 4.0, Redis đã thêm support cho multi-thread.**
...
Dù Redis 6.0 thêm multi-thread, multi-thread của Redis chỉ được dùng cho các operation tốn thời gian như network data read/write; việc execute command vẫn tuần tự bằng single-thread. Vì vậy bạn không cần lo về thread safety.
...
Sau khi enable multi-thread, mặc định chỉ dùng multi-thread cho I/O write writes, tức gửi data đến client. Nếu cần enable multi-thread I/O read reads, cũng cần sửa Redis config file `redis.conf`:
...
Về giới thiệu chi tiết background thread của Redis, có thể xem bài [Redis 6.0 có những background thread nào?](https://juejin.cn/post/7102780434739626014).
...
Redis lưu expiration time của data thông qua một expired dictionary (có thể xem như hash table). Key của expired dictionary trỏ đến một key trong Redis database, value là một số nguyên kiểu long long lưu expiration time của database key mà key đó trỏ tới (UNIX timestamp với độ chính xác millisecond).

![Redis expired dictionary](https://oss.javaguide.cn/github/javaguide/database/redis/redis-expired-dictionary.png)

Expired dictionary được lưu trong structure redisDb:
...
Khi query một key, trước hết Redis kiểm tra key đó có tồn tại trong expired dictionary không (time complexity O(1)). Nếu không có thì return trực tiếp; nếu có thì cần kiểm tra key đã expired chưa, nếu expired thì xóa key rồi return null.

### Bạn có biết Redis expired key deletion policy không?

Giả sử bạn đặt một nhóm key chỉ sống 1 phút, vậy sau 1 phút Redis xóa nhóm key này như thế nào?

Các expired data deletion policy thường dùng gồm:
...
2. **Scheduled deletion**: Định kỳ random kiểm tra một batch key đã đặt expiration time, lần lượt kiểm tra các key này có expired không và xóa key expired. So với lazy deletion, scheduled deletion thân thiện hơn với memory nhưng kém thân thiện với CPU.
...
Redis dùng strategy kết hợp **scheduled deletion + lazy/lazy-style deletion**, đây cũng là lựa chọn của phần lớn cache framework. Scheduled deletion thân thiện hơn với memory, lazy deletion thân thiện hơn với CPU. Mỗi cách đều có ưu điểm; kết hợp giúp vừa thân thiện với CPU vừa thân thiện với memory.

Dưới đây là giới thiệu chi tiết cách scheduled deletion trong Redis hoạt động.

Quá trình scheduled deletion của Redis là random (định kỳ random kiểm tra một batch key đã đặt expiration time), nên không đảm bảo mọi expired key đều bị xóa ngay. Điều này giải thích tại sao có key đã expired nhưng chưa bị xóa. Ngoài ra, Redis giới hạn duration và frequency thực thi deletion operation để giảm ảnh hưởng của thao tác xóa lên CPU time.

Ngoài ra, scheduled deletion còn bị ảnh hưởng bởi execution time và tỷ lệ expired key:

- Nếu execution time đã vượt threshold thì interrupt scheduled deletion loop lần này để tránh dùng quá nhiều CPU time.
- Nếu tỷ lệ expired key trong batch này vượt một tỷ lệ nhất định thì lặp lại deletion flow này để dọn expired key tích cực hơn. Ngược lại, nếu tỷ lệ expired key thấp hơn tỷ lệ này thì interrupt scheduled deletion loop lần này để tránh làm quá nhiều việc nhưng thu hồi được quá ít memory.
...
**Kiểm soát frequency thực thi scheduled deletion như thế nào?**

Trong Redis, frequency của scheduled deletion được điều khiển bởi parameter **hz**. hz mặc định là 10, nghĩa là thực thi 10 lần mỗi giây, tức mỗi giây thử 10 lần để tìm và xóa expired key.

Giá trị hz nằm trong khoảng 1~500. Tăng hz sẽ nâng frequency scheduled deletion. Nếu muốn thực hiện scheduled deletion task thường xuyên hơn, có thể tăng hz phù hợp, nhưng điều này sẽ tăng CPU usage. Theo khuyến nghị chính thức của Redis, không nên đặt hz quá 100; với phần lớn user, mặc định 10 là đủ.
...
Nói thêm một chút, ngoài scheduled task xóa expired key, còn có một số scheduled task khác như đóng client connection timeout, update statistic; frequency thực thi các scheduled task này cũng do parameter hz quyết định.

**Tại sao scheduled deletion không xóa toàn bộ expired key?**
...
Để tránh các vấn đề này, có thể dùng các solution sau:
...
> Câu hỏi liên quan: MySQL có 20 triệu data, Redis chỉ lưu 200 nghìn data, làm thế nào đảm bảo data trong Redis đều là hot data?
...
1. **volatile-lru (least recently used)**: Chọn và evict data ít được sử dụng gần đây nhất từ data set đã set expiration time (`server.db[i].expires`).
2. **volatile-ttl**: Chọn và evict data sắp expired từ data set đã set expiration time (`server.db[i].expires`).
3. **volatile-random**: Random chọn và evict data từ data set đã set expiration time (`server.db[i].expires`).
4. **allkeys-lru (least recently used)**: Remove và evict data ít được sử dụng gần đây nhất từ data set (`server.db[i].dict`).
5. **allkeys-random**: Random chọn và evict data từ data set (`server.db[i].dict`).
6. **no-eviction** (memory eviction policy mặc định): Cấm evict data; khi memory không đủ chứa data write mới, write operation mới sẽ báo lỗi.

Sau version 4.0 thêm hai loại sau:

7. **volatile-lfu (least frequently used)**: Chọn và evict data ít được sử dụng thường xuyên nhất từ data set đã set expiration time (`server.db[i].expires`).
8. **allkeys-lfu (least frequently used)**: Remove và evict data ít được sử dụng thường xuyên nhất từ data set (`server.db[i].dict`).

`allkeys-xxx` nghĩa là evict data từ tất cả key-value, còn `volatile-xxx` nghĩa là evict data từ key-value đã set expiration time.
...
Về giải thích chi tiết eviction policy, có thể tham khảo Redis official document: <https://redis.io/docs/reference/eviction/>.
