---
title: "Chuyên đề Redis: cache, data structure, persistence, cluster, blocking và thực tiễn kỹ thuật"
...
Redis là một trong những hệ thống cache và in-memory data store hiệu năng cao phổ biến nhất trong backend. Khi học Redis, không nên chỉ dừng ở command và data type, mà còn phải hiểu các vấn đề kỹ thuật như chiến lược đọc/ghi cache, data structure tầng dưới, persistence, nguyên nhân blocking, quản lý memory, replication và cluster.

## Dành cho ai

- Backend developer muốn học một cách có hệ thống về nguyên lý Redis, thiết kế cache và thực tiễn kỹ thuật.
- Bạn đang chuẩn bị cho các câu hỏi phỏng vấn liên quan đến data structure, persistence, cluster, high availability và cache consistency của Redis.
- Bạn đã sử dụng Redis trong project nhưng chưa nắm rõ các sự cố cache, blocking, memory fragmentation và cơ chế cluster.
- Engineer cần dùng Redis để triển khai delayed task, message queue, leaderboard, shopping cart và các chức năng khác.

## Trọng tâm học

- Các data structure thường dùng của Redis phù hợp với những business scenario nào, encoding tầng dưới ảnh hưởng đến performance ra sao?
...
- Vì sao Redis có thể bị blocking, làm thế nào để chẩn đoán slow command, big key, hot key và tác động của persistence?
- Master-slave replication, Sentinel và Cluster lần lượt giải quyết vấn đề gì, failover có những quy trình quan trọng nào?
- Khi dùng Redis để triển khai delayed task và message queue, các giới hạn về khả năng và rủi ro về reliability là gì?

## Thứ tự đọc đề xuất

1. [Tổng hợp câu hỏi phỏng vấn cache cơ bản](./cache-basics.md): trước tiên hiểu các trường hợp sử dụng cache, các sự cố cache và vấn đề consistency.
2. [Tổng hợp câu hỏi phỏng vấn Redis (phần 1)](./redis-questions-01.md), [Tổng hợp câu hỏi phỏng vấn Redis (phần 2)](./redis-questions-02.md): xây dựng danh sách các vấn đề thường gặp về Redis.
3. [Giải thích chi tiết 5 data type cơ bản của Redis](./redis-data-structures-01.md), [Giải thích chi tiết 3 data type đặc biệt của Redis](./redis-data-structures-02.md): nắm vững một cách có hệ thống data structure và các trường hợp sử dụng.
...
- [Tổng hợp câu hỏi phỏng vấn cache cơ bản](./cache-basics.md): giải thích các trường hợp sử dụng cache, cache penetration, cache breakdown, cache avalanche, cache consistency và cache eviction.
...
### Data structure và ứng dụng điển hình

- [Giải thích chi tiết 5 data type cơ bản của Redis](./redis-data-structures-01.md): tìm hiểu data structure tầng dưới và các trường hợp sử dụng của String, List, Hash, Set, Sorted Set.
- [Giải thích chi tiết 3 data type đặc biệt của Redis](./redis-data-structures-02.md): tìm hiểu cách dùng và trường hợp sử dụng phù hợp của Bitmap, HyperLogLog và Geospatial.
- [Vì sao Redis dùng skip list để triển khai ordered set](./redis-skiplist.md): tìm hiểu cấu trúc skip list, độ phức tạp của truy vấn và lựa chọn triển khai của Sorted Set.
...
- [Giải thích chi tiết cơ chế persistence của Redis](./redis-persistence.md): trình bày một cách có hệ thống về RDB, AOF, AOF rewrite và hybrid persistence.
...
- Các data type thường gặp của Redis phù hợp với những business scenario nào?
...
- Redis triển khai delayed task như thế nào? Rủi ro về reliability nằm ở đâu?
- Redis Stream có những giới hạn nào so với message queue truyền thống?
