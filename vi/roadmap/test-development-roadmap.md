---
title: "Lộ trình học Test Development (bản mới nhất 2026): Từ testing đến Quality Engineering trong kỷ nguyên AI"
description: "Lộ trình học bản mới nhất 2026 dành cho Test Development và software testing, bao quát computer science fundamentals, ngôn ngữ lập trình, testing theory, API automation, UI automation, performance testing, CI/CD, test platform, AI-assisted testing và testing ứng dụng AI."
category: Lộ trình học
head:
  - - meta
    - name: keywords
      content: lộ trình học Test Development,lộ trình Test Development, lộ trình học software testing,lộ trình học Test Development 2026,AI testing,automated testing,API testing,UI automated testing,performance testing,test platform,Quality Engineering
---

Xin chào, tôi là Guide. Đây là bản mới nhất 2026 của lộ trình học Test Development.
...
Điều này có nghĩa Test Development cần bổ sung một năng lực mới: **đánh giá chất lượng của hệ thống không deterministic**.
...
- Sau khi model switch, Prompt adjustment hoặc Embedding update, chất lượng tốt lên hay kém đi?

AI không thể thay thế khả năng phán đoán testing tốt. AI có thể giúp bạn tạo test case và script dự kiến nhanh hơn, nhưng cuối cùng bạn phải đánh giá coverage đã đầy đủ chưa, assertion có hiệu quả không, failure có thực sự chỉ ra vấn đề không.
...
- Có thể viết test code dễ bảo trì, hạn chế viết script dùng một lần.
- Có thể đưa automation vào engineering process, chạy trong CI và daily regression.
- Có thể giải thích nguyên nhân phía sau một failure, đưa ra bằng chứng từ log, data và chain.

## Giai đoạn một: Bổ sung computer science fundamentals

Phỏng vấn Test Development cũng sẽ hỏi computer science fundamentals, đặc biệt là khi tuyển sinh viên mới tốt nghiệp, thực tập sinh và tại các công ty công nghệ lớn.
...
Khi hỏi computer network trong Test Development, câu hỏi thường gắn với tình huống troubleshooting. Ví dụ, kiểm tra thế nào khi page load chậm, định vị ra sao khi API thỉnh thoảng timeout, các nguyên nhân có thể có khi bắt được 401 / 403 / 500 từ App. Bạn không thể chỉ học thuộc concept, mà phải có thể lần theo request chain để trình bày.
...
| Ngôn ngữ      | Trường hợp phù hợp hơn                                                                                           | Đề xuất                                                                                                              |
| ------------- | ---------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| Java          | Đã có nền tảng Java, công ty mục tiêu dùng Java làm tech stack, muốn làm test platform hoặc backend quality tool | Có thể trực tiếp dùng Java để ứng tuyển Test Development, trọng tâm bổ sung JUnit, TestNG, Rest Assured, Spring Boot |
| Python        | Muốn nhanh chóng viết script, API automation, xử lý data, phân tích log, gọi AI toolchain                        | Rất phù hợp để bắt đầu Test Development, trọng tâm bổ sung pytest, requests, Playwright, Locust                      |
| Java + Python | Muốn bao phủ nhiều vị trí và project hơn                                                                         | Java làm platform và engineering foundation, Python làm automation script và AI-assisted tool                        |

Nếu vốn là Java backend, đừng vứt bỏ Java rồi học lại Python chỉ vì Test Development. Kinh nghiệm về Spring Boot, MySQL, Redis, API design, unit test và troubleshooting log trong Java đều có thể chuyển sang Test Development, đồng thời cũng dễ trình bày hơn trong CV.

Nếu chưa có nền tảng ngôn ngữ rõ ràng, khi chuẩn bị Test Development trong ngắn hạn có thể học Python trước. Python gọn hơn khi viết API automation, xử lý data, batch script và gọi AI API.
...
- Có thể đọc hiểu cấu trúc thư mục project, biết code, config, test và log lần lượt nằm ở đâu.
...
Công việc Test Development không chỉ diễn ra trên browser. Nhiều vấn đề cuối cùng đều liên quan đến data, environment, log và release process.
...
- Đọc hiểu cấu trúc table, biết ảnh hưởng của primary key, unique index, normal index và foreign key.
...
Bài tập ở giai đoạn này có thể rất cụ thể: tự viết một service nhỏ, dùng Docker khởi động MySQL và backend service, sau đó dùng GitHub Actions hoặc Jenkins chạy một nhóm API automation test. Dù chức năng rất nhỏ, chỉ cần hoàn thiện cả chain cũng tốt hơn nhiều so với chỉ học tool.

## Giai đoạn bốn: Testing theory và thiết kế test case

Testing theory không nên dừng ở việc học thuộc thuật ngữ. Nó giải quyết một vấn đề cụ thể: trước một function, làm sao đánh giá mình đã testing đủ chưa?

Nội dung cơ bản bao gồm:

- Quy trình testing: requirement analysis, test plan, thiết kế test case, execution, defect tracking, regression, release verification và retrospective.
- Phân loại testing: unit test, integration test, system test, acceptance test, regression test và smoke test.
- Loại testing: functional, API, performance, security, compatibility, usability và stability.
- Phương pháp thiết kế test case: equivalence partitioning, boundary value, decision table, cause-effect graph, state transition, orthogonal experiment và error guessing.
- Quản lý defect: defect title, reproduction steps, actual result, expected result, environment information, log và screenshot.

Trong phỏng vấn thường gặp các câu hỏi tình huống như test elevator, water cup, login page, shopping cart, WeChat Moments, red packet và file upload. Khi trả lời, đừng chỉ liệt kê theo function point mà có thể triển khai theo các khía cạnh:

- Luồng function: normal path và exception path.
- Biên dữ liệu: null value, quá dài, special character, duplicate và invalid format.
...
- Observability: log, instrumentation, alert và error code có giúp định vị vấn đề không.

AI có thể giúp tạo bản nháp đầu tiên, nhưng bạn phải biết review. Ví dụ, khi yêu cầu AI tạo điểm kiểm thử cho login page, AI có thể bao phủ account, password, verification code và remember login, nhưng thường bỏ sót risk control, rate limiting, account lock, third-party login, Token renewal, concurrent login và audit log.

Test Development phải có thể bổ sung những phần bị bỏ sót này.

## Giai đoạn năm: API testing và API automation

API automation là một trong những phần Test Development nên ưu tiên hàng đầu.

Lý do rất đơn giản: API ổn định hơn UI, tốc độ thực thi nhanh hơn và dễ tích hợp vào CI hơn. API regression là phần automation test chủ lực của nhiều team.

Trước tiên bắt đầu từ tool. Với các tool như Postman, Apifox, Reqable và Insomnia, ít nhất phải biết một tool, có thể hoàn thành API debugging, environment variable, pre-request script, post-request assertion và test collection execution.

Sau đó viết code. Python có thể dùng `requests + pytest`, Java có thể dùng `JUnit / TestNG + Rest Assured`. Một API automation framework tốt ít nhất cần bao gồm:

- Cấu hình environment: test environment, staging environment, API domain, account và Token.
- Đóng gói request: xử lý thống nhất Header, Cookie, authentication, timeout và retry.
- Quản lý data: chuẩn bị test data, cleanup, parameterization và database verification.
- Hệ thống assertion: status code, response field, business code, database state và message queue side effect.
- Xuất report: Allure, HTML report, failure log, request và response detail.
- Tích hợp CI: thực thi theo mỗi commit hoặc theo lịch hằng ngày, sau failure có thể định vị đến test case cụ thể.
...
- Giúp review xem test case có chỉ testing success path hay không.
...
Chi phí của UI automation cao hơn API automation. Page structure có thể thay đổi, element locator có thể mất hiệu lực, network và rendering tạo ra instability; nếu bảo trì không tốt, nó rất dễ biến thành “ngày nào cũng có người sửa script”. Vì vậy UI automation phù hợp hơn để bao phủ các main flow có giá trị cao, ổn định và đi qua nhiều page, chẳng hạn login, place order, payment, approval và publish.
...
Nếu bắt đầu học mới từ năm 2026, tôi đề xuất ưu tiên Playwright, sau đó tìm hiểu Selenium. Hệ sinh thái và giá trị phỏng vấn của Selenium vẫn còn, nhưng về stability và development experience của project mới, Playwright thường dễ chịu hơn.
...
Performance testing không thể chỉ dừng ở việc mở JMeter lên chạy một lần rồi dán một biểu đồ QPS.
...
- k6: trải nghiệm viết script hiện đại hơn, phù hợp với developer collaboration.
...
- Metric tài nguyên: CPU, memory, disk I/O, network I/O, thread count và connection count.
- Metric dependency: database slow SQL, connection pool, Redis hit rate và message queue backlog.
...
- Bottleneck nằm ở đâu, bằng chứng là gì?
- Metric thay đổi thế nào trước và sau optimization?
- Giới hạn của conclusion hiện tại là gì?

Load test không có monitoring sẽ có giá trị rất thấp. Ít nhất phải xem được service log, system resource, database metric và error stack. Khi response chậm, trước tiên cần phán đoán đó là do application thread exhausted, database chậm, external API chậm, GC, network hay chính performance test script có vấn đề.

## Giai đoạn tám: CI/CD, quality platform và precise testing

Ranh giới để Test Development tiến lên thường nằm ngoài script: có thể tích lũy năng lực quality vào team process hay không.
...
- Quản lý test case: quản lý API, scenario, priority, tag và execution status.
- Lập lịch automation: trigger test theo project, branch, environment và tag.
- Tổng hợp report: hiển thị pass rate, failure reason và historical trend.
- Quản lý test data: chuẩn bị data như account, order, inventory và approval flow.
- Liên kết defect: tự động liên kết failure test case với defect hoặc thông báo cho người phụ trách.
...
Cách dùng thực tế hơn là chia nhỏ task, để AI tạo các phương án ban đầu và hỗ trợ review:

```text
Hãy dựa trên mô tả requirement bên dưới để đưa ra điểm kiểm thử.
Yêu cầu:
1. Phân loại theo function, API, permission, security, compatibility, performance và exception scenario;
2. Với mỗi điểm kiểm thử, viết precondition, operation step và expected result;
...
Giá trị của loại Prompt này là giúp AI liệt kê phạm vi khả năng. Phán đoán thực sự vẫn đến từ log, data, code và business rule.
...
| -------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| LLM API app    | Structured output, timeout retry, rate limiting, degradation, cost, audit                    | Phân tích CV, hỏi đáp chăm sóc khách hàng, phân loại văn bản |
| RAG app        | Document parsing, chunking, retrieval, Rerank, answer faithfulness, citation traceability    | Knowledge base doanh nghiệp, hỏi đáp chính sách              |
| Agent app      | Tool selection, parameter generation, execution trace, permission boundary, failure recovery | Tự động tra cứu order, tự động tạo report                    |
| Multimodal app | Image/audio input, recognition accuracy, abnormal file, security boundary                    | Kiểm duyệt hình ảnh, nhận dạng hóa đơn                       |
...
- Generation evaluation: xem câu trả lời có đúng không, có faithful với tài liệu không, có trích dẫn bằng chứng không.
...
- Đóng gói request, authentication, parameterization, data preparation và cleanup.
...
Khi phỏng vấn cần tập trung trình bày stability: chọn element locator thế nào, xử lý wait ra sao, bảo trì thế nào sau khi page thay đổi, phán đoán thế nào để biết failure là do script hay business issue.

### Project ba: Performance testing và định vị vấn đề

Chọn một API chain, chẳng hạn product query, place order, login hoặc search.

Project cần bao gồm:

- Thiết kế performance test scenario: single API, mixed scenario, peak scenario và stability scenario.
...
Project này rất phù hợp để chứng minh bạn biết định vị vấn đề, còn việc dùng tool chỉ là một mắt xích trong đó.

### Project bốn: Nền tảng đánh giá chất lượng ứng dụng AI
...
Project này không cần làm quá lớn. Điều quan trọng là thể hiện bạn hiểu phương pháp đánh giá chất lượng AI application, chứ không chỉ biết yêu cầu model trả lời câu hỏi.

## Trình bày năng lực Test Development khi phỏng vấn thế nào

Khi phỏng vấn Test Development, đừng mô tả bản thân là “người biết rất nhiều tool”. Tool chỉ là điểm bắt đầu, điều interviewer thực sự muốn biết là bạn có thể bảo đảm quality hay không.
...
- Tôi phân tích requirement và thiết kế điểm kiểm thử thế nào.
...
- Hiểu database, cache, message queue và distributed chain hơn, có thể testing các risk bên ngoài bề mặt function.
...
- Hiểu business flow và tư duy testing hơn.
...
Tháng 2, chinh phục testing theory và API automation. Luyện có hệ thống thiết kế test case, hoàn thành một API automation framework, tích hợp test report và CI. Cuối tháng này, CV nên có một API automation project có thể giải thích rõ.
...
Tháng 6, tập trung chuẩn bị CV và phỏng vấn. Chỉnh project thành phiên bản có thể trình bày khi phỏng vấn: background, solution, technology selection, difficulty, result, risk và retrospective. Luyện các câu hỏi tình huống testing phổ biến, câu hỏi algorithm cơ bản và câu hỏi đào sâu về project.
...
Test Development không phù hợp với người chỉ muốn tìm được việc một cách nhẹ nhàng. Nó thiên về kỹ thuật hơn functional testing truyền thống, đồng thời nhấn mạnh quality perspective hơn pure business development. Bạn vừa phải biết viết code, vừa phải sẵn sàng nghiền ngẫm nhiều lần về boundary, exception, failure và risk.
...
Trước tiên hãy chạy thông một chain: từ requirement đến test case, từ API đến assertion, từ script đến CI, từ failure đến định vị vấn đề. Năng lực cạnh tranh của Test Development lớn dần từng chút một trong chính chain này.
...
- [Lộ trình học AI testing của Black Horse Programmer (bản đầy đủ chính thức 2026)](https://yun.itheima.com/subject/testmap/index.html)
